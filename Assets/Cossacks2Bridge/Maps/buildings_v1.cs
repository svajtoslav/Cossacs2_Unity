// buildings_v1.cs
// V21 shim: old OC/COMPLEX buildings adapter is intentionally disabled.
...
namespace Cossacks2Bridge.UnityAdapters.Maps
{
    ...
    {
        public const string Contract = "V21_DISABLED_OC_COMPLEX_REDIRECT_TO_3INU_MD";
        ...
        private void OnEnable()
        {
            ...
            StartCoroutine(TryKickSettlement3InuV4LikeOriginal());
        }

        private IEnumerator TryKickSettlement3InuV4LikeOriginal()
        {
            for (int attempt = 1; attempt <= Mathf.Max(1, MaxSearchAttempts); attempt++)
            {
                ...
                if (mode != null)
                {
                    ...
                    {
                        build.Invoke(mode, new object[] { mapPath, "old-buildings-v1-shim-v21" });
                        Debug.Log("[C2:BUILDINGS OC V1 DISABLED] kicked 3INU V21 parser map='" + mapPath + "'");
                        enabled = false;
                        yield break;
                    }
            ...
            Debug.LogWarning("[C2:BUILDINGS OC V1 DISABLED] timeout. If no [C2:SETTLEMENT 3INU V21] appears, C2SettlementBuildings3INUParserLikeOriginal.cs is not installed/compiled.");
            enabled = false;
        }
    }
}
