Shader "Cossacks2Bridge/RoadLayerV17"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ZWrite Off
            ZTest LEqual
            // V17: hardware ZTest remains enabled. Exact triangle-height sampling stays,
            // but aggressive depth forcing is removed so large hills/mountains can occlude roads again.
            Offset -1, -1
            Cull Off

            // Original road.xml color/alpha is kept from V8/V9.
            // Unity adaptation: depth test remains enabled so roads do not render through hills; late queue prevents terrain transparent layers from drawing over roads. Bias is reduced to preserve small overlap smoothing without punching through mountains.
            ...
            v2f vert(appdata v)
            {
                ...
                // V17: disable clip-space depth pull. Exact CPU-side triangle-height sampling
                // is sufficient and avoids mountain bleed-through.
