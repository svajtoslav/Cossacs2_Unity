...
void Draw_PLIST(int x,int y,int Lx,int Ly,int Index,byte Active,int param){
	...
	if(Index<CSYS.CCH[CSYS.CurChannel].NPlayers){
		...
		RLCFont* FONT=Active==1?&BlackFont:&RedFont;
		RLCFont* FONT1=NULL;
		char cc3[256];
...
void ShowCentralMessage2C(char* Message){
	...
	if(LoadGP>0){
		...
		ShowString((RealLx-L)/2+1,(RealLy-GetRLCHeight(YellowFont.RLC,'W'))/2-10,Message,&BlackFont);
...
void DrawCentralLoadingText(char* Message){
	...
	if(LoadGP>0){
		...
		GPS.ShowGP((RealLx-L)/2,(RealLy-80)/2,LoadGP,1,0);
		L=GetRLCStrWidth(Message,&BlackFont);
...
void DrawCentralLoadingTextC2(char* Message){
	...
	if(LoadGP>0){
		if(LoadBlur>0)GPS.ShowGP(0,0,LoadBlur,0,0);
		int L=GPS.GetGPWidth(LoadGP,0);
		GPS.ShowGP((RealLx-L)/2,(RealLy-80)/2,LoadGP,2,0);
		L=GetRLCStrWidth(Message,&YellowFont);
		ShowString((RealLx-L)/2+1,(RealLy-GetRLCHeight(YellowFont.RLC,'W'))/2-14,Message,&BlackFont);
		ShowString((RealLx-L)/2,(RealLy-GetRLCHeight(YellowFont.RLC,'W'))/2-15,Message,&YellowFont);
		return;
	};
};
CIMPORT
...
int Process_GSC_ChatWindow(bool Active,RoomInfo* RIF){
	...
	LocalGP SCROL3("Interf3\\elements\\scroll3");
	...
	if(GameVersion==11){
		CHVIEW[0]->Font1=&SpecialBlackFont;
		CHVIEW[0]->Font2=&SpecialRedFont;
	}

	char CHATMESSAGE[256]="";
	InputBox* CHATBOX=NULL;
	if(GameVersion==11)CHATBOX=DSS.addInputBox(NULL,INI.GetInt("ENTER_CHAT_STRING_X"),INI.GetInt("ENTER_CHAT_STRING_Y"),CHATMESSAGE,200,INI.GetInt("ENTER_CHAT_STRING_LX"),INI.GetInt("ENTER_CHAT_STRING_LY"),&SpecialRedFont,&SpecialBlackFont);
	else CHATBOX=DSS.addInputBox(NULL,INI.GetInt("ENTER_CHAT_STRING_X"),INI.GetInt("ENTER_CHAT_STRING_Y"),CHATMESSAGE,200,INI.GetInt("ENTER_CHAT_STRING_LX"),INI.GetInt("ENTER_CHAT_STRING_LY"),&RedFont,&BlackFont);
	CHATBOX->Active=1;
	...
	if(GameVersion==11){
		CHVIEW[1]->Font1=&SpecialBlackFont;
		CHVIEW[1]->Font2=&SpecialRedFont;
	}
	...
	if(GameVersion==11){
		CHVIEW[2]->Font1=&SpecialBlackFont;
		CHVIEW[2]->Font2=&SpecialRedFont;
	}
