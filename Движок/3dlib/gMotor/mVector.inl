...
_finl const Vector3D& Vector3D::operator +=( const Vector3D& vec )
{
	x += vec.x;
	y += vec.y;
	z += vec.z;
...
_finl const Vector3D& Vector3D::operator *=( const float val )
{
	x *= val;
	y *= val;
	z *= val;
...
_finl const Vector3D& Vector3D::operator *=( const Matrix3D& m )
{
	Vector3D tmp;
...
_finl const Vector3D& Vector3D::operator *=( const Matrix4D& m )
{
...
/*****************************************************************/
_inl Vector4D::Vector4D( float _x, float _y, float _z ) : x(_x), y(_y), z(_z), w(1.0f)
{}
...
_inl Vector4D::Vector4D( const Vector3D& orig ) : x(orig.x), y(orig.y), z(orig.z), w(1.0f) 
{}
