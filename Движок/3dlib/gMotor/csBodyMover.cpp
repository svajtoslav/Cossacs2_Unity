...
bool BodyMover::OnMouseMove( int mX, int mY, DWORD keys )
{
	...
	pCamera = BaseCamera::GetActiveCamera();

	if (!pCamera) return false;

	Line3D ray;
	pCamera->GetPickRay( mX, mY, ray, NULL );//, &(pNode->GetTransform()) );
	...
	for (int i = 0; i < GetNChildren(); i++)
	{
		...
		if (bRiseRotateMode && prX != -1000 && prY != -1000 && ray)
		{
			...
			if (ray->IntersectPlane( Plane::xOy, tr ))
			{
				...
				const Matrix4D& ctm = pCamera->GetTransform();
...
bool BodyMover::OnMouseLButtonDown( int mX, int mY )	
{
	if (!IsActive()) return false;
...
bool BodyMover::OnMouseRButtonDown( int mX, int mY )
{
	if (!IsActive()) return false;
	return false;
}

bool BodyMover::OnMouseLButtonDblclk( int mX, int mY )
{
	if (!IsActive()) return false;
	return false;
}
...
void BodyMover::DrawHeightMarker( const Matrix4D& tr )
{
	...
	static BaseMesh hm;
	BEGIN_ONCE
		hm.create( 6, 0, vf2Tex, ptLineList );	
...
bool HudMover::OnMouseLButtonDown( int mX, int mY )
{
	if (GetKeyState( VK_CONTROL ) < 0)
	{
		bScaling = true;
...
bool HudMover::OnMouseLButtonUp( int mX, int mY )
{
	bScaling = false;
	bDragging = false;
	return true;
...
bool HudMover::OnMouseRButtonDown( int mX, int mY )
{
	return false;
}

bool HudMover::OnMouseLButtonDblclk( int mX, int mY )
{
	return false;
}
