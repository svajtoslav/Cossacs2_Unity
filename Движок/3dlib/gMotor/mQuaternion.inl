...
_inl void Quaternion::operator -=( const Quaternion& q ) 
{
	v.sub( q.v );
	s -= q.s;
}

_inl bool Quaternion::InSameHemisphere( const  Quaternion& quat ) const
{
	return (dot( quat ) >= 0.0f);
} // Quaternion::inSameHemisphere
