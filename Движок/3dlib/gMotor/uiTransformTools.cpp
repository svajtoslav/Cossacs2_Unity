...
{ 
	...
	if (m_bDragging)
	{
		...
		pCam->ScreenToWorldSpace( tr0 );
		pCam->ScreenToWorldSpace( tr1 );
	...
	else
	{
		...
		Vector2Df dirX( m.e00, m.e01 );
		Vector2Df dirY( m.e10, m.e11 );
		Vector2Df dirZ( m.e20, m.e21 );
		Vector2Df mpos( mx, my );
...
bool MoveTool::OnMouseLButtonDown( int mX, int mY )				
{ 
	m_bDragging = true;
	m_Mx = mX;
	m_My = mY;
...
bool MoveTool::OnMouseLButtonUp( int mX, int mY )				
{ 
	m_bDragging = false;
	return false; 
}
...
void MoveTool::Render()
{
	if (!m_Arrow)
	{
		m_Arrow = (Geometry*)Node::CreateFromFile( "Models\\arrow.c2m" );
		AddChild( m_Arrow );
		m_pShader = FindChild<DeviceStateSet>( "transformtools" );
		if (!m_pShader) m_pShader = AddChild<DeviceStateSet>( "transformtools" );
	}
	...
	if (pCam) 
	{
		...
		Vector4D vs( pos );
		pCam->WorldToProjectionSpace( vs );
		vs.x += m_Size;
		pCam->ProjectionToWorldSpace( vs );
		vs -= pos;
		sc = vs.norm();
	}
...
SelectionTool::SelectionTool()
{
	...
	m_pSelShader = AddChild<DeviceStateSet>( "selection" );
...
void SelectionTool::Render()
{
	...
	for (int i = 0; i < GetNSelectedNodes(); i++)
	{
		...
		IRS->SetWorldMatrix( pNode->GetParentWorldTM() );
		pNode->Render();
	}
...
bool SelectionTool::OnMouseLButtonDown( int mX, int mY )
{
	return false;
}

bool SelectionTool::OnMouseLButtonUp( int mX, int mY )
{return false;
	...
	while (it)
	{
		...
		if (pNode->HasFn( Geometry::Magic() ))
		{
			...
			float dist = pGeom->GetPrimitive().PickPoly( tray, triIdx );
			if (triIdx >= 0)
			{
	...
	return false;
} // SelectionTool::OnMouseLButtonUp

void SelectionTool::SelectAll()
{
	Node* pScene = Root::instance()->FindChild<Group>( "Scene" );
	if (!pScene) return;
	m_pSelection->RemoveChildren();
...
void SelectionTool::UnselectNode( TransformNode* pNode )
{
	for (int i = 0; i < m_SelNodes.size(); i++)
	{
		if (m_SelNodes[i] == pNode) m_SelNodes.erase( m_SelNodes.begin() + i );
	}
