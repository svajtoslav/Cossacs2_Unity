...
void BitmapFont::Flush()
{
	...
	static shID	= IRS->GetShaderID( "hud" );
	if (s_BM.getNVert() == 0)
	{
		s_BM.create( c_MaxCharInst*4, 0, vfTnL );
		s_BM.setIsQuadList( true );
	}
...
void WidgetManager::OnDrop( int mX, int mY, DWORD ctx, DWORD obj )
{
	...
	while (it)
	{
		Control* pCtl = (Control*)(Node*)it;
		pCtl->OnDrop( mX, mY, ctx, obj );
		++it;
	}
} // WidgetManager::OnDrop

void WidgetManager::OnMouseWheel( int delta )
{

}

void WidgetManager::OnMouseMove( int mX, int mY, DWORD keys )
{

}

void WidgetManager::OnMouseLButtonDown( int mX, int mY )
{

}

void WidgetManager::OnMouseMButtonDown( int mX, int mY )
{

}

void WidgetManager::OnMouseRButtonDown( int mX, int mY )
{

}

void WidgetManager::OnMouseLButtonUp( int mX, int mY )
{

}

void WidgetManager::OnMouseMButtonUp( int mX, int mY )
{

}

void WidgetManager::OnMouseRButtonUp( int mX, int mY )
{

}

void WidgetManager::OnMouseLButtonDblclk( int mX, int mY )
{

}

void WidgetManager::OnMouseRButtonDblclk( int mX, int mY )
{

}

void WidgetManager::OnMouseMButtonDblclk( int mX, int mY )
{

}

void WidgetManager::OnKeyDown( DWORD keyCode,  DWORD flags )
{

}

void WidgetManager::OnChar( DWORD charCode, DWORD flags )
{

}

void WidgetManager::OnKeyUp( DWORD keyCode,  DWORD flags )
{
...
int	WidgetManager::CreateFont( const char* name, int height, DWORD charset, bool bBold, bool bItalic )
{
	...
	TextureDescr td;
	td.setValues( texW, texH, cfARGB4444, mpManaged, 1, tuProcedural );
	int texID = IRS->CreateTexture( fullName, td );
	pFont->m_TexID		= texID;
...
int	WidgetManager::CreateUniformFont( const char* texName, int charW, int charH )
{
	...
	if (texID == -1) return -1;
	pFont->m_TexID = texID;
	const TextureDescr* td = IRS->GetTextureDescr( texID );
	int texW = td->getSideX();
	int texH = td->getSideY();
...
bool WidgetManager::DrawString( int fontID, const char* str, const Vector3D& pos, DWORD color, int spacing )
{
	...
	while (*ch)
	{
		pFont->AddCharInst( *ch, p, color );
		p.x += pFont->GetCharW( *ch ) + spacing;
		ch++;
	}
	return true; 
...
void WidgetManager::OnDestroyRenderSystem()
{
	for (int i = 0; i < m_NFonts; i++)
	{
...
} // WidgetManager::OnDestroyRenderSystem
