...
public:
	_inl					Sphere		(){}
	_inl					Sphere		( const Vector3D& _center, float _radius );
	...
	_inl const Vector3D&	GetCenter	() const;
	_inl float				GetRadius	() const;
	_inl void				SetCenter	( const Vector3D& _center );
	_inl void				SetRadius	( float _radius );
...
public:
	...
	void					Create			( const Matrix4D& tm, 
												...
												float wToH, 
												float zn, float zf );
	_inl bool				PtIn			( const Vector3D& pt );
	_inl const Plane&		GetPlane		( int idx ) const { return (&plNear)[idx]; }
