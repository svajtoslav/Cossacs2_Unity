...
const char*	TextureManager::GetTextureName( int texID )
{
...
int TextureManager::GetTextureSizeBytes( int texID )
{
	assert( texID >= 0 && texID < m_NTextures );
	DXDevice* pDevice = D3DRenderSystem::instance().GetDevice();
	return m_Texture[texID].GetSizeBytes();
} // TextureManager::GetTextureSizeBytes
...
{
	if (texID < 0 || texID >= m_NTextures) return false;
...
const TextureDescr* TextureManager::GetTextureDescr( int texID )
{
