...
void ImpostorSurface::Free()
{
    ...
    m_NAllocated = 0;
    IRS->SetRenderTarget( m_TextureID );
    IRS->ClearDeviceTarget( 0x00000000 );
...
void ImpostorCache::Flush()
{
    ...
    Matrix4D camTM      = GetCameraTM();
    Matrix4D prTM       = GetCameraProjTM();
    ...
    if (bm.getMaxVert() == 0)
    {
        bm.create( c_RenderQSize*4, 0, vf2Tex );
...
bool ImpostorCache::CreateImpostor( ImpostorInstance& imp )
{
    ...
    {
        IRS->SetRenderTarget( surf.m_TextureID, m_DepthID );
    ...
    IRS->SetViewMatrix      ( impViewTM );
    IRS->SetProjectionMatrix( impProjTM );

    if (imp.m_AnimID != 0xFFFFFFFF) IMM->Animate( imp.m_ModelID, imp.m_AnimID, float( imp.m_AnimTime )*m_TimeQuant );
    IMM->Render( imp.m_ModelID );
    return true;
...
bool ImpostorCache::Init()
{
    ...
    TextureDescr td;
    td.setValues( c_ImpostorSurfaceSide, c_ImpostorSurfaceSide, cfARGB4444, mpVRAM, 1, tuRenderTarget );
    for (int i = 0; i < c_NumImpostorSurfaces; i++)
    {
        ImpostorSurface& surf = m_Cache.GetBlock( i );
        char tname[256];
        sprintf( tname, "ImpostorSurf%d", i );
        surf.m_TextureID = IRS->CreateTexture( tname, td );
        surf.Free();
    }

    //  create depth buffer surface
    td.setDsFmt( dsfD16 );
    td.setTexUsage( tuDepthStencil );
    m_DepthID = IRS->CreateTexture( "ImpostorDepthBuffer", td );
    m_bInited = true;
    return true;
} // ImpostorCache::Init
