...
public:
	...
	_inl void				addWeighted		( const Vector3D& v1, const Vector3D& v2,
												float w1, float w2 );
	...
	_finl const Vector3D& 	operator +=		( const Vector3D& vec );
	_finl const Vector3D& 	operator -=		( const Vector3D& vec );
	_finl const Vector3D& 	operator *=		( const float val );
	_finl const Vector3D& 	operator *=		( const Vector3D& vec );
	_finl const Vector3D& 	operator *=		( const Matrix3D& m );
	_finl const Vector3D& 	operator *=		( const Matrix4D& m );
...
public:
    ...
    _inl            		Vector4D( const Vector3D& orig );
...
DWORD VectorToColor( const Vector3D& v, float h );
Vector3D ToTangentSpace( const Vector3D& ldir, const Vector3D& normal, const Vector3D& right = Vector3D::oX );
