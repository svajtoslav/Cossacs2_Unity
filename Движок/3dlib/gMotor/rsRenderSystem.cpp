...
void rsSetTexture( int texID, int stage )
{
...
void rsEnableZ( bool enable )
{
	...
	static int lines3D_blend	 = IRS->GetShaderID( "lines3D_blend" );
	static int lines3D_blend_noZ = IRS->GetShaderID( "lines3D_blendW" );
	if (enable) 
	{
...
void rsLine( float x1, float y1, float x2, float y2, float z, DWORD color1, DWORD color2 )
{
	...
	{
		g_bmLines2D.create( c_PrimPoolSize, 0, vfTnL, ptLineList );	
		rsEnableZ();
	}
...
void rsLine( const Vector3D& a, const Vector3D& b, DWORD color1, DWORD color2 )
{
	...
	{
		g_bmLines3D.create( c_PrimPoolSize, 0, vf2Tex, ptLineList );
		rsEnableZ();
	}
...
void rsRect( const Rct& rct, const Rct& uv, float z, DWORD ca, DWORD cb, DWORD cc, DWORD cd )
{
	...
	{
		g_bmPoly2D.create( c_PrimPoolSize, 0, vfTnL, ptTriangleList );
...
void rsPoly( const Vector3D& a, const Vector3D& b, const Vector3D& c, DWORD acol, DWORD bcol, DWORD ccol )
{
	...
	{
		g_bmPoly3D.create( c_PrimPoolSize, 0, vf2Tex, ptTriangleList );
		rsEnableZ();
	}
...
void rsFlushPoly3D( bool bShaded )
{
	if (g_bmPoly3D.getNVert() == 0) return;
	if (bShaded) IRS->Draw( g_bmPoly3D ); else IRS->DrawPrim( g_bmPoly3D );
	g_bmPoly3D.setNVert( 0 );
	g_bmPoly3D.setNPri( 0 );
} // rsFlushPoly3D

void rsFlushLines3D( bool bShaded )
{
	if (g_bmLines3D.getNVert() == 0) return;

	if (bShaded) IRS->Draw( g_bmLines3D ); else IRS->DrawPrim( g_bmLines3D );
	g_bmLines3D.setNVert( 0 );
	g_bmLines3D.setNPri( 0 );
}

void rsFlushLines2D( bool bShaded )
{
	if (g_bmLines2D.getNVert() == 0) return;

	if (bShaded) IRS->Draw( g_bmLines2D ); else IRS->DrawPrim( g_bmLines2D );
	g_bmLines2D.setNVert( 0 );
	g_bmLines2D.setNPri( 0 );
}

void rsFlushPoly2D( bool bShaded )
{
	if (g_bmPoly2D.getNVert() == 0) return;
	if (bShaded) IRS->Draw( g_bmPoly2D ); else IRS->DrawPrim( g_bmPoly2D );
	g_bmPoly2D.setNVert( 0 );
	g_bmPoly2D.setNPri( 0 );
}

void rsFlush( bool bShaded )
{
...
void DrawCurve( float width, int nPoints, const Vector3D* points, const DWORD* colors )
{
    ...
    for (int i = 1; i < nPoints; i++)
    {
        ...
        DWORD col1 = colors[i - 1];
        DWORD col2 = colors[i];
        DWORD coln = (i == nPoints - 1) ? colors[i] : colors[i + 1];
        ...
        rsQuad2D( pa, pc, pb, pd, Rct( cu, 0.0f, du, 1.0f ), col1, col2, col1, col2 );
        cu += du;
        ...
        if (i < nPoints - 1)
        {
            ...
            for (int j = 0; j < nSteps; j++)
            {
                ...
                DWORD cva = ColorValue::Gradient( col2, coln, grad );
                grad += dgrad;
                DWORD cvb = ColorValue::Gradient( col2, coln, grad );

                float du = dAlpha*rrad/width;
                rsQuad2D( pa, pc, pb, pd, Rct( cu, 0.0f, du, 1.0f ), cva, cvb, cva, cvb );
                cu += du;
            }
        }
    }
...
void DrawPoint( const Vector3D& pt, DWORD clr, float rad )
{
...
void DrawCircle( float x, float y, float radius, DWORD fillColor, DWORD linesColor, int nSegments )
{
	...
	for (float phi = 0.0f; phi < c_DoublePI; phi += phiStep)
	{
		...
		if (linesColor != 0)
		{
			rsLine( a.x, a.y, b.x, b.y, linesColor, linesColor );
		}

		if (fillColor != 0)
		{
			//rsPoly( a.x, a.y, b.x, b.y, x, y, fillColor );
		}	
	}
...
void DrawAnchor( const Vector3D& pos, float side, DWORD color, bool bBothSides )
{
    DrawAnchor( pos, side, color, color, color, bBothSides );
}

void DrawAnchor( const Vector3D& pos, float side, DWORD cx, DWORD cy, DWORD cz, bool bBothSides )
{
    Vector3D bX = pos;
    Vector3D bY = pos;
    Vector3D bZ = pos;
...
void CreateArrow( BaseMesh& bm, const Vector3D& start, const Vector3D& dir, DWORD color, float len )
{
} // CreateArrow

void DrawArrow( const Vector3D& start, const Vector3D& dir, DWORD color, 
                float len, float head, float headR )
{
...
void DrawFaces( BaseMesh& bm, bool bDrawNumbers )
{
    ...
    for (int i = 0; i < nF; i++)
    {
        ...
        cam->WorldToScreenSpace( c );
        rsRect( Rct( c.x, c.y, 2.0f ), 0.0f, 0xFFFFFF00 );
        if (bDrawNumbers)
        {
            DrawText( c.x + 3, c.y + 3, 0xFFFFFFFF, "%d", i );
        }
    }
} // DrawFaces
...
void DrawVertices( BaseMesh& bm, bool bDrawNumbers, bool bDrawHandles )
{
    ...
    for (int i = 0; i < nV; i++)
    {
        Vector3D v = vit.pos( i );
        cam->WorldToScreenSpace( v );
