...
const int	c_FrameInstanceHashPoolSize		= 16384;
const int	c_GPHashTableSize				= 7919;
const int   c_MaxSpriteSurfaces             = 1024;//2880;
...
const int	c_MaxFrameInstancesPerSurface	= 2048;
const int   c_MaxVertDrawn                  = 32768;
const int   c_MaxIndDrawn                   = c_MaxVertDrawn*6;
const int   c_MaxSpritesDrawn               = 16384;
...
public:
	...
	DWORD				m_SurfaceID;	//	index of the texture surface
}; // class FrameChunk
...
{
    int                 m_SurfaceID;        //  texture surface ID
    ...
    Matrix4D            m_TM;               //  sprite transform
    SpriteType          m_Type;             //  sprite render type

    VertexFormat        m_VF;     

    inline bool operator <( const SpriteRenderBit& rt ) const
    {
        return m_SurfaceID < rt.m_SurfaceID;    
    }
}; // struct SpriteRenderBit
...
typedef PointerHash< FrameInstance, FrameInstance::Key, c_FrameInstanceHashTableSize,  c_FrameInstanceHashPoolSize>	 GPFrameHash;
...
/*  Desc:	Sprite manager
/*****************************************************************/
class SpriteManager : public Node, public ISpriteManager, public IDeviceClient
{
public:
					SpriteManager		    ();
	...
	virtual	bool 	DrawSprite			    ( int gpID, int sprID, const Vector3D& pos, bool mirrored, DWORD color = 0 );
	virtual	bool 	DrawWSprite			    ( int gpID, int sprID, const Vector3D& pos, bool mirrored, DWORD color = 0 );
	...
	virtual int			GetNFrames			( int gpID ) const;
	...
	virtual	void 		FlushBatches		( bool bResetWorldTM = true );
	...
	virtual	float		GetAvgAlpha			( int gpID, int sprID, const Triangle2D& tri );
	virtual	BYTE		GetMaxAlpha			( int gpID, int sprID, const Triangle2D& tri );
    ...
    virtual void        SetManagedCache     ( bool bManaged = true ) { m_bManagedSurfaces = bManaged; }
	...
	FrameInstance*		AllocFrameInstance( int nExtraBytes ) 
						{ return m_InstanceAllocator.NewInstance( nExtraBytes ); }
	DWORD				AddFrameInstance( FrameInstance* pInst ) 
						{ return m_FrameReg.add( pInst->GetKey(), pInst ); }
	int					GetSurfaceTexID( int surfID ) 
						{ return m_Surface[surfID].m_TexID; }
	void				AddSurfaceClient( int surfID, FrameInstance* pInst ) 
						{ m_Surface[surfID].AddFrameInstance( pInst ); }
	DWORD				GetDropStamp( int surfID ) const 
						{ return m_Surface[surfID].m_DropStamp; }

    BYTE*               LockSurfRect( int surfID, const Rct& rect, int& pitch );
    void                UnlockSurfRect();

	NODE(SpriteManager,Node,GPSY);
...
extern sg::SpriteManager g_SpriteManager;
