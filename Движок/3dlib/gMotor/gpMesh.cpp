...
void Primitive::GetAABB( AABoundBox& aabb ) const
{
...
{
	int nStride = Vertex::GetNormalStride( getVertexFormat() );
	if (nStride < 0) return false;
	...
	{
		const Vector3D& v1 = vit( ibuf[i    ] );
		const Vector3D& v2 = vit( ibuf[i + 1] );
		const Vector3D& v3 = vit( ibuf[i + 2] );
		
		Plane pl; pl.from3Points( v1, v2, v3 );
		const Vector3D& faceNormal = pl.normal();
			
		Vector3D& n1 = vit.n( ibuf[i    ] );
		Vector3D& n2 = vit.n( ibuf[i + 1] );
		Vector3D& n3 = vit.n( ibuf[i + 2] );
...
void Primitive::GetTriangle( int triIdx, Vector3D& v0, Vector3D& v1, Vector3D& v2 ) const
{
	...
	v0 = it( idx[triIdx*3 + 0] );
	v1 = it( idx[triIdx*3 + 1] );
	v2 = it( idx[triIdx*3 + 2] );
...
void Primitive::createGrid( float x, float y, float w, float h, int cellsX, int cellsY )
{
	int numV = (cellsX + cellsY) * 2;
	int numI = numV + 4;
	create( numV, numI, vfTnL, ptLineList ); 
	VertexTnL* vert = (VertexTnL*)vbuf;
	float dv1, dv2;
...
void CreateEdgeTearMesh( const BaseMesh& pri, BaseMesh& edgeTear, DWORD clr )
{
    ...
    int nV = nEdges*2;
    edgeTear.create( nV, 0, vf2Tex, ptLineList );
