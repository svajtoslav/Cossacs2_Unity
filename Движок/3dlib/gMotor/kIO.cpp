...
BYTE* LoadFile( const char* path, DWORD& bufLen )
{
	...
	CloseHandle( hFile );
    //  in case we going to process it as text information
    if (pBuf) pBuf[bufLen] = 0;
	return pBuf;
} // LoadFile
...
/*****************************************************************************/
bool MemInStream::IsEndOfStream() const
{
	return (m_BufPos == MemOutStream::s_BufPos);
}

DWORD MemInStream::OnRead( void* buf, DWORD nBytes )
{
	if (nBytes > MemOutStream::s_BufPos - m_BufPos) nBytes = MemOutStream::s_BufPos - m_BufPos;
	memcpy( buf, &MemOutStream::s_pBuf[m_BufPos], nBytes );
	m_BufPos += nBytes;
	return nBytes;
}
