...
void PictureInstance::CreateChunks()
{
	...
	ClearChunks();
	TextureDescr td;
	td.setValues( c_ChunkSide, c_ChunkSide, c_DrawFormat, mpManaged, 1, tuProcedural );
	...
	while (cy < m_Height)
	{
		...
		while (cx < m_Width)
		{
			...
			chunk.m_TexID		= IRS->CreateTexture( texName, td );
...
{
	m_Primitive.create	( 4, 0, vfTnL, ptTriangleList );
