...
sg::SpriteManager	g_SpriteManager;
ISpriteManager* GetSpriteManager()
{
	return &g_SpriteManager;
}
ISpriteManager* ISM = GetSpriteManager();
...
void SpriteManager::Init()
{
    ...
    Log.Info( "Creating sprite cache. TexMem: %dMb, NSurf: %d", tmMB, nSurf );

    MemoryPool memPool = m_bManagedSurfaces ? mpManaged : mpVRAM;
	...
	for (int i = 0; i < nSurf; i++)
	{
        ...
        TextureDescr td;
        td.setValues( c_GPTexSide, c_GPTexSide, cfARGB4444, memPool, 1, tuProcedural );
        int texID = IRS->CreateTexture( texName, td );
    ...
    if (!m_bManagedSurfaces)
    {
        TextureDescr ctd;
        ctd.setValues( c_GPTexSide, c_GPTexSide, cfARGB4444, mpSysMem, 1, tuProcedural );
        m_CanvasTex = IRS->CreateTexture( "SpriteCanvas", ctd );
    }
	...
	IRS->AddOnDestroyNotified( this );
	m_bInited = true;

    static int shSprite2D = IRS->GetShaderID( "hud" );
    m_Prim.create			( c_MaxVertDrawn, c_MaxIndDrawn, vfTnL, ptTriangleList );
    m_Prim.setPriType		( ptTriangleList			);
    m_Prim.setShader		( shSprite2D				);
    m_Prim.setVertexFormat	( vfTnL	);
    m_Prim.setNPri			( 0 );
    m_Prim.setNInd			( 0 );
    m_Prim.setNVert		    ( 0 );
}  // SpriteManager::Init
...
const char*	SpriteManager::GetPackagePath( int gpID )
{
	...
	//  default path
	{
        static char root[_MAX_PATH];		
        static FilePath path;
        path.SetPath( m_PackageReg[gpID].m_Name );

        path.SetExt( "g2D" );
        if (LocateFile( path.GetFileWithExt(), root )) 
        { path.SetDir( root ); return path.GetFullPath(); }
...
DWORD SpriteManager::GetAlpha( int gpID, int sprID, DWORD color, int ptX, int ptY, bool precise )
{
	...
	if (frameInst == NULL || !frameInst->GetBounds().PtIn( ptX, ptY )) return c_OutsideChunks;
...
{
	FlushBatches();
    /*float sec = m_Timer.seconds();
    if (sec >= c_PurgeTimeout)
    {
        Purge();
        m_Timer.start();
    }*/
    ...
    for (int i = 0; i < nB; i++)
    {
        ...
        for (int j = 0; j < rb.m_NChunks; j++)
        {
                ...
                cV + nV >= c_MaxVertDrawn)
            {
                ...
                m_Prim.setTexture( m_Surface[cSurf].m_TexID );
                IRS->Draw( m_Prim );

                cSurf   = rb.m_SurfaceID;
                cVF     = rb.m_VF;
                cI      = 0;
            ...
            if (rb.m_VF == vfTnL)
            {
                ...
                {
                    Vector3D pt( v[i].x, v[i].y, v[i].z );
                    pt *= rb.m_TM;
                    vTnL[cV].x = pt.x - c_HalfPixel;
                    vTnL[cV].y = pt.y - c_HalfPixel;
                    vTnL[cV].z = pt.z;
                    vTnL[cV].w = 1.0f;
                    vTnL[cV].u = v[i].u;
                    vTnL[cV].v = v[i].v;
                    vTnL[cV].diffuse = rb.m_DiffuseColor;
                }
            }
            else
            {
                for (int i = 0; i < nV; i++, cV++)
                {
                    Vector3D pt( v[i].x, v[i].y, v[i].z );
                    pt *= rb.m_TM;
                    v2t[cV].x = pt.x - c_HalfPixel;
                    v2t[cV].y = pt.y - c_HalfPixel;
                    v2t[cV].z = pt.z;
                    v2t[cV].u = v[i].u;
                    v2t[cV].v = v[i].v;
                    v2t[cV].diffuse = rb.m_DiffuseColor;
                }
            }
        }
    }
    ...
    for (int i = 0; i < nB; i++)
    {
        ...
        for (int j = 0; j < rb.m_NChunks; j++)
        {
            ...
            if (rb.m_VF == vf2Tex)
            {
                ...
                __declspec(align(16)) static const DWORD CurDifMask2t[4] = {0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0x00000000};
                CurDiffuse2t[3] = rb.m_DiffuseColor;
                __asm{
                ...
                _man_loop_1:
                    ...
                    movntps		XMMWORD PTR [edi]Vertex2t.x,xmm4
                    movntps		XMMWORD PTR [edi]Vertex2t.u,xmm7
                    
                    add			esi,    TYPE FrameVert
                    add			edi,    TYPE Vertex2t;
                    dec			ecx
                    jnz			_man_loop_1
            ...
            {
                void*       dV    = &vTnL[cV];
                __declspec(align(16)) static DWORD       CurDiffuseTnL[4] = {0x00000000,0xFFFFFFFF,0x00000000,0x00000000};
                __declspec(align(16)) static const DWORD CurDifMaskTnL[4] = {0x00000000,0x00000000,0xFFFFFFFF,0xFFFFFFFF};
...
void SpriteManager::FlushBatches( bool bResetWorldTM )
{
    int nB = m_RenderBits.size();
    if (nB == 0) return;

    m_Prim.setShader( m_Shader );
    if (bResetWorldTM) IRS->ResetWorldMatrix();
    //  sort render bits by sprite surface
    std::sort( m_SortedRenderBits.begin(), m_SortedRenderBits.end(), RenderBitCmp() );
    
    if (m_bUseSSE) sse_DrawBatches(); else DrawBatches();
    
    m_RenderBits.clear();
    m_SortedRenderBits.clear();
} // SpriteManager::FlushBatches

struct CountAlpha
{
	static SpriteManager*	pGP;
	static DWORD		alpha;
	static DWORD		nPix;
	static int			gpID;
	static int			sprID;
	...
	{
		nPix++;
		alpha += pGP->GetAlpha( gpID, sprID, x, y, true );
		return true;
	}
	...
	{
		nPix++;
		DWORD cAlpha = pGP->GetAlpha( gpID, sprID, x, y, true );
		if (cAlpha > alpha) alpha = cAlpha;
		return true;
	}
}; // struct CountAlpha

SpriteManager*	CountAlpha::pGP		= NULL;
DWORD		CountAlpha::alpha	= 0;
DWORD		CountAlpha::nPix	= 0;
int			CountAlpha::gpID	= 0;
int			CountAlpha::sprID	= 0;

BYTE SpriteManager::GetMaxAlpha( int gpID, int sprID, const Triangle2D& tri )
{
	CountAlpha::pGP		= this;
	CountAlpha::alpha	= 0;
	CountAlpha::nPix	= 0;
	CountAlpha::gpID	= gpID;
	CountAlpha::sprID = sprID;

	tri.Rasterize( 1.0f, CountAlpha::MaxAlpha );
	return CountAlpha::alpha;
} // SpriteManager::GetMaxAlpha 

float SpriteManager::GetAvgAlpha( int gpID, int sprID, const Triangle2D& tri )
{	
	CountAlpha::pGP		= this;
	CountAlpha::alpha	= 0;
	CountAlpha::nPix	= 0;
...
bool SpriteManager::DrawSprite( int gpID, int sprID, const Matrix4D& transf, DWORD color )
{
    ...
    if (m_RenderBits.size() + nChunks >= c_MaxSpritesDrawn) FlushBatches();
    ...
    for (int i = 0; i < nChunks; i++)
    {
        ...
        if (chunk.m_SurfaceID != curSurf)
        {
            ...
            rb->m_SurfaceID     = chunk.m_SurfaceID;
            rb->m_TM            = transf;
            rb->m_VF            = vfTnL;
                                ...
                                DWORD color )
{
    ...
    if (chunk.m_NVert == 4)  //  HACK
    {
        chunk.m_NVert = nVert;
        chunk.m_NTri  = nPoly;
        Vertex2t* pV  = ((Vertex2t*)geom.getVertexData()) + cVert;
...
int SpriteManager::GetFrameHeight( int gpID, int sprID )
{
	...
	if (sprID < 0 || sprID >= pPackage->GetNFrames()) return 0.0f;
...
void SpriteManager::OnDestroyRenderSystem()
{
    Purge();
...
BYTE* SpriteManager::LockSurfRect( int surfID, const Rct& rect, int& pitch )
{
    ...
    m_LockSurfID    = surfID;
    m_LockRect      = rect;
    return IRS->LockTexBits( m_bManagedSurfaces ? texID : m_CanvasTex, rect, pitch );
