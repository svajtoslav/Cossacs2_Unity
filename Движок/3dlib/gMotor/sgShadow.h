...
public:
	...
	NODE(ShadowMapper,Node,SHMP);

protected:

	std::vector<ShadowCaster>	m_Casters;		//  array of casters
	Light*						m_pLightSource;	//  light source casting shadows	
}; // class ShadowMapper
...
public:
	...
	NODE(BlobShadowMapper,ShadowMapper,BLSH);

protected:
	Texture*			m_pBlobTexture;
	DeviceStateSet*		m_pBlobShadowShader;
	ZBias*				m_pDecalZBias;
...
public:
	...
	NODE(DBlobShadowMapper, BlobShadowMapper, DBSH);
}; // class DBlobShadowMapper
...
class ProjectiveShadowMapper : public ShadowMapper
{
	...
	DeviceStateSet*				m_pCasterShader;
	DeviceStateSet*				m_pReceiverShader;
	TextureMatrix*				m_pTextureMatrix;
...
public:
	...
	NODE(ProjectiveShadowMapper,ShadowMapper,PRSH);
}; // class ProjectiveShadowMapper
...
{
	DeviceStateSet*		m_pPass1Shader;
	DeviceStateSet*		m_pPass2Shader;
	DeviceStateSet*		m_pQuadShader;
...
public:
	...
	NODE(ShadowVolumeMapper,ShadowMapper,SVSH);
}; // class ShadowVolumeMapper
...
public:
	...
	NODE(IDShadowMapper,ShadowMapper,IDSH);
}; // class IDShadowMapper
...
public:
	...
	NODE(ShadowBlob,MorphedGeometry,SHBL);
...
/*	Desc:	Distorted blob 
/*****************************************************************************/
class RubberBlob : public ShadowBlob
{
public:
							RubberBlob			();
	virtual void			Expose				( PropertyMap& pm );
	...
	NODE(RubberBlob,ShadowBlob,RUBL);
...
public:
	...
	NODE(ProjectiveBlob,ShadowBlob,PRBL);
...
protected:
	...
	RenderTarget*				m_pTarget;
	Texture*					m_pTexture;
	OrthoCamera*				m_pLightCamera;
	DeviceStateSet*				m_pShadowMapDSS;
	DeviceStateSet*				m_pReceiverDSS;	
}; // class ProjectiveBlob
