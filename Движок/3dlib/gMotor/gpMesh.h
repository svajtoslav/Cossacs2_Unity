...
void ConvertVF( BaseMesh& bm )
{
	...
	for (int i = 0; i < nV; i++)
	{
		...
		vd.SetPos		( vs.GetPos()				);
		vd.SetNormal	( vs.GetNormal()			);
		vd.SetUV		( vs.GetU(), vs.GetV()		);
		vd.SetUV2		( vs.GetU2(), vs.GetV2()	);
		vd.SetDiffuse	( vs.GetDiffuse()			);
		vd.SetSpecular	( vs.GetSpecular()			);
		for (int j = 0; j < vs.GetNBlendI(); j++)
		{
			vd.SetBlendI( j, vs.GetBlendI( j ) );
		}
		for (int j = 0; j < vs.GetNBlendW(); j++)
		{
			vd.SetBlendW( j, vs.GetBlendW( j ) );
		}
		vd.SetW( 1.0f );
	}
