...
{ 
	...
	strcpy( path, val );
	Application::instance()->ToRelativePath( path, _MAX_PATH );
...
{	
	...
	pm.p ( "TexName", GetTextureName, SetTextureName, "file|Textures" );
...
void FramerateCursor::Activate()
{
	...
	const TextureDescr* td = IRS->GetTextureDescr( m_TexID );
	m_UV.x = m_TexRct.x / td->getSideX();
	m_UV.y = m_TexRct.y / td->getSideX();

	m_UV.w = m_TexRct.w / td->getSideX();
	m_UV.h = m_TexRct.h / td->getSideX();
