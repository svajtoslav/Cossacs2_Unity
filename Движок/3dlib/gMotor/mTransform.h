...
public:
	...
	_inl Vector3D	getTranslation	() const;	
	_inl void		scaling			( float sx, float sy, float sz );
	...
	_inl bool		equal			( const Matrix4D& m );
	...
	void			Random		( float minV = 0.0f, float maxV = 1.0f );
	void			FactorizeSRT( Vector3D& scale, Matrix3D& rot, Vector3D& trans ) const;
	...
	void			TransformationFromPlaneToWorld(
...
public:
	...
	_inl void				Push( const Matrix4D& m );
