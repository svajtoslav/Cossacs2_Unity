...
void LensFlareElement::Render()
{
	...
	{
		s_QuadMesh.create( 4, 0, vfTnL, ptTriangleList );
...
void LensFlare::Render()
{
	...
	//if (!m_pScene)
	//{
	//	m_pScene = sg::Root::instance()->FindChild<Group>( "Scene" );
	//	DeviceStateSet* pDSS = GetChild<DeviceStateSet>( "sunmap" );
	//	pDSS->RemoveChildren();
	...
	pCam->WorldToScreenSpace( vc );
	pCam->WorldToScreenSpace( vf );
...
void LensFlare::Create( bool bOccluded )
{
	if (!bOccluded)
	{
		AddChild<DeviceStateSet>( "LensFlare" );
		return;
	}
	...
	//  sunmap texture
	sg::Texture* pSunMap = pRT->AddChild<Texture>( "SunMap" );
	pSunMap->SetUsage		( tuRenderTarget		 );
	pSunMap->SetMemoryPool	( mpVRAM				 );
	...
	//  sunmap viewport 
	Canvas* pVP = pRT->AddChild<Canvas>( "SunMapVP" );
	...
	sg::Texture* pIntMap = pIRT->AddChild<Texture>( "IntMap" );
	pIntMap->SetUsage		( tuRenderTarget		 );
	pIntMap->SetMemoryPool	( mpVRAM				 );
	...
	pIRT->AddChild<DeviceStateSet>( "FlareIntensityMap" );

	AddInput( pIntMap );

	//  intensity map viewport 
	Canvas* pIVP = pIRT->AddChild<Canvas>( "IntMapVP" );
	...
	pGeom->Create( c_SunMapSide * c_SunMapSide * 4, 0, vfTnL );
	...
	//DeviceStateSet* pDSS = pRT->AddChild<DeviceStateSet>( "sunmap" );
	//
	//m_pScene = sg::Root::instance()->FindChild<Group>( "Scene" );
	//pDSS->AddInput( m_pScene );

	//AddChild<DeviceStateSet>( "OccludedLensFlare" );
} // LensFlare::Create
