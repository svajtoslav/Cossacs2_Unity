...
/*****************************************************************************/
void Light::Unserialize( InStream& is )
{
	Parent::Unserialize( is );
	is >> m_Ambient >> m_Diffuse >> m_Specular;
} // Light::Unserialize

void Light::Serialize( OutStream& os ) const
{
	Parent::Serialize( os );
	os << m_Ambient << m_Diffuse << m_Specular;
} // Light::Serialize

void Light::Expose( PropertyMap& pm )
{
	pm.start<Parent>( "Light", this );
...
Frustum	Light::GetFrustum( const AABoundBox& aabb ) const
{
	Frustum res;
...
{
	Parent::Render();
	IRS->SetDirectionalLight( this, m_Index );
