	...
	if(x)
	{
		if(!strcmp(x->GetQuoteName(),"A"))
		{
			...
			if(t)
			{
				...
				RegisterUnitType(&UnitType,ds.str);
	...
	for(int i=0;i<n;i++)
	{
		...
		if(B)
		{
			if((OB->NMask&B->NMask)&&B->Life==B->MaxLife)
			{
				...
				{
					if(CT->ComeIn[j].BT) 
					if(!strcmp(CityPopulation::CBild[i]->BT->Name.str,CT->ComeIn[j].BT->Name.str))
					{
...
int CityPopulation::GetCityzenTypeIndexByName(const char* name)
{
	...
	{
		if(CType[i])
			if(!strcmp(name,CType[i]->Name.str))
				return i;
	}
	return -1;
}
int CityPopulation::GetBildTypeIndexByName(const char* name)
{
	...
	{
		if(BType[i])
			if(!strcmp(name,BType[i]->Name.str))
				return i;
	}
	return -1;
}
