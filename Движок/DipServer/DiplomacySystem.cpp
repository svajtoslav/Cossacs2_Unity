...
void DiplomacySystem::Process(){
	...
	*/
				...
				if(prp==0&&t>120||t>5156){
					...
					if(xc>=0){
						...
						if(top>=0&&top<GetNZones()){
							...
							int maxdang;
							//if(Group->Index==168) SelectUnits(Group,0);
							//if(!find){
								/*
								if(Grp->Difficulty>1){
							...
							//}else{
								//zf=FindNextZoneOnTheSafeWay(top,Can->Top,Dang,&maxdang,100);
							//}
							//
							maiSetWayArray();
							//							
									...
									if(prp==0){
										...
										if(prp==0){
											if(DP){
												...
												SGP_MoveToPoint(Owner,Group,x,y,512,0,0,32+1);
											}
										}
									}*/
								}else{
									...
									if(!find){
										if(zf!=top&&r>3){
											...
											/*if(r<30){
												...
												int NSkip=2;
												if(DP) NSkip=4;
												if(NWayPoint>NSkip) GetTopZRealCoor(Way[NWayPoint-NSkip-1],&x,&y);
												//if(NSkip<NWayPoint) GetTopZRealCoor(Way[NSkip],&x,&y);
												else{
													//GetTopZRealCoor(zf,&x,&y);
													GetTopZRealCoor(Way[0],&x,&y);
													//GetTopZRealCoor(Way[NWayPoint-1],&x,&y);
												}
	...
	byte NI=GetRND(8);
	CreateDangerMapForHealer(NI,hDang[NI],GetNZones(),4);
	}
	if(GetRND(4)==3){
