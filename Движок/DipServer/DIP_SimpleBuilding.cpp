...
int DIP_SimpleBuilding::GetSaveData(byte* buf,bool write){
	...
	DString str;
	xml.GetXMLSource(&str);
	s=strlen(str.str)+1;
	if(write)memcpy(buf+sz,str.str,s);
	sz+=s;
	/*
	int n=SetlUpg.GetAmount();
...
void DIP_SimpleBuilding::Load(char* GroupName, SettlementInfo* si){	
	if(RegisterUnits(&CentralGroup,GroupName)){		
		...
		if(!si->SecondGroupName.isClear()){
			if(RegisterUnits(&SecondGroup,si->SecondGroupName.str)){
		...
		GAMEOBJ type;
		if(RegisterUnitType(&type,si->CopID.str)){
		...
		if(RegisterUnitType(&type,si->PeonID.str)){
		...
		if(RegisterUnitType(&type,si->ObozID.str)){
