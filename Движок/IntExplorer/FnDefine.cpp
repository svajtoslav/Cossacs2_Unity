...
bool ADI_Stbl(sicExplorer* SXP,DialogsSystem* DSS,int* x,int* y,int* x1,int* y1,
	...
	if(!OSW->PANELS[sty].panel_gp[0])return true;
	int GPF=SXP->GetGPPictureIndex(OSW->PANELS[sty].panel_gp);
	int GPF1=SXP->GetGPPictureIndex(OSW->PANELS[10].panel_gp);
	if(GPF==-1)return true;
	int PX[8];
	memcpy(PX,&OSW->PANELS[sty].LU,8*4);
	int PX1[8];
	...
	for(int i=0;i<nclm;i++){
		...
		if(GPF1!=-1 && OSW->CTBL_VIndex!=-1 && i<nclm-1)
			DSS->addCustomBorder(bx,cury-OSW->CTBL_LineDY-OSW->CTBL_LineDLY,bx,cury+TLY+OSW->CTBL_LineDY+OSW->CTBL_LineDLY,GPF1,PX2,PX2,OSW->PANELS[10].C,OSW->PANELS[10].C1);
	};
	int cpr=2+nclm*2;
	int thisLy=0;
	if(GPF1!=-1 && OSW->CTBL_HIndex!=-1 && cpr<NParam){
		thisLy=atoi(Param[cpr]);
		DSS->addCustomBorder(curx-OSW->CTBL_LineDX-OSW->CTBL_LineDLX,cury+thisLy+OSW->CTBL_LineDY-1,curx+TLX+OSW->CTBL_LineDX+OSW->CTBL_LineDLX,cury+thisLy+OSW->CTBL_LineDY-1,GPF1,PX1,PX1,OSW->PANELS[9].C,OSW->PANELS[9].C1);
	}
	bool doit=1;
	TLY=0;
	do{
