...
CIMPORT void SetChatWState(int ID,int State);//new 7.02
...
int CReadX(char** sx,int x0,int x1,OneBox* BOX){
	int xx=0;
	int xx2=0;
	int com=0;
	char* s=*sx;
	while(s[0]!=','&&s[0]!=')'&&s[0]!=']'&&s[0]){
		if(s[0]=='%'){
			...
			while((c=s[p1])!=0&&c!=','&&c!=']'&&c!=')'&&c!='+'&&c!='-'&&c!='['&&c!='('&&c!='<'&&c!='>'&&p1<14)p1++; // new 24.01
			if(p1<14&&BOX){
				char name[16];
				memcpy(name,s,p1);
				name[p1]=0;
				for(int i=0;i<BOX->NISPOS;i++)if(!strcmp(BOX->ISPOS[i].ID,name))xx=BOX->ISPOS[i].x1;
				if((com==-2 && xx2<xx) || (com==2 && xx2>xx))xx=xx2; // new 24.01
			};
			s+=p1-1;
		}else
		if(s[0]=='R')xx=x1;
...
void ReadCoordinates(char** src,int& x,int& y,int& xR,int& yR,OneBox* BOX){
	...
	do{
		...
		if(c=='x'&&(*src)[1]==':'){
			(*src)+=2;
			x0=CReadX(src,x,xR,BOX);
			vx0=1;
		}else
		if(c=='x'&&(*src)[1]=='1'&&(*src)[2]==':'){
			(*src)+=3;
			x1=CReadX(src,x,xR,BOX);
			vx1=1;
		}else
		if(c=='x'&&(*src)[1]=='c'&&(*src)[2]==':'){
			(*src)+=3;
			xc=CReadX(src,x,xR,BOX);
			vxc=1;
		}else
		if(c=='y'&&(*src)[1]==':'){
			(*src)+=2;
		...
		if(c=='#'){
			...
			if(B1||B2||B3){
				...
				if(Result[pos]=='%'){
					...
					if(NBoxes>=MaxBoxes){
						MaxBoxes++;
						Boxes=(OneBox**)realloc(Boxes,MaxBoxes*4);
						Boxes[MaxBoxes-1]=new OneBox;
					};
					int X0=EXP->x;
					int Y0=EXP->y;
					int X1=EXP->x1;
					...
					/// end new /////////
					char* src=Result+pos;
					ReadCoordinates(&src,X0,Y0,X1,Y1,CURBOX);
			...
			if(!strcmp(com,"#def_panel_vars")){
				pos++;
				int p1=pos;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>60)return pos;
				memcpy(panel_selected_var,Result+pos,p1-pos);
				panel_selected_var[p1-pos]=0;
				p1++;
				pos=p1;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>60)return pos;
				memcpy(panel_link_id,Result+pos,p1-pos);
				panel_link_id[p1-pos]=0;
				p1++;
				pos=p1;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>15)return pos;
				memcpy(panel_group_name,Result+pos,p1-pos);
				panel_group_name[p1-pos]=0;
				pos=p1+1;
			}else // end new
			if(!strcmp(com,"#def_sbtn")){
				...
				pos+=L+1;
				sstb_FontDx=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				sstb_FontDy=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
			}else
			...
			if(!strcmp(com,"#def_combo2")){//#def_combo2 gp_file idx font_dx font_dy first_line_dx first_line_dy center
				if(Result[pos]!='(')return pos;
				pos++;
				int p1=pos;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				...
				cmb_bdx=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				cmb_bdy=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				cmb_odx=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				cmb_ody=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				cmb_center=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
			}else // new 15.02
			if(!strcmp(com,"#def_combo_fix")){
				if(Result[pos]!='(')return pos;
				pos++;
				int p1=pos;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>60)return pos;
				memcpy(combo1_var,Result+pos,p1-pos);
				combo1_var[p1-pos]=0;
				p1++;
				pos=p1;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>60)return pos;
				memcpy(combo2_var,Result+pos,p1-pos);
				combo2_var[p1-pos]=0;
				pos=p1+1;
				int L=0;
				int i=0;
				while(Result[pos]!=')' && i<10){
					combo1_fix_line[i]=ReadNumber(Result+pos,&L,0);
					pos+=L+1;
					i++;
				}
			}else // end new
			...
			if(!strcmp(com,"#def_tbl_top")){
				if(Result[pos]!='(')return pos;
				pos++;
				int p1=pos;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>120)return pos;
				memcpy(CTBL_top_gp_file,Result+pos,p1-pos);
				CTBL_top_gp_file[p1-pos]=0;
				pos=p1+1;
				int L=0;
				CTBL_TopLeftID=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				CTBL_TopCenterID=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				CTBL_TopRightID=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				CTBL_top_panel_dx=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				CTBL_top_panel_dy=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				CTBL_top_panel_lx=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
			}else
			if(!strcmp(com,"#def_tbl_line")){
				if(Result[pos]!='(')return pos;
				pos++;
				int L=0;
				CTBL_HIndex=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				CTBL_VIndex=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				CTBL_LineDX=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				CTBL_LineDY=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				CTBL_LineDLX=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				CTBL_LineDLY=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
			}else
			if(!strcmp(com,"#def_dtbl_fon")){
				if(Result[pos]!='(')return pos;
				pos++;
				int L=0;
				int FIDX=ReadNumber(Result+pos,&L,0);
				if(FIDX>9)FIDX=9;
				pos+=L+1;
				int p1=pos;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>120)return pos;
				memcpy(FONS[FIDX].fon_gp,Result+pos,p1-pos);
				FONS[FIDX].fon_gp[p1-pos]=0;
				pos=p1+1;
				FONS[FIDX].FonID=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				FONS[FIDX].FonDX=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				FONS[FIDX].FonDY=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
			}else
			if(!strcmp(com,"#def_dtbl_top")){
				if(Result[pos]!='(')return pos;
				pos++;
				int p1=pos;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>120)return pos;
				memcpy(TBL_top_gp_file,Result+pos,p1-pos);
				TBL_top_gp_file[p1-pos]=0;
				pos=p1+1;
				int L=0;
				TBL_TopLeftID=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_TopRightID=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_TopCenterID=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_TopMiddleLID=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_TopMiddleRID=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_top_dx=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_top_dy=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
			}else
			if(!strcmp(com,"#def_dtbl_top_text")){
				if(Result[pos]!='(')return pos;
				pos++;
				int p1=pos;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>15)return pos;
				memcpy(TBL_TopAFont,Result+pos,p1-pos);
				TBL_TopAFont[p1-pos]=0;
				p1++;
				pos=p1;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>15)return pos;
				memcpy(TBL_TopPFont,Result+pos,p1-pos);
				TBL_TopPFont[p1-pos]=0;
				pos=p1+1;
				int L=0;
				TBL_top_text_dx=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_top_text_dy=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
			}else
			if(!strcmp(com,"#def_dtbl_message")){
				if(Result[pos]!='(')return pos;
				pos++;
				int p1=pos;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>15)return pos;
				memcpy(TBL_MessageAFont,Result+pos,p1-pos);
				TBL_MessageAFont[p1-pos]=0;
				p1++;
				pos=p1;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>15)return pos;
				memcpy(TBL_MessagePFont,Result+pos,p1-pos);
				TBL_MessagePFont[p1-pos]=0;
				pos=p1+1;
				int L=0;
				TBL_message_align=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_message_valign=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_message_dx=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_message_dy=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_message_lx=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
			}else
			if(!strcmp(com,"#def_dtbl_button")){
				if(Result[pos]!='(')return pos;
				pos++;
				int p1=pos;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>120)return pos;
				memcpy(TBL_button_gp_file,Result+pos,p1-pos);
				TBL_button_gp_file[p1-pos]=0;
				pos=p1+1;
				int L=0;
				TBL_gpb_picture=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_gpb_active=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_gpb_passive=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_bottom_dx=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_bottom_dy=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_gpb_dx=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_gpb_dy=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
			}else
			if(!strcmp(com,"#def_dtbl_button_text")){
				if(Result[pos]!='(')return pos;
				pos++;
				int p1=pos;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>15)return pos;
				memcpy(TBL_ButtonAFont,Result+pos,p1-pos);
				TBL_ButtonAFont[p1-pos]=0;
				p1++;
				pos=p1;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>15)return pos;
				memcpy(TBL_ButtonPFont,Result+pos,p1-pos);
				TBL_ButtonPFont[p1-pos]=0;
				pos=p1+1;
				int L=0;
				TBL_gpb_text_dx=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_gpb_text_dy=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_gpb_text_lx=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_gpb_defalt_hint=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
			}else 
			if(!strcmp(com,"#def_dtbl_button_hotkey")){
				if(Result[pos]!='(')return pos;
				pos++;
				int L=0;
				TBL_gpb_hotkey[0]=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_gpb_hotkey[1]=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				TBL_gpb_hotkey[2]=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
			}else 
			if(!strcmp(com,"#def_dtbl_button_hint")){
				if(Result[pos]!='(')return pos;
				int Index=0;
				while((c=Result[pos])!=')' && c && Index<3){
					pos++;
					int p1=pos;
					while((c=Result[p1])!=','&&c!=')'&&c)p1++;
					if(p1-pos>120)return pos;
					memcpy(HINT[Index].hint,Result+pos,p1-pos);
					HINT[Index].hint[p1-pos]=0;
					pos=p1;
					Index++;
				}
				pos++;
			}else
			if(!strcmp(com,"#def_mtext")){
				if(Result[pos]!='(')return pos;
				pos++;
				int L=0;
				mtext_dx=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				mtext_dy=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				mtext_align=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
			}else
			if(!strcmp(com,"#def_sbox_spos")){
				if(Result[pos]!='(')return pos;
				pos++;
				int p1=pos;
				while((c=Result[p1])!=','&&c!=')'&&c)p1++;
				if(p1-pos>15)return pos;
				char sb_name[16];
				memcpy(sb_name,Result+pos,p1-pos);
				sb_name[p1-pos]=0;
				pos=p1+1;
				int L=0;
				int sb_spos=ReadNumber(Result+pos,&L,0);
				pos+=L+1;
				OneBox* CURBOX=NULL;
				for(int j=0;j<NBoxes;j++)if(!strcmp(Boxes[j]->Name,sb_name)){
					if(Boxes[j]->VS)Boxes[j]->VS->SPos=sb_spos;
				};
			}else// end new //
			if(!strcmp(com,"#def_sbox")){
				if(Result[pos]!='(')return pos;
...
void OneSicWindow::Erase(){
	...
	if(PRC)free(PRC);
	if(VARS)free(VARS);
	for(int i=0;i<NBoxes;i++)if(Boxes[i])delete(Boxes[i]);
	NBoxes=0;
	MaxBoxes=0;
	if(Boxes)free(Boxes);
	Boxes=NULL;
...
void sicExplorer::ChangeOutput(int x,int y,int x1,int y1){
	...
	for(int j=0;j<NWindows;j++){
		...
		OW->NPRC=0;
		//OW->NVARS=0;
		for(int i=0;i<OW->NBoxes;i++)if(OW->Boxes[i])delete(OW->Boxes[i]);
		OW->NBoxes=0;
		OW->MaxBoxes=0;
	...
	};
	{
		...
		for(int j=0;j<SXPR->Windows[i]->NBoxes;j++){
			...
			if(OBX->VS){
				...
				bool mover=false;
				if(N>3 && (DSS->DSS[2]->MouseOver || DSS->DSS[3]->MouseOver) && DSS->DSS[2]->Enabled && DSS->DSS[3]->Enabled)
					mover=true; // new 18.02
				for(int q=st;q<N;q++)if(DSS->DSS[q]){
					//if(DSS->DSS[q]->MouseOver)mover=true;
					int y=p0+DSS->DSS[q]->y1;
					if(y>ymax)ymax=y;
				};
				if(ymax>OBX->yi1){
					//OBX->VS->SMaxPos=ymax-OBX->yi1+10;
					OBX->VS->SMaxPos=ymax-OBX->yi1+2;//new 31.03
					if(OBX->VS->SPos>OBX->VS->SMaxPos)OBX->VS->SPos=OBX->VS->SMaxPos;
					OBX->VS->Visible=1;
					if(mover&&GetWheelDelta(false)){
						OBX->VS->SPos-=GetWheelDelta(true)/8;
						if(OBX->VS->SPos<0)OBX->VS->SPos=0;
						if(OBX->VS->SPos>OBX->VS->SMaxPos)OBX->VS->SPos=OBX->VS->SMaxPos;
					}
				}else{
					OBX->VS->Visible=1;
			...
			OBX->DSS.IsDrawActive=false;// new 25.02
		...
		// test 24.02
		if(SXPR->Windows[i]->NBoxes){
			OneBox* OBX=SXPR->Windows[i]->Boxes[SXPR->Windows[i]->NBoxes-1];
			if(OBX->DSS.ActiveParent&&OBX->DSS.ActiveParent->OnDrawActive&&!OBX->DSS.IsDrawActive)
				OBX->DSS.ActiveParent->OnDrawActive(OBX->DSS.ActiveParent);
		}
		// end test
	};
	ProcessWindowMessages();
...
int sicExplorer::GetGPPictureIndex(char* NAME0){
	...
	char Name[256];
	strcpy(Name,NAME);
	if(!(strstr(NAME,".g17")||strstr(NAME,".G17")))strcat(Name,".g17");
	if(Name[2]!='*'){
		...
		if(F!=INVALID_HANDLE_VALUE){
			...
			char cc[256];
			strcpy(cc,Name);
			char* ss=strstr(cc,".g17");
			if(ss)ss[0]=0;
			return GPS.PreLoadGPImage(cc);
		};
		// new
		strcpy(Name,NAME);
		if(!(strstr(NAME,".g2d")||strstr(NAME,".G2D")))strcat(Name,".g2d");
		F=RReset(Name);
		if(F!=INVALID_HANDLE_VALUE){
			RClose(F);
			char cc[256];
			strcpy(cc,Name);
			char* ss=strstr(cc,".g2d");
			if(ss)ss[0]=0;
			return GPS.PreLoadGPImage(cc);
		};
		// end new
