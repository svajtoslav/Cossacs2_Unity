...
char* SaveCurrentResult(OneSicWindow* OSW,char* dest){
	if(OSW->Result){
		...
		if(F!=INVALID_HANDLE_VALUE){
			...
			//new 18.04
			for(int j=0;j<OSW->NBoxes;j++)if(OSW->Boxes[j]->VS){
				char ccc[64];
				sprintf(ccc,"#def_sbox_spos(%s,%d)",OSW->Boxes[j]->Name,OSW->Boxes[j]->VS->SPos);
...
extern tpUpload* Upload;
void exec_LW_upload(int Np,char** par,int* size){
	//if(Np<4)return;
	char ftp_name[256];
	char ftp[128];
	char log[128];
	...
	int ch=0;
	int ch1=0;
	ResFile F=RReset("Internet\\Cash\\Cookies\\%GV_VE_FTP_NAME");
	if(F!=INVALID_HANDLE_VALUE){
		int sz=RFileSize(F);
		if(sz>128)sz=128;
		RBlockRead(F,ftp_name,sz);
		ftp_name[sz]=0;
		RClose(F);
		
		if(ftp_name[0]>0){
			while(ftp_name[ch]!=':' && ftp_name[ch]>0){
				log[ch1]=ftp_name[ch];
				ch1++;
				ch++;
			}
			...
			ch1=0;
			ch++;
			while(ftp_name[ch]!='\\' && ftp_name[ch]!='/' && ftp_name[ch]>0){
				ftp[ch1]=ftp_name[ch];
				ch1++;
				ch++;
			}
			...
			ch++;
			while(ftp_name[ch]>0){
				folder[ch1]=ftp_name[ch];
				ch1++;
				ch++;
			}
			if(ch1>0 && (ftp_name[ch-1]=='\\' || ftp_name[ch-1]=='/'))
				ch1--;
			folder[ch1]=0;
		}
	}
	ResFile F1=RReset("Internet\\Cash\\Cookies\\%GV_VE_UPLOAD");
	if(F1!=INVALID_HANDLE_VALUE){
		int sz=RFileSize(F1);
		if(sz>128)sz=128;
		RBlockRead(F1,file,sz);
		file[sz]=0;
		RClose(F1);

		if(file[0]!=0){
			if(ch==0)Upload("ftp.gsc-game.kiev.ua","ftpus","34op75jh5v7u","update",file);
			else Upload(ftp,log,pass,folder,file);
		}
	}
};
/*CIMPORT bool DecompressData(unsigned char** pOutData, unsigned* pOutLen, unsigned char* InData);
void exec_LW_decompress(int Np,char** par,int* size){
	if(Np<1)return;
	char fnm[128];
	char fnm1[128];
	char fnm2[128];
	char fsz[128];
	sprintf(fnm,"Internet\\Cash\\Upload\\%s",par[0]);
	ResFile F=RReset(fnm);
	if(F!=INVALID_HANDLE_VALUE){
		int sz=RFileSize(F);
		char* sd=new char[sz];
		RBlockRead(F,sd,sz);
		...
		unsigned dLen=0;
		DecompressData(&dd,&dLen,(unsigned char*)sd);
		RClose(F);
		if(dd){
			DeleteFile(fnm);
			...
			while(dLen>128){
				...
				while(dd[0]==0)
					dd++;
				int sz1=atoi(fsz);
				if(strstr(fnm,"Updator.exe"))
					sprintf(fnm,"Upload\\Updator.exe");
				else{
					int ch=0;
					int ch1=0;
					do{
						fnm1[ch1]=fnm[ch];
						if(fnm[ch]==47 || fnm[ch]==92)ch1=-1; //  \ or /
						ch++;
						ch1++;
					}while(fnm[ch]>0);
					fnm1[ch1]=0;
					sprintf(fnm2,"Upload\\%s.path",fnm1);
					ResFile F1=RRewrite(fnm2);
					if(F1!=INVALID_HANDLE_VALUE){
						RBlockWrite(F1,fnm,strlen(fnm)+1);
						RClose(F1);
					}
					sprintf(fnm,"Upload\\%s",fnm1);
				}
				ResFile F=RRewrite(fnm);
				if(F!=INVALID_HANDLE_VALUE){
					RBlockWrite(F,dd,sz1);
					RClose(F);
				}
				dLen-=(sz1+128);
				dd+=sz1;
			...
			delete[](dd);
			WinExec("\"Upload\\Updator.exe\"",SW_SHOWDEFAULT);
		}
		delete[](sd);
	}
	ExitProcess(0);
};
CIMPORT bool CompressData(unsigned char** pOutData, unsigned* pOutLen, unsigned char* InData, unsigned InLen);
//LW_compress&dest_file&src1&src2&...
void exec_LW_compress(int Np,char** par,int* size){
	if(Np<2)return;
	char fnm[256];
	char fnm_tmp[256];
	char head[128];
	sprintf(fnm_tmp,"Internet\\Cash\\Upload\\temp");
	ResFile F=RRewrite(fnm_tmp);
	if(F!=INVALID_HANDLE_VALUE){
		for(int i=1;i<Np-1;i++){
			sprintf(fnm,"%s",par[i]);
			ResFile F1=RReset(fnm);
			if(F1!=INVALID_HANDLE_VALUE){
				int sz=RFileSize(F1);
				sprintf(head,"%s %d",fnm,sz);
				int L=strlen(head);
				for(int i=L;i<128;i++)
					head[i]=0;
				char* Buf=new char[sz];
				RBlockRead(F1,Buf,sz);
				RBlockWrite(F,head,128);
				RBlockWrite(F,Buf,sz);
				delete[](Buf);
				RClose(F1);
			};
		...
		if(F!=INVALID_HANDLE_VALUE){
			...
			if(F1!=INVALID_HANDLE_VALUE){
				...
				unsigned char* dd=NULL;
				unsigned dLen=0;
				CompressData(&dd,&dLen,(unsigned char*)Buf,sz);
				if(dd)RBlockWrite(F,dd,dLen);
				delete[](dd);
				free(Buf);
			}
			RClose(F);
...
void exec_LW_decompress(int Np,char** par,int* size){
	...
	if(F!=INVALID_HANDLE_VALUE){
		int sz=RFileSize(F);
		char head[64];
		RBlockRead(F,head,64);
		int size1=atoi(head);
		char* sd=new char[size1];
		RBlockRead(F,sd,size1);
		int size2=sz-size1-64;
		char* sg=new char[size2];
		RBlockRead(F,sg,size2);
		unsigned char* dd=NULL;
		unsigned dLen=0;
		DecompressData(&dd,&dLen,(unsigned char*)sd);
		RClose(F);
...
CIMPORT bool CompressData(unsigned char** pOutData, unsigned* pOutLen, unsigned char* InData, unsigned InLen);
//LW_compress&dest_file&sorce_folder
//do not compress .g17  .g2d .m3d
void ReadDir(char* Pass0,char* Pass,ResFile &F0,ResFile &F){
	WIN32_FIND_DATA	FD;
	HANDLE	hFindDir;
	DString root;
	root.Add(Pass0);
	root.Add(Pass);
	root.Add("*");
	hFindDir=FindFirstFile(root.str,&FD);
		...
		do{
			if(strcmp(FD.cFileName,".") && strcmp(FD.cFileName,"..")){
				if(FD.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY){
					...
					new_root.Add("\\");
					ReadDir(Pass0,new_root.str,F0,F);
				}else{
					DString file0;
					file0.Add(Pass0);
					file0.Add(Pass);
					file0.Add(FD.cFileName);
					DString file;
					file.Add(Pass);
					file.Add(FD.cFileName);
					ResFile F1=RReset(file0.str);
					if(F1!=INVALID_HANDLE_VALUE){
						int sz=RFileSize(F1);
						char head[128];
						bool no_compress=false;
						int L0=strlen(file.str);
						// hide this for old version
						if(strstr(FD.cFileName,".g17") || strstr(FD.cFileName,".g2d")
						   || strstr(FD.cFileName,".m3d")){
							no_compress=true;
							sprintf(head,"%s@ %d",file.str,sz);
							L0++;
						}else{
							sprintf(head,"%s %d",file.str,sz);
						}
						// end hide
						//sprintf(head,"%s %d",file.str,sz);//old version
						int L=strlen(head);
						head[L0]=0;
						for(int i=L;i<128;i++)
							head[i]=0;
						char* Buf=new char[sz];
						RBlockRead(F1,Buf,sz);
						RBlockWrite(F0,head,128);
						if(no_compress)
							RBlockWrite(F,Buf,sz);
						else
							RBlockWrite(F0,Buf,sz);
						delete[](Buf);
						RClose(F1);
					}
				}
...
void exec_LW_compress(int Np,char** par,int* size){
	...
	char fnm_tmp0[128];
	char fnm_tmp[128];
	sprintf(fnm_tmp0,"Internet\\Cash\\Upload\\temp0");
	sprintf(fnm_tmp,"Internet\\Cash\\Upload\\temp");
	ResFile F0=RRewrite(fnm_tmp0);
	ResFile F=RRewrite(fnm_tmp);
	if(F!=INVALID_HANDLE_VALUE && F0!=INVALID_HANDLE_VALUE){
		for(int i=1;i<Np-1;i++){
			sprintf(fnm,"%s",par[i]);
			char cc[1];
			cc[0]=0;
		...
		if(F!=INVALID_HANDLE_VALUE){
			...
			if(F1!=INVALID_HANDLE_VALUE){
				...
				if(dd){
					...
					for(int i=L;i<64;i++)
						head[i]=0;
					RBlockWrite(F,head,64); //hide this for old version
			...
			if(DEVSCOPE.DEVS[DEVSCOPE.REQ[i].DevIndex].GetRequestResult){
				...
				if(R==128){//success
					...
					for(int j=0;j<P1.NComm;j++){
						...
						if(!strcmp(com,"LW_upload")){
							exec_LW_upload(P1.Comm[j].NParams,P1.Comm[j].Params,P1.Comm[j].ParamSize);
						}else
						if(!strcmp(com,"LW_decompress")){
							exec_LW_decompress(P1.Comm[j].NParams,P1.Comm[j].Params,P1.Comm[j].ParamSize);
						}else
						if(!strcmp(com,"LW_compress")){
							exec_LW_compress(P1.Comm[j].NParams,P1.Comm[j].Params,P1.Comm[j].ParamSize);
						};
						//will grow there
					};
...
void FilterRQ2Send(sicExplorer* SXP,ParsedRQ* RQ,bool AllowNew){
	...
	for(int i=0;i<RQ->NComm;i++){
		...
		if(cid[0]=='L'&&cid[1]=='W'&&cid[2]=='_'){
			...
			if(!strcmp(cid,"LW_upload")){
				char* win=SXP->Windows[SXP->CurWPosition]->WinID;
				RQ->AddParamToCom(i,win,strlen(win));
				exec_LW_upload(RQ->Comm[i].NParams,RQ->Comm[i].Params,RQ->Comm[i].ParamSize);
			}else
			if(!strcmp(cid,"LW_decompress")){
				char* win=SXP->Windows[SXP->CurWPosition]->WinID;
				RQ->AddParamToCom(i,win,strlen(win));
				exec_LW_decompress(RQ->Comm[i].NParams,RQ->Comm[i].Params,RQ->Comm[i].ParamSize);
			}else
			if(!strcmp(cid,"LW_compress")){
				char* win=SXP->Windows[SXP->CurWPosition]->WinID;
				RQ->AddParamToCom(i,win,strlen(win));
				exec_LW_compress(RQ->Comm[i].NParams,RQ->Comm[i].Params,RQ->Comm[i].ParamSize);
			};
