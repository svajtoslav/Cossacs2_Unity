...
public:
	...
	virtual const char* GetThisElementView(const char* Local){
		static char cc[128];
		if(Description.str)
			sprintf(cc,"%s: %s",Local,Description.str);
		else
			sprintf(cc,"%s",Local);
		return cc;
	};
...
public:
	...
	virtual int CreateControl(ParentFrame* Base,int x,int y,int x1,int y1,BaseClass* Class,void* DataPtr,void* ExtraPtr, ControlParams* CParam){
		...
		CParam->GP_dx=int((CParam->LeftColumnWidth-CParam->AbsMinLeftWidth-CParam->MinLeftWidth+36)/2);
		CParam->TextFont=&BlackFont;
...
public:
	...
	virtual int CreateControl(ParentFrame* Base,int x,int y,int x1,int y1,BaseClass* Class,void* DataPtr,void* ExtraPtr, ControlParams* CParam){
		IB=Base->addInputBox(NULL,x+1,y+2,str,4,25,y1-y+1,&CED_Font,&CED_AFont);
		IB->Centering=true;
...
public:
			...
			do{
				if(strcmp(FD.cFileName,".") && strcmp(FD.cFileName,"..")){
					if(FD.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY){
						...
						new_root.Add("\\");
						ReadDir(new_root.str);
					}else{
						if(strstr(FD.cFileName,".c2m")){
							DString file;
							file.Add(Pass);
							file.Add(FD.cFileName);
							FileArr.Add(new DString(file.str));
						}
					}
				}
	...
	static bool MinusClick(SimpleDialog* SD){
		...
		ME->id--;
		if(ME->id<0) ME->id=ME->FileArr.GetAmount()-1;
		Lpressed=false;
		return false;
	}
	static bool PlusClick(SimpleDialog* SD){
		_ModelID_editor* ME=(_ModelID_editor*)SD->UserParam;
		ME->id++;
		if(ME->id>=ME->FileArr.GetAmount()) ME->id=0;
		Lpressed=false;
		return false;
	}
	virtual int CreateControl(ParentFrame* Base,int x,int y,int x1,int y1,BaseClass* Class,void* DataPtr,void* ExtraPtr, ControlParams* CParam){
		...
		sprintf(str,"%s",IMM->GetModelFileName(*model_id));
		strcpy(pstr,str);
		id=-1;
		for(int i=0;i<FileArr.GetAmount();i++){
			if(!stricmp(str,FileArr[i]->str)){
				id=i;
				break;
			}
		}
		IB=Base->addInputBox(NULL,x+28,y+4,str,120,x1-x-30,y1-y+1,&CED_Font,&CED_AFont);
		IB->Centering=true;
		int idx=GPS.PreLoadGPImage("Interf3\\btnz");
		Plus=Base->addGP_Button(NULL,x,y+32,idx,5,4);
		Minus=Base->addGP_Button(NULL,x,y+82,idx,7,6);
		y1+=100;
		IB->Enabled=0;
		SimpleDialog* SD=Base->addViewPort(x+25,y,x1-x-24,y1-y+1);
		SD->UserParam=int(this);
		Minus->UserParam=int(this);
		Plus->UserParam=int(this);
		p=new LS_gp_param;
		p->destname=str;
		p->ext="*.C2M";
		p->startdir="Models\\";
		p->CE=CParam->CE;
		SD->OnDraw=&ModelDraw;
		Minus->OnUserClick=&MinusClick;
		Plus->OnUserClick=&PlusClick;
	...
	virtual bool Process(){
		...
		if(id!=-1 && FileArr.GetAmount()>0){
			sprintf(str,"%s",FileArr[id]->str);
			strcpy(pstr,str);
			*model_id=IMM->GetModelID(FileArr[id]->str);
		}
		return true;
	}	
	/*virtual bool Assign(xmlQuote* xml){
...
public:
	...
	virtual int CreateControl(ParentFrame* Base,int x,int y,int x1,int y1,BaseClass* Class,void* DataPtr,void* ExtraPtr, ControlParams* CParam){
		...
		if(!OC->ViewType){
			show=true;
			IB=Base->addInputBox(NULL,x,y+3,str,4,25,y1-y+1,&CED_Font,&CED_AFont);
			IB->Centering=true;
			int idx=GPS.PreLoadGPImage("Interf3\\btnz");
