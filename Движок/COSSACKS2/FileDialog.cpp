		...
		do{
			if(strcmp(FD.cFileName,".") && strcmp(FD.cFileName,"..")){
				if(FD.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY){
					...
					this->DirArr.Add(ss);
					char cc[128];
					sprintf(cc,"{P Interf3\\btnz 26 12 0 -1 18}%s",FD.cFileName);
...
bool ceFileDialog::CreateDialogEditor(int x,int y,int x1,int y1){
	...
	for(int i=0;i<NT;i++)
		CB->AddLine(this->FileType[i]->str);
	...
	do{
		...
		ProcessMessages();
		DSS.ProcessDialogs();
		if(FPV&&Preview&&FullFileName.str&&bActive){			
			char cc2[256];
			strcpy(cc2,FullFileName.str);
			int L=strlen(cc2);
			...
			FPV->DrawPreview(Preview->LastDrawX,Preview->LastDrawY,
				Preview->LastDrawX+Preview->x1-Preview->x,
				Preview->LastDrawY+Preview->y1-Preview->y,FullFileName.str);
			PopWindow(&TW);
		}
		DSS.RefreshView();
		if(ItemChoose==1){
			...
			_str* ss=new _str;
			ss->Assign(this->DirArr[ItemChoose-10]->str);
			this->Pass.Add(ss);
			this->DrawDir(DD1,Mask);
		}else if(ItemChoose>=1000){
			this->FileName=this->DirArr[ItemChoose-1000]->str;			
			IB->CursPos=100;
			IB->Selected=-1;
		}
		...
		if(NPath){
			for(int i=0;i<NPath;i++){
				this->FullFileName.Add(this->Pass[i]->str);
				this->FullFileName.Add("/");
			}
		}
		if(IB->_Str->str&&IB->_Str->str[0]){
			_str str;
			if(OnlyName(this->FileName.str,&str,Mask)){
				this->FullFileName.Add(str);
				this->FullFileName.Add(Mask);
			}
		}
		TB->SetMessage(this->FullFileName.str);
		ItemChoose=-1;
	}while(ItemChoose==-1);
	ProcessMessages();
	DSS.ProcessDialogs();
	DSS.RefreshView();
	if(FPV)delete(FPV);
	int L=strlen(FullFileName.str);
	for(int j=0;j<L;j++)if(FullFileName.str[j]=='/')FullFileName.str[j]='\\';
	ItemChoose=-1;
	return res;
}