...
void GroundUpdate::Create(){
	...
	IDX[5]=2;
	TextureDescr TD;
	TD.setValues(256,256,cfRGB565,mpManaged,1,tuProcedural);
	DEF_textureID=IRS->CreateTexture("DEF_texture",TD);
	//TD.setValues(256,256,cfRGB565,mpSysMem,1,tuProcedural);
	SYS_textureID=DEF_textureID;//IRS->CreateTexture("SYS_texture",TD);
	UpMesh.setNInd(6);
	UpMesh.setNPri(2);
	UpMesh.setNVert(4);
...
void GroundUpdate::Update(byte* ptr,int pitch,int x,int y,int Lx,int Ly){
	...
	if(sPitch){
		...
		if(Copy16dest)IRS->SetRenderTarget(IRS->GetVBufferID());
		IRS->Draw(UpMesh);
		if(Copy16dest)IRS->SetRenderTarget(0);
	};
};
...
void PiroSystem::Draw(){
	if(!PiroMesh){
		...
		//for(int i=0;i<3;i++){
			...
			PiroMesh->create(4*MAXMESH,6*MAXMESH,vfTnL);
