...
DialogsSystem::DialogsSystem(int x,int y){
	...
	CenteredHint=1;
	SetHintStyle(1,1,0,0,3,3,20,20,400,400,&SmallBlackFont,0xFFFFFFFF,0);
	x1=RealLx-1;
	y1=RealLy-1;
		...
		case 7:
			{
                ...
                rsFlushPoly2D();
                IRS->SetCurrentShader( IRS->GetShaderID( "hud" ) );
...
bool GPPicture_OnDraw(SimpleDialog* SD){
	...
	if(pic->ShapeFileID!=0xFFFF){
		pic->ShapeFileID=pic->FileID;
		pic->ShapeSpriteID=pic->SpriteID&4095;
	}	
	pic->x1=pic->x+GPS.GetGPWidth(pic->FileID,pic->SpriteID&4095)-1;
	pic->y1=pic->y+GPS.GetGPHeight(pic->FileID,pic->SpriteID&4095)-1;
	if(pic->Rotated){
		DrawRotatedPicture(pic->x,pic->y,pic->FileID,pic->SpriteID,pic->Nation,pic->rx,pic->ry,pic->Angle,pic->ScaleX,pic->ScaleY);
	}else GPS.ShowGP(pic->x,pic->y,pic->FileID,pic->SpriteID,pic->Nation);
	FLUSH();
	return false;
};
GPPicture::GPPicture(){
	...
	*/
	if(CSpr>=0){		
		...
		if(DisableCycling){
			ShapeFileID=GP_File;
			ShapeSpriteID=CSpr;	
			GPS.ShowGP(x,y,GP_File,CSpr,Nation);
		}else{
			ShapeFileID=0xFFFF;
			ShapeSpriteID=0xFFFF;	
			if(OneSprited) DrawHeaderEx2(x,y,Lx,GP_File,-1,-1,CSpr,CSpr,CSpr,Nation);
			else DrawHeaderEx2(x,y,Lx,GP_File,CSpr,CSpr+1,CSpr+2,CSpr+3,CSpr+4,Nation);
		}
		/*
	...
	if(CFN){
		//int xc=(x+x1)>>1;
		//int yc=((y+y1)>>1)-(GetRLCHeight(CFN->RLC,'W')>>1);
		//int rlen=GetRLen(Message,CFN);
		//ShowStringEx(xc-(rlen>>1)+FontDx,yc+FontDy,Message,CFN);
		//int dy=CSpr>=0?(y+y1-GetRLCHeight(CFN->RLC,'W'))>>1:y;
		DWORD fc=CFN->GetColor();
		if(!Enabled){
			//CFN->SetColor(0xFF665F57); //0x60665F57
...
bool NewGP_VScrollBar_OnDraw(SimpleDialog* SD){
	...
	//-----------------------------
	if(!DontShow)GPS.ShowGP(scx,scy,SB->GP_File,SB->StartGP_Spr+4,0);
	return false;
};
int ScrollFileID=-1;
...
bool InputBoxOld_OnDraw(SimpleDialog* SD){
	...
	if(SD&&(IB->Str||IB->_Str)){
		...
		int LL=IB->x1-IB->x;
		char base[2048];
		strcpy(base,IB->Str?IB->Str:IB->_Str->str);
		if(IB->Anonim){
			int L=strlen(IB->Str?IB->Str:IB->_Str->str);
			for(int i=0;i<L;i++)base[i]='*';
		};
		int xx=0;
		if(IB->CursPos>strlen(IB->Str?IB->Str:IB->_Str->str))IB->CursPos=strlen(IB->Str?IB->Str:IB->_Str->str);
...
bool InputBoxOld_OnClick(SimpleDialog* SD){
	...
	if(IB->MouseOver&&Lpressed&&(IB->Str||IB->_Str)){
		...
		int xx1=0;
		int L=IB->Str?strlen(IB->Str):strlen(IB->_Str->str);
		for(int j=0;j<L;j++){
...
bool InputBoxOld_OnKeyDown(SimpleDialog* SD){
	...
	if(LastKey==VK_RIGHT){
		if(IB->CursPos<strlen(IB->Str?IB->Str:IB->_Str->str))IB->CursPos++;
		KeyPressed=0;
		LastKey=0;
		return true;
	}else
	if(LastKey==VK_END){
		IB->CursPos=strlen(IB->Str?IB->Str:IB->_Str->str);
		KeyPressed=0;
		LastKey=0;
		return true;
	}else
	...
	if(LastKey==46){//VK_DEL
		if(IB->Str){
            ...
            if(IB->CursPos<strlen(IB->_Str->str))IB->_Str->DeleteChars(IB->CursPos,1);
		}
		//if(strlen(IB->Str)>0)IB->Str[strlen(IB->Str)-1]=0;
		return true;
	}else{
		if(LastAsciiKey&&LastAsciiKey>=32){
			...
			xx[1]=0;
			xx[0]=char(LastKey);
			if(strlen(IB->Str?IB->Str:IB->_Str->str)+1<DWORD(IB->StrMaxLen)){
				char ccc[2048];
				strcpy(ccc,IB->Str?IB->Str:IB->_Str->str);
				ccc[IB->CursPos]=0;
				strcat(ccc,xx);
				strcat(ccc,(IB->Str?IB->Str:IB->_Str->str)+IB->CursPos);
...
bool ComboBox_OnDraw(SimpleDialog* SD){
	...
	if(CB->GP_File!=-1){
		...
		if(CB->DLX){
			...
			GPS.ShowGP(CB->x,CB->y,CB->GP_File,CB->UpPart+1-((CB->MouseOver||CB->IsActive)&&CB->Enabled),0);
			PopWindow(&TW);
			IntersectWindows(XI,0,RealLx,RealLy);
			GPS.ShowGP(CB->x-CB->DLX,CB->y,CB->GP_File,CB->UpPart+1-((CB->MouseOver||CB->IsActive)&&CB->Enabled),0);
			PopWindow(&TW);
		}else GPS.ShowGP(CB->x,CB->y,CB->GP_File,CB->UpPart+1-((CB->MouseOver||CB->IsActive)&&CB->Enabled),0);
		GPS.SetCurrentDiffuse(c);
	};
			...
			if(i==CB->LightIndex){
				if(CB->GP_File!=-1){
					if(ADLX){//CB->DLX){
						...
						PopWindow(&TW);
						IntersectWindows(XI,0,RealLx,RealLy);
						GPS.ShowGP(CB->DropX-ADLX,y0,CB->GP_File,CB->UpPart+(randoma[i*17]&1)+3,0);
						PopWindow(&TW);
...
ComboBox::ComboBox(){
	...
	Center=9;
	DownPart=0;
	GP_File=GPS.PreLoadGPImage("Interf3\\elements\\combo");
	x=0;y=0;x1=200;
	OneLx=x1-x+1;
...
SimpleDialog* SelectNearestFrame(ClassArray<SimpleDialog>* DSS,int x,int y,int& xd,int& yd){
	...
	int N=EnumerateFrames(DSS,SDS,128,&CheckFrameInside,&mxy);
	int MinR=10000;
	...
	bool DoOneLine(byte Draw){		
		...
		while(s[pos]){
			if(s[pos]=='{'){
				...
				int dp=0;
				pos++;
				while(dp<254&&s[pos+dp]!='}'&&s[pos+dp]!=0){
					comstr[dp]=s[pos+dp];
					comstr[dp+1]=0;
					dp++;
				}
				...
				if((comstr[0]=='P'||comstr[0]=='G')&&comstr[1]==' '){
					...
					if(idx!=-1){
						...
						if(Draw==1){
							GPS.ShowGP(xL+CurLx-dx,yL-Ly-dy,idx,sidx,0);
						}
						CurLx+=Lx-dx;
                        int H=Ly-dy;
						if(H>Height){
				...
				if(comstr[0]=='I'){
					...
					if(OTI){
						if(OTI->IconType==1){
							if(OTI->TextureID>0){
								const TextureDescr* td=IRS->GetTextureDescr(OTI->TextureID);
								float TLX=td->getSideX();
								float TLY=td->getSideY();
								if(TLX&&TLY){
									if(Draw==1){
						...
						if(OTI->IconType==2){
							...
							if(Draw==1){
								GPS.ShowGP(xL+CurLx-OTI->dx,yL-OTI->Ly,OTI->gpFile,OTI->Sprite,0);
							}
							CurLx+=OTI->Lx-OTI->dx;						
							int H=OTI->Ly-OTI->dy;						
							if(H>Height){
				...
				if(pos==sposL&&s[pos]==' ')pos++;
					...
					if(CurLx+cLx>MaxWidth){
						if(LastSpacePos>0){//&&LastSpaceLx>MaxWidth*3/4){
                            //breaking at space
							sposR=LastSpacePos;
							CurLx=LastSpaceLx;
							NSym=LastSpaceSym;
	...
	if(Draw){
		if(GlobalTextMouseOverCommand[0]&&!strncmp(GlobalTextMouseOverCommand,"$HINT ",6)){
			...
			if(!isins){
				...
				GPS.FlushBatches();
				int LX=320;
				int LY=DrawMultilineText(0,0,s,&SmallBlackFont,LX,100000,false,false);
				int x=mouseX+30;
				int y=mouseY+30;
				...
				GPS.FlushBatches();
				LX=320;
				DrawMultilineText(x+10,y+10,s,&SmallBlackFont,LX,100000,false,true);
				PopWindow(&TW);
...
void ParentFrame::ProcessAligning(){
	...
	if(LeftAlign==2){
		x=int(float(pw)*LA_param);
	}
	if(RightAlign==1){
		x1=pw-int(RA_param)-1;
	}
	if(RightAlign==2){
		x1=int(float(pw)*RA_param);
	}
	if(HorizontalCenterAlign==1){
		int dx=pw-int(HCA_param)-(x+x1)/2;
		x+=dx;
		x1+=dx;
	}
	if(HorizontalCenterAlign==2){
		int dx=int(float(pw)*HCA_param)-(x+x1)/2;
		x+=dx;
		x1+=dx;
	}
	if(TopAlign==1){
	...
	if(TopAlign==2){
		y=int(float(ph)*TA_param);
	}
	if(BottomAlign==1){
		y1=ph-int(BA_param)-1;
	}
	if(BottomAlign==2){
		y1=int(float(ph)*BA_param);
	}
	if(VerticalCenterAlign==1){
		int dy=ph-int(VCA_param)-(y+y1)/2;
		y+=dy;
		y1+=dy;
	}
	if(VerticalCenterAlign==2){
		int dy=int(float(ph)*VCA_param)-(y+y1)/2;
		y+=dy;
		y1+=dy;
	}
	//*
...
void ParentFrame::ProcessLocalAligning(){
	...
	if(LeftAlign==1){
		x=px+pw-int(LA_param);
	}
	if(LeftAlign==2){
		x=px+int(float(pw)*LA_param);
	}
	if(RightAlign==1){
		x1=px+pw-int(RA_param)-1;
	}
	if(RightAlign==2){
		x1=px+int(float(pw)*RA_param);
	}
	if(HorizontalCenterAlign==1){		
	...
	if(TopAlign==1){
		y=py+ph-int(TA_param);
	}
	if(TopAlign==2){
		y=py+int(float(ph)*TA_param);
	}
	if(BottomAlign==1){
		y1=py+ph-int(BA_param)-1;
	}
	if(BottomAlign==2){
		y1=py+int(float(ph)*BA_param);
	}
	if(VerticalCenterAlign==1){
...
void GlobalHintInfo::Draw(){
	if(!HintMessage.str)return;
	char* Hint=HintMessage.str;
...
void DialogsSystem::ProcessDialogs(){
	...
	if(ActiveParent&&ActiveParent->OnDrawActive&&IsDrawActive){
		TempWindow TW;
		PushWindow(&TW);
	...
	if(EditMode){
		...
		int mdx=(((GetKeyState(VK_LEFT)&0x8000)?-1:0)+((GetKeyState(VK_RIGHT)&0x8000)?1:0))*((GetKeyState(VK_CONTROL)&0x8000)?10:1);
		int mdy=(((GetKeyState(VK_UP)&0x8000)?-1:0)+((GetKeyState(VK_DOWN)&0x8000)?1:0))*((GetKeyState(VK_CONTROL)&0x8000)?10:1);
	...
	*/
	if(SD_Hinted)Hint=SD_Hinted->Hint;
	TempWindow TW;
	if(ActiveParent&&ActiveParent->OnDrawActive&&IsDrawActive){
		TempWindow TW;
		PushWindow(&TW);
	...
	if(s->str&&s->str[0]){
		...
		if(addHotKey){
			...
			if(EngSettings.HintTemplate.HotKey.isClear()){
				s->print(" ({CG}%s{C})",GetTextByID(GetKeyName(HotKey)));
			}else{
				s->Add(" ");
				s->print(EngSettings.HintTemplate.HotKey.str,GetTextByID(GetKeyName(HotKey)));
			}				
			HintWithHotKey=true;
		}else{
			text=GetTextByID(s->str);
		}
		if(text!=s->str){
			Hint=text;
			AllocHint=0;
...
void SimpleDialog::Process(SimpleDialog** LeftClick,SimpleDialog** RightClick,SimpleDialog** HintDlg){
	...
	if(HotKey&&Enabled&&LastKey==HotKey&&!(EditMode||EnterChatMode)/*&&(!ChangeNation&&!EditMapMode)*/){
		if(!KeysAreFocusedOnInputBox||LastKey==VK_ESCAPE||LastKey==VK_RETURN){
			if(!((GetKeyState(VK_CONTROL)&0x8000)&&HotKey=='A')){
				*LeftClick=this;
			}
			LastKey=0;
			//KeyPressed=0;
...
SimpleDialog* ParentFrame::Find(char* ID){
	if(ID){
		int N=DSS.GetAmount();
		for(int i=0;i<N;i++)if(DSS[i]){
			if(DSS[i]->Name.str&&!strcmp(DSS[i]->Name.str,ID))return DSS[i];
			else{
				SimpleDialog* SD=DSS[i]->Find(ID);
				if(SD)return SD;
			}
...
bool ParentFrame::DeleteItem(char* ID){
	bool Done=0;
	int N=DSS.GetAmount();
	for(int i=0;i<N;i++)if(DSS[i]&&DSS[i]->Name.str&&!strcmp(DSS[i]->Name.str,ID)){
		SimpleDialog* SD=DSS[i];
		delete(SD);
		DSS.Del(i,1);
		i--;
...
int ListDesk::AddElement(int Pos, char* message, int HotKey){
	...
	if(SD){
		...
		if(VB){
			...
			VBN->x1=VB->x1;
			VBN->y1=VB->y1;
			VBN->FontPassive=VB->FontPassive;
			VBN->FontOver=VB->FontOver;
			VBN->FontDx=VB->FontDx;
			VBN->FontDy=VB->FontDy;		
			VBN->Align=VB->Align;
			VBN->OneSprited=VB->OneSprited;
			VBN->DisableCycling=VB->DisableCycling;		
			VBN->GP_File=VB->GP_File;
			VBN->SpritePassive[0]=VB->SpritePassive[0];
			VBN->SpriteOver[0]=VB->SpriteOver[0];
			VBN->SpriteDx[0]=VB->SpriteDx[0];
			VBN->SpritePassive[1]=VB->SpritePassive[1];
			VBN->SpriteOver[1]=VB->SpriteOver[1];
			VBN->SpriteDx[1]=VB->SpriteDx[1];
			VBN->SpritePassive[2]=VB->SpritePassive[2];
			VBN->SpriteOver[2]=VB->SpriteOver[2];
			VBN->SpriteDx[2]=VB->SpriteDx[2];
			VBN->SetMessage(message);
            VBN->Hint=VB->Hint;
			VBN->HotKey=VB->HotKey;
			if(VB->v_Actions.GetAmount()){
				VB->v_Actions.Copy(&VBN->v_Actions);
			}
		}else{
...
void ListDesk::_Draw(){
	...
	//VitButton* VB=dynamic_cast<VitButton*>(Element.Get());
	//int ButH=VB?VB->GetHeightGP()+marginY:25;	
	/*
	int ButH=25;
	SimpleDialog* SD=Element.Get();
	if(SD) ButH=SD->GetHeight();
	*/
	int yy=marginY;//marginY;
	//int dy=ButH+marginY;
	for(int i=0;i<N;i++){
		SimpleDialog* SD=DSS[i];
		if(SD&&SD->Visible){
			...
			SD->Sety(yy);
			SD->SetWidth(ButW);
			//DSS[i]->SetHeight(ButH);
			//if(VB) DSS[i]->SetHeight(VB->GetHeightGP());
	...
	if(EditMode){
		...
		if(vb){
			...
			int st=vb->State;
			vb->State=1;
			vb->Setx(x+l+marginX+xx);
			vb->SetWidth(ButW);
			vb->SetHeight(vb->GetHeight());
			for(int i=0;i<15;i++){
				vb->Sety(y+t+yy+yyy);
				vb->_Draw();
				yyy+=vb->GetHeight()+marginY;
			}
			vb->Setx(xx);
			//vb->SetWidth(ButW);
			vb->Sety(yy);
		...
		TextButton* tb=dynamic_cast<TextButton*>(Element.Get());
		if(tb){
			int xx=tb->x;
			int yy=tb->y;
			int yyy=0;
			//int st=vb->State;
			//vb->State=1;
			tb->Setx(x+l+marginX+xx);
			tb->SetWidth(ButW);
			tb->SetHeight(tb->GetHeight());
			for(int i=0;i<15;i++){
				tb->Sety(y+t+yy+yyy);
				tb->_Draw();
				yyy+=tb->GetHeight()+marginY;
			}
			tb->Setx(xx);
			//vb->SetWidth(ButW);
			tb->Sety(yy);
...
void ProgressBar::_Draw(){
	...
	GPS.ShowGP(x+BackDx,y+BackDy,BackGP_File,BackSprite,Nation);
    TempWindow TW;
	PushWindow(&TW);
	if(MaxValue){
		switch(ProgressType){
	...
	GPS.ShowGP(x+ProgressDx,y+ProgressDy,ProgressGP_File,ProgressSprite,Nation);
	PopWindow(&TW);
	GPS.SetCurrentDiffuse(0xFFFFFFFF);
	GPS.SetScale(1.0);
	if(Font&&Message.str)ShowStringEx((x+x1-GetRLen(Message.str,Font))/2,y+MessageDy,Message.str,Font);	
}
const char* ParentFrame::GetThisElementView(const char* LocalName){	
	...
	if(Name.str&&Name.str[0]){
		sprintf(pf_text+strlen(pf_text)," [%s]",Name.str);
	}		
	return pf_text;
}
...
void SimpleDialog::NewAction::EvaluateFunction(){
	char* Name=SimpleDialog::NewAction::ActionName.str;
	if(Name&&Name[0]){		
		_str s0;
		...
		if(CPP&&H&&REG){
			...
			fprintf(H,"    ENDSAVE\n");
			fprintf(H,"};\n",Name);
			fprintf(REG,"REG_CLASS_EX(%s,\"%s\")\n",Name,SimpleDialog::NewAction::ActionGroup.str);
			...
			if(SD){
				...
				if(OCS){
					char* top=SimpleDialog::NewAction::ActionGroup.str;
					if(!top)top="";
