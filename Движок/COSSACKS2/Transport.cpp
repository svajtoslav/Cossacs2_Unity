...
char GetBestDirection(MotionField* MFI,int xd,int yd,int LX,char CurDir){
	...
	for(int D0=0;D0<256;D0+=8){
		int DY=((LX+10)*TSin[D0])/256;
		int DX=((LX+10)*TCos[D0])/256;
        int H=GetHeight((xd+DX)*16,(yd+DY)*16);
		if(H>0){
			tdx+=DX;
			tdy+=DY;
			NDir++;
...
void RetLock(OneObject* OB){
	...
	if(OB->GLock)MFI->SetRound(OB->x,OB->y,LX);
	if(OB->LLock)UnitsField.SetRound(OB->x,OB->y,LX);
};
bool FindCostPosition(OneObject* OB,int* XDest,int* YDest,char* Direction){
	int LX=OB->Lx;
	MotionField* MFI=MFIELDS+OB->LockType;
	if(OB->GLock)MFI->ClearRound(OB->x,OB->y,LX);
	if(OB->LLock)UnitsField.ClearRound(OB->x,OB->y,LX);
	int LX2=(LX)>>1;
	int yd=(*YDest)-LX2;
	int xd=(*XDest)-LX2;
	int xs=OB->x;
...
bool SearchTransportPlace(int* x,int* y,int Lx){
	...
	for(int r=0;r<40;r++){
		...
		for(int p=0;p<N;p++){
			int xx=x0+xi[p];
			int yy=y0+yi[p];
			if(!MFIELDS[1].CheckRound(xx-2,yy-2,Lx+4)){
				*x=xx;
				*y=yy;
				return true;
			};
	...
	if(tt){
		...
		if(abs(ddir)<=MRot){
			...
			///if(OB->newMons->Transport){
				...
				if(!MFIELDS->CheckRound((xx1>>8)-2,(yy1>>8)-2,4)){
					//OB->DstX=xx1;
					//OB->DstY=yy1;
				};
			//};
			...
			if(ddir>0)OB->RealDirPrecise+=MRot*GameSpeed;
			else OB->RealDirPrecise-=MRot*GameSpeed;
			OB->RealDir=OB->RealDirPrecise/256;
			OB->GraphDir=OB->RealDir;
		};	
	}else{
		int xxx=OB->x;
		int yyy=OB->y;
		if(OB->GLock)MFIELDS[1].ClearRound(xxx,yyy,LX);
		if(distan<8&&MFIELDS[1].CheckRound(xxx-3,yyy-3,LX+6)){
			OB->DeletePath();
			OB->DestX=-1;
		...
		if(OB->GLock)MFIELDS[1].SetRound(xxx,yyy,LX);
	};	
};
