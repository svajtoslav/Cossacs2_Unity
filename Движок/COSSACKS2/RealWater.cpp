...
void SetDeepSpot(int x,int y,int r,int dr,int dh){
	...
	if(Change){
		CreateMiniMapPart((x0>>1)-2,(y0>>1)-2,(x1>>1)+2,(y1>>1)+2,false);
	};
};
	...
	for(int iy=x;iy<y1;iy++){
		for(int ix=x;ix<x1;ix++){
			...
			*/
			...
			if(D1>UDeep)MFIELDS->BSetPt(xx,yy);
			if(D2>UDeep)MFIELDS->BSetPt(xx+1,yy);
			if(D3>UDeep)MFIELDS->BSetPt(xx,yy+1);
			if(D4>UDeep)MFIELDS->BSetPt(xx+1,yy+1);

			if(D1>HDeep)MFIELDS[2].BSetPt(xx,yy);
			if(D2>HDeep)MFIELDS[2].BSetPt(xx+1,yy);
			if(D3>HDeep)MFIELDS[2].BSetPt(xx,yy+1);
			if(D4>HDeep)MFIELDS[2].BSetPt(xx+1,yy+1);
