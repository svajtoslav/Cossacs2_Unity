...
bool AddExGroupInfo(char* Name, GroupExInfo* Info){
	...
	for(int i=0;i<N;i++)
		if(!strcmp(AGroups[i]->Name.str,Name)){
...
bool RegisterUnits(GAMEOBJ* GOBJ,char* Name){
	for(int i=0;i<AGroups.GetAmount();i++)if(!strcmp(AGroups[i]->Name.str,Name)){
		GOBJ->Type='UNIT';	
...
void RegisterZoneEx(GAMEOBJ* GOBJ,char* Name, bool ErrRaport){
	...
	for(int i=0;i<AZones.GetAmount();i++){
		if(!strcmp(Name,AZones[i]->Name.str)){
			if(NZON<64){
				ZIDS[NZON]=i;
				NZON++;
				AZones[i]->Dir=0;
...
void InitialUpgrade(char* Grp,char* Upgrade){
	for(int i=0;i<AGroups.GetAmount();i++)if(!strcmp(AGroups[i]->Name.str,Grp)){
		GAMEOBJ UP;
...
int	GetBrigadesID(byte NI,GAMEOBJ* pGrp){
	...
	for (int i=0; i<NU; i++){
		...
		if (MID!=0xFFFF) {
			OneObject* OB=Group[MID];
			if (OB&&(OB->Hidden||!OB->Sdoxlo)&&OB->Serial==SIDS[i]){
...
DLLEXPORT
void OBJ_ChangeNation(OneObject* OB, byte DstNat){
	if(OB&&!OB->Sdoxlo&&OB->NNUM!=DstNat){
		DelObject(OB);
		//OB->Nat->CITY->UnRegisterNewUnit(OB);
		OB->Ref.General=NATIONS[DstNat].Mon[OB->NIndex];
		OB->Nat=&NATIONS[DstNat];
...
void StartAIEx(byte Nat,char* Name,int Land,int Money,int ResOnMap,int Difficulty){
	...
	if(MOptions.Players.Player[Nat].CustomAI_Script.str&&MOptions.Players.Player[Nat].CustomAI_Script.str[0]){
		Name=MOptions.Players.Player[Nat].CustomAI_Script.str;
	}
...
bool ProduceUnitTicks(GAMEOBJ* Units,GAMEOBJ* UnitType,GAMEOBJ* DestGroup,int Ticks){
	...
	for(int p=0;p<Nu;p++){
		...
		if(MID!=0xFFFF){
			OneObject* OB=Group[MID];
			if(OB&&(OB->Hidden||!OB->Sdoxlo)&&(OB->Serial^SIDS[p])<2&&OB->Produce(UnitType->Index,DestGroup->Index)){
				Order1* Ord=OB->LocalOrder;
				while(Ord->NextOrder){
					Ord=Ord->NextOrder;
				}
...
char* GetZoneName(int Index){
	if(Index<AZones.GetAmount())return AZones[Index]->Name.str;
	else return NULL;
};
DLLEXPORT
int	  GetZoneXbyName(char* name){
	if( name ) {//grey
		for (int i=0; i<AZones.GetAmountOfElements(); i++){
			if (strcmp(AZones[i]->Name.str,name)==0) {
				return AZones[i]->x;
			}
		}
	}
...
char* GetGroupName(int Index){
	if(Index<AGroups.GetAmount())return AGroups[Index]->Name.str;
	else return NULL;
};
DLLEXPORT
...
int GetSearchRadius(int Index){
	...
	return OB->Ref.General->MoreCharacter->VisRange>>4;
}
DLLEXPORT
...
#ifdef _USE3D
	Matrix4D M;
	M4_srtpp(M,x,y,0,dx,dy,Angle,sx,sy);
...
OneObject* TransformUnitType(OneObject* OB,int DestType)
{
	if(OB&&OB->NIndex!=DestType){
		...
		if(OB->NewBuilding||OB->CObjIndex!=0xFFFF){
			...
			OB->GroundState=0;
			OB->SetZeroFrame();
			if(!OB->NewBuilding)OB->NewAnm=OB->newMons->GetAnimation(anm_Stand);
			//OB->OverEarth=0;
		}
	}
	return OB;
