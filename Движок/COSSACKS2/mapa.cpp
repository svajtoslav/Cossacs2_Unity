...
Order1* GetOrdBlock(){
	assert(OSC.Orders.GetAmount()<49999);
	int N;
...
void DrawMarker(OneObject* OB)
{
	...
	if(!(OB->NMask & NATIONS[GSets.CGame.cgi_NatRefTBL[MyNation]].NMask) )GPS.SetCurrentDiffuse(0xFFFF0000);
	NewMonster* NM=OB->newMons;
...
void DrawHealth(OneObject* OB)
{
	...
	//  main border 
	Vector3D lt = SkewPt( cx - sprW/2 + 5,	cy, ceilf( OB->RZ + OB->OverEarth + sprH ) );
	WorldToScreenSpace( lt );
	lt.x = roundf( lt.x );
	lt.y = roundf( lt.y );
	Vector3D lb( lt ); lb.y += 2;
	...
	GPS.DrawLine( lt.x, lt.y, lt.x, lb.y, 0xFFFFFFFF );
	...
	if(OB->NewBuilding)
	{
		...
		if(!IsEnemyUnit( OB ) || OB->WatchNInside)
		{
			...
			if(nn>OB->NInside){
				sprintf(cc,"%d+%d/%d",OB->NInside,nn-OB->NInside,OB->Ref.General->MoreCharacter->MaxInside+OB->AddInside);
			}else
			if(nn<OB->NInside){
				sprintf(cc,"%d-%d/%d",OB->NInside,OB->NInside-nn,OB->Ref.General->MoreCharacter->MaxInside+OB->AddInside);
			}else{
...
void ShowCentralText0(char* sss){
	...
	do{
		...
		if(c!='\\'&&c){
			...
			ShowString(x0-L,y0-9,ssp,&BigBlackFont);
			y0+=Dy;
			p0=0;
		};
	}while(c);
...
void ShowAllMarkers(){
	...
	for(i=0;i<MAXOBJECT;i++){
		...
		if(OB&&!OB->Sdoxlo&&!OB->Hidden&&OB->ActiveAbility&&OB->ActiveAbility->ActiveAbilities.GetAmount()>0){
			DrawAurasMarker(OB);
		}
	}
	// enemy under cursor
...
void ShowBuildingPreview(int BuildingID,int xscr,int yscr){
	...
	if(!NM->Building){
		//preview for unit must be there...
		return;
	}
	int xxx,yyy;
	if(GetRealCoords(xscr,yscr,&xxx,&yyy)){
		if(NM->SpriteObject&&NM->SpriteVisual!=0xFFFF){
			...
			if(GetRealCoords(xscr,yscr,&xxx,&yyy)){
				...
				R=CheckSmartCreationAbility(GSets.CGame.cgi_NatRefTBL[MyNation],NM,&xx3,&yy3)!=-1;
				if(R){
					if(MINESPRITEID!=-1){
...
void GFieldShow(){
	...
	if(CheckIfNewTerrain()){
        IRS->ClearDeviceTarget(0xFF306030);
	}
	static float z0=840;
	if(GetKeyState(VK_CONTROL)&0x8000){
    ...
    float effBias = tmin( EngSettings.EffectsZBias, g_ZNear - 10.0f );
    ICam->ShiftZ(effBias);
    IPMgr->Render();
	...
	GPS.SetCurrentShader( shNoShadow );
    ...
    GPS.SetCurrentShader( shShadow );
    IRS->SetCurrentShader( shShadow );		
    PRFUNC(FlushSprites( 1 ));

	//  draw unit shadows, z-write disabled
	static int shShadowZ = IRS->GetShaderID( "units_shadow" );
	GPS.SetCurrentShader( shShadowZ );
	IRS->SetCurrentShader( shShadowZ );
	FlushSprites( 2 );
    ...
    GPS.SetCurrentShader( shHud2sm );
    IRS->SetCurrentShader( shHud2sm );		
	...
	GPS.SetCurrentShader( shHud2 );
	IRS->SetCurrentShader( shHud2 );
	FlushSprites( 3 );
	FlushSprites( 6 );
    ...
    IRS->SetViewPort( 0, 0, RealLx, RealLy );
	...
	/*
	// мигающий домик при нехватке жилых мест
	extern bool HideBorderMode;
	if(!(PlayGameMode==1||HideBorderMode||(SCENINF.hLib&&!CanProduce)||EditMapMode)){
		int NG=NATIONS[GSets.CGame.cgi_NatRefTBL[MyNation]].NGidot;
		int NF=NATIONS[GSets.CGame.cgi_NatRefTBL[MyNation]].NFarms;
		if(NF<=NG){
	...
	*/
					...
					for(int q=0;q<no;q++){
						...
						if(!(ms&prevms)){
							...
							if(w>1){
								...
								case 1:
									sprintf(cc,"%d",SumSquare[GSets.CGame.cgi_NatRefTBL[ord[q]]]);
									break;
								default:
									sprintf(cc,"%.2f",float(SumAccount[GSets.CGame.cgi_NatRefTBL[ord[q]]])/100);
								};
						...
						_str n;
						n=GetPName(ord[q]);
						LimitString(n.str,&SmallBlackFont,50);
						//
						sprintf(cc1,"%s (%s)",n.str,ccr);
	...
	if(!(NOPAUSE||LockPause||EnterChatMode)){
		...
		//ShowString(x,y-2,PAUSE,&BigBlackFont);
		ShowString(x-2,y-4-6,PAUSE,&BigBlackFont); //BigWhiteFont
	};
	...
	if(rules.str){	
		ShowString(smapx,ssy,rules.str,&RedFont);
		ssy+=20;
	}
	...
	//if(NSL[MyNation]){
		...
		if(MID!=0xFFFF){
			...
			if(OB){
				...
				sprintf(ggg,"%s : ID=%d  x=%d  y=%d dir=%d",OB->Ref.General->Message,OB->Index,OB->RealX>>4,OB->RealY>>4,OB->RealDir);
				ShowString(smapx,ssy,ggg,&WhiteFont);
				ssy+=16;
...
void RenderMinimap(){
	...
	if(MiniMapUTexture==0){
		TextureDescr td;
		td.setValues(128,128,cfARGB4444,mpManaged,1,tuProcedural);
		MiniMapUTexture=IRS->CreateTexture("FogMinimapTexture",td);
	}
	int TC=GetTickCount();
	...
	if(TC-T0>300){
		...
		if(MMU){
			...
			for(int q=0;q<8;q++){
				...
				C16   [q]=DWORD2WORD(C);
				C16p  [q]=DWORD2WORD(MulDWORD(C,290));
				C16m  [q]=DWORD2WORD(MulDWORD(C,220));
				C16mm [q]=DWORD2WORD(MulDWORD(C,180));
			}
            //setting units
			OneObject* OO;
			word Seen[1024];
...
lopx2:				add		edi,2
			...
			for(int g=0;g<MAXOBJECT;g+=st){
				...
				if(OO&&!(OO->Sdoxlo||OO->Invisible||OO->newMons->InvisibleOnMinimap||(OO->InvisibleForEnemy&&(!OO->NMask&MyMask)))){
					...
					int mxx=(OO->RealX>>(10+ADDSH));
					int myy=(OO->RealY>>(10+ADDSH))-(OO->RZ>>6);
					if(mxx>=2&&myy>=2&&mxx<120&&myy<120){
			...
			for(int i=0;i<NSeen;i++){
				...
				if(OO&&!OO->Sdoxlo){			
					int mxx=(OO->RealX>>(10+ADDSH));
					int myy=(OO->RealY>>(10+ADDSH))-(OO->RZ>>6);
					if(mxx>0&&myy>0&&mxx<120&&myy<120){
						word val=C16[OO->NNUM];						
						if(OO->ImSelected&mmsk){
							val=0xFFF0;
							if(OO->BrigadeID!=0xFFFF){
	...
	extern int ExtraDir;
	DrawTexturedBar(minix,miniy,MiniLx,MiniLy,MinMapX/sc,MinMapY/sc,float(MaxMapX-MinMapX)/sc,float(MaxMapY-MinMapY)/sc,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,GetMinimapTexture(),shs,-ExtraDir);
	DrawTexturedBar(minix,miniy,MiniLx,MiniLy,MinMapX/sc,MinMapY/sc,float(MaxMapX-MinMapX)/sc,float(MaxMapY-MinMapY)/sc,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,MiniMapUTexture,shs,-ExtraDir);
	if(!EngSettings.DontUseRoads)DrawTexturedBar(minix,miniy,MiniLx,MiniLy,MinMapX/sc,MinMapY/sc,float(MaxMapX-MinMapX)/sc,float(MaxMapY-MinMapY)/sc,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,GetMinimapRoadsTexture(),shs,-ExtraDir);    
}
...
void LimitCamera()
{
	if(fMapX<MinMapX)fMapX=MinMapX;
	if(fMapY<MinMapY)fMapY=MinMapY;
	if(fMapX+smaplx>MaxMapX)fMapX=MaxMapX-smaplx;
	if(fMapY+smaply>MaxMapY)fMapY=MaxMapY-smaply;
	/*
    if(fMapX<MinScrollX)fMapX=MinScrollX;
    if(fMapY<MinScrollY)fMapY=MinScrollY;
    if(fMapX+smaplx>MaxScrollX)fMapX=MaxScrollX-smaplx;
    if(fMapY+smaply>MaxScrollY)fMapY=MaxScrollY-smaply;
	*/
}
...
{	
	...
	*/
	...
	int xmx,ymy;
	extern bool MiniMapVisible;
	if(MiniMapVisible && MiniLx&&MiniLx&&x>=minix&&y>=miniy&&x<minix+(msx>>1)&&y<miniy+(msy>>1)&&!(LockMouse||WasSelectionBar)){
	...
	if(Rpressed&&ItemPickUp(Att!=0xFFFF?Att:Friend)){
		Rpressed=false;
		return;
	}
	...
	if(v_Targ_MA){
		...
		if(SP&&SP->ActAbl==v_Targ_MA){
			//UnitAbility* UA=v_Targ_MA->AbilitiesList[v_Targ_MA_id];
			ActiveUnitAbility* UA=NULL;
			if(v_Targ_MA_id!=-1){
				UA=v_Targ_MA->ActiveAbilities[v_Targ_MA_id];
			}else{
			...
			if(UA){
				...
				bool CanPress=UA->CanTarget(TID,LastMx,LastMy,0);
				if(!UA->DrawCursor(LastMx,LastMy)){
					if(CanPress){
						VitCursor=15;
					}else{
						VitCursor=14;
				...
				if(Lpressed){
					if(CanPress){
						...
						IS.Data.AblID=v_Targ_MA_id!=-1?v_Targ_MA_id:v_Targ_MA_id2;
						IS.Data.TargOB=TID;					
	...
	if(cooren&&mini&&Lpressed&&GetKeyState(VK_CONTROL)&0x8000){
		...
		UnPress();
		char chmess[128];
		sprintf(chmess,"@@@spot %d %d",xmx,yreal);
	...
	if(SpecCmd==112){
		...
		if(LastActionX!=0){
            SetCameraPos((LastActionX>>5)-(smaplx>>1),(LastActionY>>5)-(smaply>>1)-(GetHeight(LastActionX,LastActionY)>>(Shifter-1)));
		};
	};
	...
	if(cooren){
		...
		if(Lpressed&&!(mini)){
			...
			if(LockMode==2){
				AddLockRound(xmx>>6,yreal>>6,ReliefBrush/4);
				goto Edgetest;
			};
			if(LockMode==3){
			...
			if(BuildMode){
				...
				int x0=xmx<<4;
				int y0=yreal<<4;
				if(CheckSmartCreationAbility(GSets.CGame.cgi_NatRefTBL[MyNation],NATIONS[MyNation].Mon[BuildingID]->newMons,&x0,&y0)!=-1){;
	...
	if(cooren){
		...
		if(mini){
			...
			if(Rpressed){ //GetKeyState(VK_CONTROL)&0x8000
				...
				if(Nsel){
					...
					OneObject* OB=Group[MID];
					if(MID!=0xFFFF&&OB&&OB->NewBuilding&&OB->NNUM==GSets.CGame.cgi_NatRefTBL[MyNation]){
		...
		if(Lpressed&&!mini){
			...
			if(!(DrawPixMode||DrawGroundMode)){
				...
				Vector3D VS = SkewPt(StartX,StartY,max(Mode3D?GetTotalHeight(StartX,StartY):0,0));
		...
		if(!(mini||( VitCursor!=-1 && !StrelMode ))){
			...
			if(Att!=0xFFFF&&Entr==0xFFFF&&!GoAndAttackMode){
				...
				if(AOB){
					...
					if(MTMASK||CanAttBLD||CanCapt){				
						...
						if(AOB->MathMask&MTMASK)SetCurPtr(AttackPtr);
						else{
							if(AOB->NewBuilding||AOB->Wall||AOB->LockType){
								if(CanAttBLD){
									SetCurPtr(AttGrPtr); //AttackPtr
									//VitCursor=-1;
								}
							}
						};
			...
			if(curptr!=GoToMinePtr&&curptr!=RepairPtr&&CanEnter&&Friend!=0xFFFF){
				OneObject* FR=Group[Friend];
				if(FR&&(((FR->NewBuilding)&&FR->Stage>=FR->Ref.General->MoreCharacter->ProduceStages)||(!FR->NewBuilding))&&!FR->Sdoxlo){
					if(FR->NNUM!=7 && (FR->Ready||FR->LocalOrder)&&FR->Ref.General->MoreCharacter->MaxInside+FR->AddInside>FR->NInside){
			...
			if(Rpressed){
				...
				if(Nsel){
					int MID=Selm[MyNation][0];
					OneObject* OB=Group[MID];
					if(curptr!=AttackPtr&&MID!=0xFFFF&&OB&&OB->NewBuilding&&OB->NNUM==GSets.CGame.cgi_NatRefTBL[MyNation]){
						CmdSetDst(MyNation,xmx<<4,yreal<<4);
						dest=true;
						StrelMode=false;
					}
...
void EDGETEST(){
	...
	if(SHOWSLIDE){;
		...
		int OLDMX=mapx;
		int OLDMY=mapy;
		if(!(EnterChatMode||RESMODE||LockManualScroll)){
			OneObject* SOB=NULL;
	...
	/*	if (IsPerspCameraMode())
		...
		if(OB){
			...
			SetCameraPosEx( 
				(OB->RealX/32-smaplx*16)/32.0f ,
				(OB->RealY/32-smaply*16 - GetTotalHeight( OB->RealX/16 , OB->RealY/16 ) )/32.0f );
		}
		...
		if((!(LockMouse||WasSelectionBar))&&(Lpressed)&&(mouseX>minix)&&(mouseY>miniy)&&
			...
			SetCameraPos(LastMx/32-smaplx/2,LastMy/32-smaply/2);
...
void GoToCurSelPosition(){
	if(ImNSL[MyNation]){
		...
		if(OBJ){
			SetCameraPos((OBJ->RealX>>9)-(smaplx>>1),(OBJ->RealY>>9)-(smaply>>1)-(OBJ->RZ>>4));			
			...
			for(int i=0;i<NU;i+=st){
				...
				if(MID!=0xFFFF){
					...
					if(OB&&OB->Serial==SNS[i]){
                        ...
                        int R=OB->Ref.General->MoreCharacter->MaxR_Attack*92/100;
			...
			for(int i=0;i<NU;i+=st){
				...
				if(MID!=0xFFFF){
					...
					if(OB&&OB->Serial==SNS[i]){
						int udx=OB->RealX/16-xc;
						int udy=OB->RealY/16-yc;
						int R=OB->Ref.General->MoreCharacter->AttackRadius2[AttType];
						int RR1=OB->newMons->VisibleRadius1;
						int RR2=OB->newMons->VisibleRadius2;
						if(R>150){
							int x0=OB->RealX/16;
...
void TestCGARB(){
	//try{
	int amt = CPGARB.CPointer.GetAmount();
		for(int i=0;i<amt;i++){
			OneClassPointer* CP=CPGARB.CPointer[i];
