...
public:
		...
		if(p!=LeftAlign){
			switch(p){
				...
				case 1://AbsoluteRightAlign
					if(ParentDS)LA_param=ParentDS->GetWidth()-x;
					break;
				case 2://RelativeLeftAlign
					if(ParentDS&&ParentDS->x1!=ParentDS->x)LA_param=float(x)/float(ParentDS->GetWidth());
					break;				
			}
		}
		LeftAlign=p;
		...
		if(p!=RightAlign){
			switch(p){
				case 0://AbsoluteLeftAlign
					break;
				case 1://AbsoluteRightAlign
					if(ParentDS)RA_param=ParentDS->GetWidth()-x1;
					break;
				case 2://RelativeLeftAlign
					if(ParentDS&&ParentDS->x1!=ParentDS->x)RA_param=float(x1)/float(ParentDS->GetWidth());
					break;				
			}
		}
		RightAlign=p;
	...
	void SetHCenterAlign(int p){
		if(x!=HorizontalCenterAlign){
			switch(p){
				case 0://AbsoluteLeftAlign
					break;
				case 1://AbsoluteRightAlign
					if(ParentDS)HCA_param=ParentDS->GetWidth()-(x+x1)/2;
					break;
				case 2://RelativeLeftAlign
					if(ParentDS&&ParentDS->x1!=ParentDS->x)HCA_param=float(x1+x)/2.0f/float(ParentDS->GetWidth());
					break;				
			}
		}
		...
		if(p!=TopAlign){
			switch(p){
				...
				case 1://AbsoluteBottomAlign
					if(ParentDS)TA_param=ParentDS->GetHeight()-y;
					break;
				case 2://RelativeAlign
					if(ParentDS&&ParentDS->y1!=ParentDS->y)TA_param=float(y)/float(ParentDS->GetHeight());
					break;				
			}
		}
        TopAlign=p;
		...
		if(p!=BottomAlign){
			switch(p){
				case 0://AbsoluteTopAlign
					break;
				case 1://AbsoluteBottomAlign
					if(ParentDS)BA_param=ParentDS->GetHeight()-y1;
					break;
				case 2://RelativeAlign
					if(ParentDS&&ParentDS->y1!=ParentDS->y)BA_param=float(y1)/float(ParentDS->GetHeight());
					break;				
			}
		}
		BottomAlign=p;
		...
		if(p!=VerticalCenterAlign){
			switch(p){
				case 0://AbsoluteLeftAlign
					break;
				case 1://AbsoluteRightAlign
					if(ParentDS)VCA_param=ParentDS->GetHeight()-(y+y1)/2;
					break;
				case 2://RelativeLeftAlign
					if(ParentDS&&ParentDS->y1!=ParentDS->y)VCA_param=float(y1+y)/2.0f/float(ParentDS->GetHeight());
					break;				
			}
		}
	...
	//////////////////////////////////////////////////////////////////////////	
	inline virtual int GetWidth(){return x1-x+1;}
...
public:
	...
	virtual void CheckMouseOver(int mx,int my){
		if(x<=mx&&y<=my&&x1>=mx&&y1>=my&&(ParentFrame::ActiveParent==this||!ParentFrame::ActiveParent)){
			if(ShapeFileID!=0xFFFF){
				DWORD alpha = GPS.GetGPAlpha( ShapeFileID, ShapeSpriteID, 0, mx-x, my-y, true );
                MouseOver = (alpha > 0x8);
			}else MouseOver=1;
            MouseRelX=mx-x;
			MouseRelY=my-y;
...
public:
	...
	void* GetFileName(){
		//
		return (void*)&PassivePicture->Name.str;
	}
...
public:
	...
	const char* GetThisElementView(const char* LocalName){	
		return Name.str?Name.str:LocalName;
	}	
	SAVE(StdBorder){
...
public:
	virtual DWORD GetValue(const char* ID){
		for(int i=0;i<BScope.Borders.GetAmount();i++){
			char* s=BScope.Borders[i]->Name.str;
	...
	virtual char* GetValue(DWORD ID){
		if(ID<BScope.Borders.GetAmount()){
			return BScope.Borders[ID]->Name.str;
		}
		return "";
	}
...
public:
	/*
	ClassPtr<SimpleDialog> Top;
	int TopDy;
	ClassPtr<SimpleDialog> Bottom;
	int BottomDy;
	ClassArray<SimpleDialog> Buttons;
	int ButtonsDx;
	int ButtonsDy;
	*/
	~DialogsDesk();
	...
	SAVE(DialogsDesk)
		...
		/*
		REG_AUTO(Top);
		REG_MEMBER(_int,TopDy);
		REG_AUTO(Bottom);
		REG_MEMBER(_int,BottomDy);
		REG_AUTO(Buttons);
		REG_MEMBER(_int,ButtonsDx);
		REG_MEMBER(_int,ButtonsDy);
		*/
...
public:
	...
	SAVE(ProgressBar);	
		...
		REG_ENUM  (_index,ProgressType,ProgressBarType);
...
public:
	//bool ForceProperty() { return true; }
};
...
public:
	...
	const char* GetThisElementView(const char*){
		...
		sprintf(s,"{I%s}%s",Name.str,Name.str);
		return s;
	}	
	DWORD GetClassMask(){
...
public:
	...
	OneTextIcon* get(char* s){
		for(int i=0;i<Icons.GetAmount();i++)if(Icons[i]->Name.str){
			if(!strcmp(Icons[i]->Name.str,s))return Icons[i];
		}
		return NULL;
	}
	SAVE(TextIcons);
...
public:
	...
	OneFontParam* GetByName(char* Name){
		for(int i=0;i<Fonts.GetAmount();i++)if(Fonts[i]->Name.str){
			if(!strcmp(Fonts[i]->Name.str,Name))return Fonts[i];
		}
		return NULL;
	}
	SAVE(FontParam){
