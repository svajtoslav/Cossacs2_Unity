...
void va_EW2_Mis_List::SetFrameState(SimpleDialog* SD){
	...
	if(LD&&CurPlayer){
		...
		if( CurEW2miss>=0 && CurEW2miss<EW2_Missions.GetAmount() ){			
			...
			//SetMapPreviewJpg(CM->MapPreview.str,CM->MapCenterX,CM->MapCenterY);
...
bool va_EW2_Mis_Start::LeftClick(SimpleDialog* SD){
	if(CurEW2miss<EW2_Missions.GetAmount()&&!vls_Active){
		...
		strcpy(g->cgi_CurrentMap,EW2_Missions[CurEW2miss]->FileM3D.str);	
		// name correct
		strcpy(g->PL_INFO[0].name,PlayerName.str);
		for(int i=1;i<8;i++){
