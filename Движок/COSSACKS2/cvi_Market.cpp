...
cvi_Market* v_addMarket(DialogsSystem* Parent, int x, int y, vtf_GetResource* Get, vtf_AddResource* Add){
	...
	if(M->mDialog->SafeReadFromFile(M->mDialogName.str)){
...
SimpleDialog* cvi_Market::Init(){
	if(!mInit){
		...
		if(!mDialog->SafeReadFromFile(mDialogName.str)){
			delete mDialog;
			mDialog=NULL;
		}
		return mDialog;
...
void cvi_M_SellSelect::SetFrameState(SimpleDialog* SD){
	//SD->Visible=false;
	if(vMarket->mDialog){
		int ResID=GetResID(SD->Name.str);
		if(ResID>=0){
...
bool cvi_M_SellSelect::LeftClick(SimpleDialog* SD){
	if(vMarket->mDialog){		
		vMarket->SetResSell(GetResID(SD->Name.str));
	}
	return true;
}
//
...
bool cvi_M_BuySelect::LeftClick(SimpleDialog* SD){
	if(vMarket->mDialog){
		vMarket->SetResBuy(GetResID(SD->Name.str));
	}
	return true;
}
//
