...
bool CBR_OnUserClick(SimpleDialog* SD){	
	...
	if(EP->BC->CheckDirectCasting()){
		...
		if(!(GetKeyState(VK_CONTROL)&0x8000)) EP->Owner->ExpInf.Clear();
...
void ClassEditor::CreateFromClass(ParentFrame* Dest,int x,int y,int x1,int y1,BaseClass* BC,DWORD Opt1,const char* Border){
	...
	if(E&&E->GetAmount()>=2){//load,save		
		...
		*LL=*LS;
		...
		BS->AllocPtr=(char*)LS;
		GP_Button* BL=Base->addGP_Button(NULL,x+26,0,bidx,1,0);
		BL->ShapeFileID=bidx;
		BL->ShapeSpriteID=1;
		ColoredBar* CB=Base->addColoredBar(x+26*2,0,x1-x-26*2,21,0xFF);
...
bool CE_LoadClick(SimpleDialog* SD){
	...
	if(ProcessLoadSave(x0,y0,LS->CE->_x1-LS->CE->_x+1,LS->CE->_y1-LS->CE->_y+1,str,LS->Mask,src,1,P00)){
		...
		LS->BC->reset_class(LS->DataPtr);
		ErrorPager Err;
		if(LS->BC->CheckDirectCasting()&&((BaseClass*)LS->DataPtr)->SafeReadFromFile(str.str)){
			...
			if(LS->BC->GetElementByName("SourceFile",&SF_BaseClass,&SF_DataPtr,&SF_ExtraPtr,LS->DataPtr)){
				if(!strcmp(SF_BaseClass->GetClassName(),"_str")){
					((_str*)SF_DataPtr)->Assign(str.str);
				}
			}			
		}
...
bool CE_SaveClick(SimpleDialog* SD){
	...
	if(LS->BC->CheckDirectCasting()&&ProcessLoadSave(x0,y0,LS->CE->_x1-LS->CE->_x+1,LS->CE->_y1-LS->CE->_y+1,str,LS->Mask,src,0,P00)){
		((BaseClass*)LS->DataPtr)->WriteToFile(str.str);
	}
	return true;
}
struct LS_MoveParam{
...
int ClassEditor::CreateElement(ParentFrame* Base,int Level,DWORD Mask,int x,int y0,int l_lx,int r_lx,
	...
	char* ElmView=(char*)BC->GetEditableElementView(i,BC,eview.str,Opt);
	...
	if(!(GetKeyState(VK_MENU)&0x8000)){		
		...
		*/
		if(ElmBC&&ElmBC->CheckDirectCasting()){
			...
			Enumerator* E=ENUM.Get(cc);
			if(Level>0&&((NE==1&&ExpList.ExpList.GetAmount()==0&&OED==NULL&&E->GetAmount()<2&&!(strchr(eview.str,'{')||BC->GetExpansionRules()&3))||ElmBC->ForceSimplification())){
	...
	if(ehint.str&&ehint.str[0]){
		CB0->Hint=znew(char,strlen(ehint.str)+1);
		strcpy(CB0->Hint,ehint.str);
		CB0->AllocHint=1;
		CB->Hint=znew(char,strlen(ehint.str)+1);
		strcpy(CB->Hint,ehint.str);		
		CB->AllocHint=1;
	}
	ControlParams CP;
	...
	CP.MinLeftWidth=GetRLCStrWidth(eview.str,&CED_Font)+8;
	...
	if(OED){
		...
		/*if(eview.str){
			...
			int yy2=y0+GetRHeight(eview.str,CP.TextFont,)+4;
			yy=max(yy2,yy);
		}*/
	}
	...
	if(eview.str){
		...
		int yy2=y00+GetRHeight(eview.str,CP.TextFont,maxwid)+CED_TextDy+4;
		y0=max(yy2,y0);
		yy=y0;
	}
	...
	if(ExpList.ExpList.GetAmount()){//if there is something to expand
		...
		if(ExpList.TopicsList.GetAmount()){			
			Tops = Base->addGP_ComboBoxDLX(NULL,l_lx+2,y00,r_lx-BTLX,CBB_GPFILE,0,9,0,&CED_AFont,&CED_Font,NULL);
			VScrollBar* VS1;
			Tops->AssignScroll((DialogsSystem*)Base,&VS1,3,0,8);

			CBB  = Base->addGP_ComboBoxDLX(NULL,l_lx+2,y00+CED_LineDist,r_lx-BTLX,CBB_GPFILE,0,9,0,&CED_AFont,&CED_Font,NULL);
			VScrollBar* VS2;
			CBB->AssignScroll((DialogsSystem*)Base,&VS2,3,0,8);

			y0=max(y00+2*CED_LineDist,y0);
			...
			CBB  = Base->addGP_ComboBoxDLX(NULL,l_lx+2,y00,r_lx-BTLX,CBB_GPFILE,0,9,0,&CED_AFont,&CED_Font,NULL);
			VScrollBar* VS;
			CBB->AssignScroll((DialogsSystem*)Base,&VS,3,0,8);

			y0=max(y00+CED_LineDist,y0);
			for(int j=0;j<ExpList.ExpList.GetAmount();j++){				
				CBB->AddLine(ExpList.ExpList[j]);
				CAAP->Classes.Add(ExpList.ExpList[j]);
			}
	...
	//adding text field
	if(!CP.HideText){
		char* el=EPR?EPR->FieldName:eview.str;
		char cc[256];
		sprintf(cc,"$cid_%s",el);
		char* e1=GetTextByID(cc);
		if(e1[0]=='$')e1=el;
		if(CP.VerticalText){
...
DWORD GetXML_Hash(xmlQuote& xml){
	...
	//	H+=GetXML_Hash(*(xml.GetSubQuote(i)));
	//}	
	return H;
}
	...
	if(LastKey=='C'&&(GetKeyState(VK_CONTROL)&0x8000)){//copy
		if(CurElement!=-1){
			...
			if(ELP->BaseParent){
				...
				xmlQuote* xml2=new xmlQuote((char*)(ELP->BC)->GetClassName());
				...
				if(OCS->Members.GetAmount()){
					...
					if(OCMS->GeneralPropertyR){
						...
						if(HV!=ELP->XMLHash){
							...
							ELP->XMLHash=OEE->GetHashValue(ELP->BC,ELP->DataPtr,ELP->ExtraPtr);
							if(r==2){
								ExpInfChanged=1;
							}
						}						
...
const char* ClassEditor::GetCurrentFieldName(){
	if(CurElement!=-1){
		return ElParams[CurElement]->FieldName;
	}else return NULL;
}
...
void DrawCircle2(int x,int y,int r,int r2,DWORD Color){
	int Np=r*20;
	int xp,yp;
	for(int i=0;i<=Np;i++){
		float a=3.1415f*2.0f*i/Np;
...
bool ProcessLoadSaveOld(int x,int y,int Lx,int Ly,_str& str,const char* mask,const char* StartDir,bool Load,ParentFrame* Back){	
	...
	do{
		...
		if(bActive){
			...
			if(FPV){
				TPZ.CheckName();
				FPV->DrawPreview(x+1,y+Ly-PR_Ly+10,x+Lx-1,y+Ly-1,TPZ.FILEPATH.str);
			}
			GDS.RefreshView();		
			if(ItemChoose==1||LastKey==13){//load/save
				TPZ.CheckName();
				str=TPZ.FILEPATH.str;
				if(FPV)delete(FPV);
				LastKey=0;
				ItemChoose=-1;
				LastKey=0;
...
int _byte_index_editor::CreateControl(ParentFrame* Base,int x,int y,int x1,int y1,
	...
	static int CBB_GPFILE=GPS.PreLoadGPImage("Interf3\\elements\\Combo");
	Box=Base->addGP_ComboBoxDLX(NULL,x,y,x1-x+1,CBB_GPFILE,0,9,0,&CED_AFont,&CED_Font,NULL);
	VScrollBar* VS;
...
int _index_editor::CreateControl(ParentFrame* Base,int x,int y,int x1,int y1,
	...
	if(ExtraPtr){
		...
		if(ctg){
			Topics=Base->addGP_ComboBoxDLX(NULL,x,y,x1-x+1,CBB_GPFILE,0,9,0,&CED_AFont,&CED_Font,NULL);
			VScrollBar* VSC;
			Topics->AssignScroll((DialogsSystem*)Base,&VSC,3,0,8);							

			Box=Base->addGP_ComboBoxDLX(NULL,x,Topics->y1,x1-x+1,CBB_GPFILE,0,9,0,&CED_AFont,&CED_Font,NULL);
			VScrollBar* VS;
			Box->AssignScroll((DialogsSystem*)Base,&VS,3,0,8);				
			Box->AutoID=E->EnumeratorID;
...
int _index_editor::Get(xmlQuote* xml){	
	if(Topics&&Topics->Changed){
		char* s=Topics->Lines[Topics->CurLine];		
		...
		if (ElemArray!=NULL){
			...
			for(int i=0;(i<N)&&(ElemID==-1);i++){
				...
				if(ClassType->GetElementByName(ElemName,&E_Class,&E_Data,&E_Extra,ClassType)){
					...
					if (s!=NULL && !strcmp( s, CI->str )) ElemID=i;
				}
...
DWORD _ClassIndex_editor::GetHashValue	(BaseClass* BC,void* Ptr,void* ExtraPtr){
	_ClassIndex* CI=(_ClassIndex*)Ptr;
	return CI->str?0:GetSHash(CI->str);
}
...
int ClassRef_editor::CreateControl(ParentFrame* Base,int x,int y,int x1,int y1,
	...
	static int CBB_GPFILE=GPS.PreLoadGPImage("Interf3\\elements\\Combo");
	List=Base->addGP_ComboBoxDLX(NULL,x,y,x1-x+1,CBB_GPFILE,0,9,0,&CED_AFont,&CED_Font,NULL);
	VScrollBar* VS;
	...
	if(ROOT){
		for(int i=0;i<CPGARB.CPointer.GetAmount();i++){
			OneClassPointer* CP=CPGARB.CPointer[i];
			if(CP->ClassPtr&&CP->NamePtr&&CP->NamePtr->str){
				...
				if(OCS){
					...
					if(!unf){
						...
						if(CP->ClassPtr->CheckIfObjectIsGlobal()||R1==ROOT){
							if(CP->NamePtr->str&&CP->NamePtr->str[0]){
								List->AddLine(CP->NamePtr->str);
								PIDS.Add(CP->Index);
								if(CP->ClassPtr==P->Get()){
									List->CurLine=List->NLines-1;
								}
							}
						}
					}
	...
	if(Top.CurElement!=-1){
		...
		if(ptr!=PrevSelPtr){
			...
			if(Top.ElParams[Top.CurElement]&&Top.ElParams[Top.CurElement]->BC&&Top.ElParams[Top.CurElement]->BC->CheckDirectCasting()){
				...
				Bottom.Base=NULL;
				Bottom.ClearAll();				
				Bottom.CreateFromClass(&BottomDS,Bottom._x,Bottom._y,Bottom._x1,Bottom._y1,(BaseClass*)ptr,1,border.str);
			}
		}
	}
...
bool FileElement::Load(xmlQuote& xml,void* ClassPtr,ErrorPager* Error,void* Extra)
{
	if(OnClickSet)
	{
		OnClickSet->Clear();
		OnClickSet->Add(Way);
		OnClickSet->Add(this->str);
		xml.Assign_string(OnClickSet->str);
	}
	return true;
}
...
void DirectoryElement::ReadDir()
{
	...
	TMD.Add(TM);
	TMD.Add("*");
	hFindFile=FindFirstFile(TMD.str,&FindData);
	...
	//read files
	TM.Add(Mask);
	hFindFile=FindFirstFile(TM.str,&FindData);
	if(hFindFile!=INVALID_HANDLE_VALUE)
	{
		ReadFile(&FindData);
		while(FindNextFile(hFindFile,&FindData))
...
int DirectoryElement::compare(const void* p1, const void* p2)
{
	...
	if((*fe1)&&(*fe2))
	{
		...
		{
			const char* n1=(*fe1)->str;
			const char* n2=(*fe2)->str;
...
void DirectoryElement::ReadFile(WIN32_FIND_DATA* FD)
{
	if(strcmp(FD->cFileName,".")&&strcmp(FD->cFileName,".."))
	{
		...
		{
			FileArr.Add(new FileElement(FD->cFileName, Way.str,OnClickSet));
		}
	}
}
void DirectoryElement::ReadDir(WIN32_FIND_DATA* FD)
{
	if(strcmp(FD->cFileName,".")&&strcmp(FD->cFileName,".."))
	{
		if(FD->dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY)
		{
			...
			NP.Add(FD->cFileName);
			NP.Add("\\");
			FileArr.Add(new DirectoryElement(FD->cFileName, Mask.str, NP.str,OnClickSet));
		}
	}
}
...
void testFilePathEditor::CheckName(){
	strupr(FILEPATH.Way.str);
	strupr(FILEPATH.str);
	strupr(FILEPATH.Mask.str);
	if(!strstr(FILEPATH.str,FILEPATH.Way.str)){
		_str s1;
		s1=FILEPATH.Way.str;
		s1+=FILEPATH.str;
		FILEPATH.Assign(s1.str);			
	}		
	if(!strstr(FILEPATH.str,FILEPATH.Mask.str+1)){
		FILEPATH+=FILEPATH.Mask.str+1;
	}
}
...
int _picfile_editor::CreateControl(ParentFrame* Base,int x,int y,int x1,int y1,
	...
	if(Data->str) strcpy(str,Data->str); else str[0]=0;
	//
...
bool _ModelID_editor::ModelDraw(SimpleDialog* SD){
	...
	if(*ME->model_id>0){
		...
		try{
			IMM->BeginThumbnail(Rct(SD->x,SD->y+25,SD->x1-SD->x,SD->y1-SD->y-25));
			GetGameLight()->Render();
			IMM->Render(*ME->model_id);
			IMM->EndThumbnail();
		}catch(...){};
...
bool _strUserClick(SimpleDialog* SD){
	...
	_str str;
	str=slp->dest;
	if(ProcessLoadSave(slp->x,slp->y,slp->Lx,slp->Ly,str,slp->mask.str,slp->dest,true)){
		strcpy(slp->dest,str.str);
	};
	return true;
}
int _textid_editor::CreateControl(ParentFrame* Base,int x,int y,int x1,int y1,
	...
	//Data=(char*)DataPtr;
	if(ExtraPtr){
		IB=Base->addInputBox(NULL,x,y,str,120,x1-x+1-26,y1-y+1,&CED_Font,&CED_AFont);
		static int bidx=GPS.PreLoadGPImage("Interf3\\btnz");
...
bool MapRef_editor::_DrawPalette(SimpleDialog* SD){
	...
	if(MRE->Meshes.GetAmount()&&SD->Enabled){
		void DrawCircle(int x,int y,int r,DWORD Color);
		BaseMeshDialog* BM=MRE->Meshes[MRE->Meshes.GetAmount()-1];
		ShowStringEx(SD->x,SD->y-20/*+BM->y-BM->y1*/,Map->Desc.str,&SmallWhiteFont);
		if(BM->MouseOver){
			int lx,ly;
			Map->GetSize(lx,ly);
			int x=mouseX;
			...
			DrawCircle2(x,y,r,r2,0x80FFFFFF);
			DrawCircle2(x,y,r+dr,r22,0x80FFFFFF);
			PopWindow(&TW);
		}
	}
	int NP=MRE->Meshes.GetAmount();
	if(NP){
		if(NP>256)NP=256;
		int xi[257];
		for(int i=0;i<=NP;i++){
		...
		if(SD->MouseOver){
			if(Lpressed){
				MRE->MRef->TopLayerIndex=SD->MouseRelX*NP/(SD->x1-SD->x);
...
int MapRef_editor::CreateControl(ParentFrame* Base,int x,int y,int x1,int y1,
	...
	for(int i=0;i<N;i++){
		...
		Meshes.Add(BM);
		if(i==N-1)BM->CreateSquareTnL(0,0,1,1,"LayersEditor",0xA0FFFFFF);
		...
		if(BM->MouseOver){
			...
			if(BM->MouseRelX!=prevX||BM->MouseRelY!=prevY){
				...
				if(realLpressed&&!ParentFrame::ActiveParent){							
					int lx,ly;
					Map->GetSize(lx,ly);
					int x=BM->MouseRelX*lx/(BM->x1-BM->x);
					int y=BM->MouseRelY*ly/(BM->y1-BM->y);
					int dx=0;
					int dy=0;
					if(prevpress){
						dx=(BM->MouseRelX-prevX)*lx/(BM->x1-BM->x);
						dy=(BM->MouseRelY-prevY)*ly/(BM->y1-BM->y);
					}
		...
		if(B){
			...
			for(int iy=0;iy<Ly;iy++){
				...
				for(int ix=0;ix<Lx;ix++){
					...
					*((DWORD*)(B+ofs))=C;//0xFF000000+(V<<16)+(V<<8)+V;
					ofs+=TEXPIXSIZE;
				}
			}
