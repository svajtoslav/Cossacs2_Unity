...
void LoadTilesEx(ResFile f1){
	...
	SetTexturesShadowInSquare(0,0,8192<<ADDSH,8192<<ADDSH);
};
#endif //_USE3D
...
void SaveUnits3(ResFile f1){
	...
	for(i=0;i<ULIMIT;i++){
		...
		if(OB&&!(OB->Wall||OB->Sdoxlo||OB->NotSavable)){
			...
			RBlockWrite(f1,&ry,4);
			RBlockWrite(f1,&OB->Life,2);
			word ST=OB->Ref.General->MoreCharacter->ProduceStages==OB->Stage?OB->Stage:0xFFFF-OB->Stage;
			RBlockWrite(f1,&ST,2);
			RBlockWrite(f1,&OB->WallX,2);
			RBlockWrite(f1,&OB->WallY,2);
...
void SaveZonesAndGroups(ResFile f1){
	...
	int sz1=strlen(D1.str)+1;
	int sz2=strlen(D2.str)+1;
	int sz=sz1+sz2+8+4;
	RBlockWrite(f1,&sz,4);
	RBlockWrite(f1,&sz1,4);
	RBlockWrite(f1,D1.str,sz1);
	RBlockWrite(f1,&sz2,4);
	RBlockWrite(f1,D2.str,sz2);	
};
...
void Save3DMap(char* Map){
	...
	if(!Prv->UserPicture.isClear()){
		SaveSomething(f1,Prv->UserPicture.str,Prv->Temp.str);
	}	
	SavePreview(f1);
...
void ProcessMapAutosave(){
	...
	if(GetTickCount()-ME_prevtime>300000){
		RenameAnyway("MapAutosave.40min.m3d","MapAutosave.45min.m3d");
		RenameAnyway("MapAutosave.35min.m3d","MapAutosave.40min.m3d");
		RenameAnyway("MapAutosave.30min.m3d","MapAutosave.35min.m3d");
		RenameAnyway("MapAutosave.25min.m3d","MapAutosave.30min.m3d");
		RenameAnyway("MapAutosave.20min.m3d","MapAutosave.25min.m3d");
		RenameAnyway("MapAutosave.15min.m3d","MapAutosave.20min.m3d");
		RenameAnyway("MapAutosave.10min.m3d","MapAutosave.15min.m3d");
		RenameAnyway("MapAutosave.5min.m3d" ,"MapAutosave.10min.m3d");
		RenameAnyway("MapAutosave.m3d","MapAutosave.5min.m3d");
...
void LoadZonesOnlyDialog(){
	...
	if(ProcessLoadSave(100,100,RealLx-200,RealLy-200,name,".m3d","",true,NULL)){
		if(name.str)LoadZonesOnly(name.str);
	}
}
