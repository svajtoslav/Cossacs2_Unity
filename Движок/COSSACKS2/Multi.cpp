...
CEXPORT int AddAbilityToBrigade(byte NI,word BrIndex,int AblIndex){
	...
	if(BR->NABL<MAXABL){
		OneAbility* ABL=ABILS.Abils+AblIndex;
			...
			for(int i=0;i<BR->NMemb;i++){
				...
				if(MID!=0xFFFF){
					...
					if(OB&&OB->Serial==BR->MembSN[i]){
						OB->Life+=ABL->Value;
						if(OB->Life>OB->Ref.General->MoreCharacter->Life)OB->Life=OB->Ref.General->MoreCharacter->Life;
					};
				}
			}
		}
...
void CmdSendPureData(byte* Data,int size){
	SCMD;
	ExBuf[EBPos]=112;
...
void EqualizeGroupSpeed(byte NI,byte AttType){
	...
	for(int i=0;i<N;i++){
		...
		if(MID!=0xFFFF){
			OneObject* OB=Group[MID];
			if(OB&&OB->Serial==SNS[i]){
				int V=OB->newMons->MotionDist;//(OB->newMons->MotionDist*OB->Ref.General->MoreCharacter->Speed)/100;
				if(AttType&&OB->newMons->Rate[0]!=16){
		...
		if(MID!=0xFFFF){
			...
			if(OB&&OB->Serial==SNS[i]){
				if(OB->BrigadeID!=0xFFFF&&OB->BrIndex<NBPERSONAL){
					...
					if(ONEU){
						int V=ONEU->newMons->MotionDist;//(OB->newMons->MotionDist*OB->Ref.General->MoreCharacter->Speed)/100;
						if(AttType&&OB->newMons->Rate[0]!=16){
...
void CreateNewTerrMons(byte NI,int x,int y,word Type){
	if(Type&32768){
		if(NATIONS[NI].Mon[Type&8191]->newMons->Building){		
			...
			CheckSmartCreationAbility(NI,NATIONS[NI].Mon[Type&8191]->newMons,&xx,&yy);
			x=xx>>4;
			y=yy>>4;
		}
		int r0[8];
...
void GoToMineWithSelected(byte NI,word ID){
	...
	for(int i=0;i<Nsel&&Maxins;i++){
		...
		if(!(MID==0xFFFF||CmdDone[MID])){
            ...
            if(OB&&CheckWorker(OB)&&CheckPossibilityToComeInside(OB,OBJ)){
...
void ImSelectAllBuildings(byte NI){
	...
	for(int i=0;i<MAXOBJECT;i++){
		OneObject* OB=Group[i];
		if(OB&&OB->NNUM==GSets.CGame.cgi_NatRefTBL[NI]&&OB->NewBuilding&&!(OB->Sdoxlo||OB->notSelectable()))N++;
	};
	...
	for(i=0;i<MAXOBJECT;i++){
		OneObject* OB=Group[i];
		if(OB&&OB->NNUM==GSets.CGame.cgi_NatRefTBL[NI]&&OB->NewBuilding&&!(OB->Sdoxlo||OB->notSelectable())){
			SMN[N]=OB->Index;
			SRN[N]=OB->Serial;
			N++;
			OB->ImSelected|=GM(NI);
...
void SelectAllUnits(byte NI,bool Re){
	...
	for(int i=0;i<MAXOBJECT;i++){
		OneObject* OB=Group[i];
		if(OB&&OB->NNUM==GSets.CGame.cgi_NatRefTBL[NI]&&!(OB->LockType==1||OB->NewBuilding||OB->newMons->Peasant||OB->Sdoxlo||OB->Wall||OB->UnlimitedMotion||OB->Guard!=0xFFFF||OB->UnlimitedMotion||OB->notSelectable()))N++;
	};
	UNI[0]=znew(word,N);
	USN[0]=znew(word,N);
	lpN[0]=N;
	...
	for(i=0;i<MAXOBJECT;i++){
		OneObject* OB=Group[i];
		if(OB&&OB->NNUM==GSets.CGame.cgi_NatRefTBL[NI]&&!(OB->LockType==1||OB->NewBuilding||OB->newMons->Peasant||OB->Sdoxlo||OB->Wall||OB->UnlimitedMotion||OB->Guard!=0xFFFF||OB->UnlimitedMotion||OB->notSelectable())){
			SMN[N]=OB->Index;
			SRN[N]=OB->Serial;
			if(Re)OB->Selected|=GM(NI);
			else OB->ImSelected|=GM(NI);
...
void SelectAllShips(byte NI,bool Re){
	...
	for(int i=0;i<MAXOBJECT;i++){
		OneObject* OB=Group[i];
		if(OB&&OB->NNUM==GSets.CGame.cgi_NatRefTBL[NI]&&OB->LockType==1&&!(OB->Sdoxlo||OB->notSelectable())){
