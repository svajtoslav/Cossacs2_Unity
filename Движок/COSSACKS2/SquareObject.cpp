...
void DrawFogInWorldSpace(){
	...
	if(!BM){
		...
		BM->setShader(IRS->GetShaderID("fow"));
		...
		for(int ix=0;ix<Nx;ix++){
			...
			if(FogOfWar->Create(ix<<8,iy<<8,&GetFOW,ix<<3,iy<<4,qt_32,qt_16,(void*)ofs,0,0,1,1)){
				...
				if(first){
					...
					//else 
					FogOfWar->BM->setShader(IRS->GetShaderID("fow"));
                    static int ftx=IRS->GetTextureID("FogOfWar.tga");
...
void ReflectionImage::CreateImage(int mx,int my){
	...
	if(!Sky){
		...
		Sky=new BaseMesh;
		Surf=new BaseMesh;
		Sky->create((SkySize+1)*(SkySize+1),SkySize*SkySize*6,vfTnL);
...
GW4_2:
		...
		IRS->SetCurrentShader(BM->getShader());
