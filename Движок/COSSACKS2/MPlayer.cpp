...
void SortPLIDS(){
	...
	for(i=0;i<NPlayers;i++){
		GSets.CGame.PL_INFO[i]=TPI[SC.Uids[i]];
...
bool StartIGame(bool SINGLE){
	...
	FreePDatas();
	if(SINGLE||((!SINGLE)&&SendToAllPlayers(sizeof NSP,&NSP,1))){
		int ttm=GetRealTime();
		do{
	...
	do{
		do{
			...
			if(yyy>5000){
				...
				GPS.FlushBatches();
				int y0=yc+12;
				if(!MiniActive)ShowCString(xc,y0-8,PAUSE,&BigBlackFont);
				y0+=24;
				int Problem=-1;
				...
				for(int i=0;i<NPlayers;i++){
					...
					if(EBufs[i].Enabled){
						if(EBufs[i].RealTime!=NON)strcat(ccc,READYTX);
							...
							if(PlState[i]==-1){//player does not answered still
								if((yyy>45000&&GLOBALTIME>40)||yyy>120000){//player does not answer for a long time
					...
					if(!MiniActive)ShowCString(xc,y0,ccc,&BlackFont);
					y0+=18;
				};
				bool UseF3=0;
				...
				/*if(UseF3){
					if(!MiniActive)ShowCString(xc,y0,DISCNTX,&BlackFont);
				};*/
				yyy=100*64;
				NMessM=0;
				NSysM=0;
