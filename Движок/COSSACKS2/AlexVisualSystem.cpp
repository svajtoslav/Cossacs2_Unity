						...
						if(GetMiddleFrame(&Elm,"gpfile",&gpfile))
						{
							...
							if(but)
							{
								...
								if(GetMiddleFrame(&Elm,"onclick",&tmp))
									but->OnUserClick=GetFunctionByName(tmp.str,&OnUserClick);
							}
...
void AddStlInfoPanel(DialogsSystem* DS){
	...
	if(!stName.isClear()){
		...
		DS->addTextButton(NULL,98-5,yy+2,stName.str,&SmallBlackFont,&SmallBlackFont,&SmallBlackFont,0);
	}	
	// flag
