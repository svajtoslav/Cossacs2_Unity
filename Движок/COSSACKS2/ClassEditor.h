...
#define CED_LineDist 24
#define CED_Font YellowFont
#define CED_AFont WhiteFont
#define CED_SqSize 4
#define CED_LevelWidth 12
#define CED_TextDy 5
extern IRenderSystem*	IRS;
#define TEXFORMAT cfARGB8888
#define TEXPIXSIZE 4
...
public:
	...
	static int GetTexture(){
		if(TexList.GetAmount()){
			...
			TextureDescr td;
			td.setValues(256,256,TEXFORMAT,mpManaged,1,tuProcedural);
			int TextureID=IRS->CreateTexture(cc,td);
			return TextureID;
		}
	}
...
public:
	...
	virtual float GetFValue(int x,int y){		
		...
		if(Mapping){
			...
			x+=phaseX;
			y+=phaseY;
			float V=float(GetFractalValEx(int(x*Scale),int((y/2-H)*Scale),Type))/512.0f;
