...
void InternalProcess(){
	...
	if(NeedToReportInGameStats&&UseGSC_Login){
		...
		if(tmtmt-LastTimeReport_tmtmt>=512){
			...
			for(i=0;i<NPlayers;i++)if(GSets.CGame.PL_INFO[i].PlayerID==MyDPID){
				int NI=GSets.CGame.PL_INFO[i].ColorID;
				int NP=0;
				int NW=0;
				...
				OPR[0].NBornP=NP;
				OPR[0].NBornUnits=NW;
				OPR[0].Population=NMN[GSets.CGame.PL_INFO[i].ColorID];
				OPR[0].Profile=GSets.CGame.PL_INFO[i].ProfileID;
...
void SetGameDisplayModeAnyway(int SizeX,int SizeY){
	...
	prop.m_Width		 = SizeX;
	prop.m_Height		 = SizeY;
	prop.m_ColorFormat = cfBackBufferCompatible;
	if (DDDebug)
	{
...
bool EnterStr(char * str,char* Message){
	...
	InputBox* IB=DSY.addInputBox(NULL,30,45,str,30,224,28,&RedFont,&RedFont);
	IB->Active=true;
	TextButton* OkBtn=DSY.addTextButton(NULL,55+164/2,90,"OK",&WhiteFont,&YellowFont,&YellowFont,1);
	OkBtn->UserParam=mcmOk;
	OkBtn->OnUserClick=&MMItemChoose;
...
//			NCallsMakeDamage=0;
			...
			if(!strcmp(passw,"reinforce"))MultiTvar=!MultiTvar;
			if(!strcmp(passw,"scout"))FogMode=!FogMode;
...
void ShowAbout(){
	...
	//
	int Left=v_ISys.CreditsLeft; //0
	int Right=v_ISys.CreditsRight; //RealLx-1;
	int Top=v_ISys.CreditsTop;
	int Bottom=v_ISys.CreditsBottom;
	int Speed=v_ISys.CreditsSpeed;	
	NAboutLn=400;
	int LDis=v_ISys.CreditsLDis; //25
	...
	for(int i=0;i<NAboutLn;i++){
		if(y0>Top){ //y0>-LDis&&
			if(y0<Bottom){
				...
				if(tx!=pp){
					//DWORD col=TextFon->GetColor();
					//TextFon->SetColor(0xFF7E4710);
					ShowString(((Right+Left)-GetRLCStrWidth(tx,TextFon))>>1,y0,tx,TextFon);
					//TextFon->SetColor(col);
				}else{
...
void EditMiniMap2(){
	...
	byte*  V_TexMap=new byte[(MaxTH+1)*MaxTH];
	memcpy(V_TexMap,TexMap,(MaxTH+1)*MaxTH);
	byte*  V_TexMapEx=new byte[(MaxTH+1)*MaxTH];
	memcpy(V_TexMapEx,TexMapEx,(MaxTH+1)*MaxTH);
	byte*  V_WTexMapEx=new byte[(MaxTH+1)*MaxTH];
	memcpy(V_WTexMapEx,WTexMapEx,(MaxTH+1)*MaxTH);
	byte*  V_FactureMap=new byte[(MaxTH+1)*MaxTH];
	memcpy(V_FactureMap,FactureMap,(MaxTH+1)*MaxTH);
	byte*  V_FactureWeight=new byte[(MaxTH+1)*MaxTH];
	memcpy(V_FactureWeight,FactureWeight,(MaxTH+1)*MaxTH);
	
	ShowHeightMode=true;
	CreateMiniMap();
	DialogsSystem DSS;
	smoothMask=new byte[MaxPointIndex];
	memset(smoothMask,0,MaxPointIndex);
	extern bool mMapPreview;
	mMapPreview=true;
	if(DSS.SafeReadFromFile("Dialogs\\HeightEditor.DialogsSystem.xml")){
		int x0,y0,x1,y1;
		DSS.GetDialogsFrame(x0,y0,x1,y1);
		SimpleDialog* DESK=DSS.Find("#MAPDESK");
		if(x1>x0&&DESK){
			DSS.x=(RealLx-x1+x0)/2;
			DSS.y=(RealLy-y1+y0)/2;
			SimpleDialog* SD=DSS.Find("#PENHEIGHT");			
			VScrollBar* PenHeight=dynamic_cast<VScrollBar*>(SD);
			SD=DSS.Find("#PENRADIUS");
			VScrollBar* PenRadius=dynamic_cast<VScrollBar*>(SD);
			SD=DSS.Find("#PENWIDTH");
			VScrollBar* PenWidth=dynamic_cast<VScrollBar*>(SD);
			SD=DSS.Find("#PENCURVATURE");
			VScrollBar* PenCurvature=dynamic_cast<VScrollBar*>(SD);
			SD=DSS.Find("#TEXTURING");
			CheckBox* TexBox=dynamic_cast<CheckBox*>(SD);
			SD=DSS.Find("#PRESET");
			ComboBox* Preset=dynamic_cast<ComboBox*>(SD);
			SD=DSS.Find("#FRACTALMODULATION");
			VScrollBar* FractMOD=dynamic_cast<VScrollBar*>(SD);
			SD=DSS.Find("#FRACTALFREQ");
			VScrollBar* FractSCAL=dynamic_cast<VScrollBar*>(SD);

			FILE* F=fopen("UserTerrain\\Presets\\he_settings.dat","r");
			if(F){
				fscanf(F,"%d%d%d%d%d%d%d",&PenHeight->SPos,&PenRadius->SPos,&PenWidth->SPos,&PenCurvature->SPos,&PenHeight->SPos,&FractMOD->SPos,&FractSCAL->SPos);
				fclose(F);
			}
			...
			CreateHiMap();
			//if(DESK){
			//	BPXView* BV=DSS.DSS[0]->addBPXView(NULL,DESK->x,DESK->y,HiLx,HiLy,1,1,1,HiMap1,NULL);
			//}
			int R1=1000;
			int DR=700;
			int H=256;
			...
			int RLX=(MaxMapX-MinMapX)*32;
			int RLY=(MaxMapY-MinMapY)*32;
			int SLX=DESK->x1-DESK->x;
			int SLY=DESK->y1-DESK->y;
			bool cht=ShowHeightMode;
			char curpres[256]="";
			Lpressed=false;
			do{		
				FractAmpl=FractMOD->SPos;
				FractScale=FractSCAL->SPos;
				if(Preset->NLines){
					char* pres=Preset->Lines[Preset->CurLine];
					if(strcmp(pres,curpres)){
						strcpy(curpres,pres);
						SetCurrentPreset(curpres);
					}
				}
				if(TexBox){
					Preset->Visible=TexBox->State;
					ShowHeightMode=!TexBox->State;
				}
				if(cht!=ShowHeightMode){
                    cht=ShowHeightMode;
					CreateMiniMap();
				}
				R1=PenRadius->SPos*8;
				DR=PenWidth->SPos*8;
				H=PenHeight->SPos-200;
				ProcessMessages();
				int LP=Lpressed;
				int RP=Rpressed;
                DSS.ProcessDialogs();
				if(WheelDelta){
					if(GetKeyState(VK_MENU)&0x8000){
						PenCurvature->SPos+=WheelDelta/30;
					}else
					if(GetKeyState(VK_SHIFT)&0x8000){
						PenWidth->SPos+=WheelDelta/30;
					}else
					if(GetKeyState(VK_CONTROL)&0x8000){
						PenHeight->SPos+=WheelDelta/30;
					}else{
						PenRadius->SPos+=WheelDelta/30;
					}
					WheelDelta=0;
				}
				if(DESK){
					TempWindow TW;
					PushWindow(&TW);
					IntersectWindows(DESK->LastDrawX,DESK->LastDrawY,DESK->LastDrawX+DESK->x1-DESK->x,DESK->LastDrawY+DESK->y1-DESK->y);
					if(DESK->MouseOver){
                        int dx=mouseX-DESK->LastDrawX;
						int dy=mouseY-DESK->LastDrawY;            
						int N1=R1*SLX/RLX;
						tpr5=PenCurvature->SPos;
						Circ2(mouseX,mouseY,N1,0xFFFFFFFF,true);
						int N2=(R1+DR)*SLX/RLX;
						Circ2(mouseX,mouseY,N2,0xFFFFFFFF,true);
						GPS.FlushBatches();
						int xxx=dx*RLX/SLX;
						int yyy=dy*RLY/SLY;
						if(LP){
							...
							yc=yyy;
							HiRound=1;
							if(TexBox->State)CreateEffect(xxx,yyy,(tpr2+tpr3)*2,&TransfTex);
							else CreateEffect(xxx,yyy,(tpr2+tpr3)*2,&Transf1);                            
							void CreateMiniMapPart(int x0,int y0,int x1,int y1,bool MiniMap);
							CreateMiniMapPart( (xxx-R1-DR-512)>>6 , (yyy-R1-DR-512)>>6 , (xxx+R1+DR+512)>>6 , (yyy+R1+DR+512)>>6 ,false);							
							//CreateMiniMap();
						}
						if(RP){
							int xxx=dx*RLX/SLX;
							int yyy=dy*RLY/SLY;
                            PenHeight->SPos=GetHeight(xxx,yyy)+200;
						}
					}
					PopWindow(&TW);
				}
			...
			F=fopen("UserTerrain\\Presets\\he_settings.dat","w");
			if(F){
				fprintf(F,"%d %d %d %d %d %d %d",PenHeight->SPos,PenRadius->SPos,PenWidth->SPos,PenCurvature->SPos,PenHeight->SPos,FractMOD->SPos,FractSCAL->SPos);
				fclose(F);
			}
		}		
	}		
	mMapPreview=false;
	if(ItemChoose==11){//cancel
		memcpy(THMap,VERHI,(MaxTH+1)*MaxTH*2);
		memcpy(TexMap,V_TexMap,(MaxTH+1)*MaxTH);
		memcpy(TexMapEx,V_TexMapEx,(MaxTH+1)*MaxTH);
		memcpy(WTexMapEx,V_WTexMapEx,(MaxTH+1)*MaxTH);
		memcpy(FactureMap,V_FactureMap,(MaxTH+1)*MaxTH);
		memcpy(FactureWeight,V_FactureWeight,(MaxTH+1)*MaxTH);
	}else{
		for(int j=0;j<20;j++){
			for(int i=0;i<MaxPointIndex;i++)if(smoothMask[i]){
...
void PrepareGameMedia(byte myid,bool SaveNR,bool DisableDLL){
			...
			if(GSets.CGame.UnitTransform&&NPlayers>0) for(int j=0;j<MAXOBJECT;j++){
				...
				if(OB&&!OB->Sdoxlo){ //&&OB->newMons->Peasant
					for(int i=0;i<NPlayers||COMPSTART[i];i++){
						...
						int t;
						if(OB->NNUM==I->ColorID&&GetNewUnitType(OB->NIndex,I->NationID,t)){
							OneObject* TransformUnitType(OneObject* OB,int DestType);
							TransformUnitType(OB,t);
							/*
							GeneralObject** GOS=NATIONS->Mon;
							int N=NATIONS->NMon;
							for(int t=0;t<N;t++)if(!strcmp(GOS[t]->MonsterID,GlobalAI.Ai[I->NationID].Peasant_ID)){
								OB->NIndex=t;
								OB->Ref.General=NATIONS[OB->NNUM].Mon[t];
				...
				if(COMPSTART[i]){
					...
					if(P&&P->DisableStrategicAI){
						...
						for(int j=0;j<MAXOBJECT;j++){
							OneObject* OB=Group[j];
							if(OB&&OB->NNUM==c&&!OB->Sdoxlo&&(OB->newMons->Peasant||OB->NewBuilding)){
	...
	for(int i=0;i<EngSettings.SpecifySpeedForMaps.GetAmount();i++){
		...
		for(int j=0;j<n;j++){
			char* s=EngSettings.SpecifySpeedForMaps[i]->MapsList[j]->MapName.str;
...
//				NCallsMakeDamage=0;
			...
			if(!strcmp(ChatString,"reinforce"))MultiTvar=!MultiTvar;
			if(!strcmp(ChatString,"scout")){
#ifndef AI_CHEATS
				CurrentSinBattle=-1;
#endif
				FogMode=!FogMode;
				...
				if(z==2&&!strcmp(cc1,"bstage")){
					...
					for(int i=0;i<N;i++){
						...
						if(OB&&!OB->Sdoxlo&&OB->NewBuilding){
							if(vv>100)vv=100;
							if(vv==100){
								OB->Stage=OB->Ref.General->MoreCharacter->ProduceStages;                                
								OB->Life=OB->MaxLife-1;
								OB->Ready=false;
								OB->NextStage();
							}else{
								OB->Stage=OB->Ref.General->MoreCharacter->ProduceStages*vv/100;
...
void ShowChat(){
	...
	if(EnterChatMode){
		...
		//
		if(NPlayers!=1){
			ShowString(x0-L-4,y0-10+v_ISys.ChatTitleDy+addY0,ctit,&SmallBlackFont);
		}		
		ShowString(x0-L-4,y0-10+v_ISys.ChatTitleDy,cstr,&BigBlackFont);
		//
		y0+=v_ISys.ChatStringDy;
		//		
		if((GetRealTime()/200)&1)ShowString(x0-L+L0+4,y0-10,ChatString,&BlackFont);
		else{
			int xx=0;
			for(int i=0;i<ChatCursPos;i++)xx+=GetRLCWidth(YellowFont.RLC,ChatString[i]);
			//sprintf(cc2,"%s|",ChatString);
			ShowString(x0-L+L0+4,y0-10,ChatString,&BlackFont);
			ShowString(x0-L+L0+xx+4,y0-10,"|",&BlackFont);
		};
	};
	if(CHATSTRING[0]){
...
//DWORD ov_GetStreamLength(DWORD dwStream=0);
...
bool GetGlobalBrigProp(byte NI){
	...
	int C,O,T,A=0,BA=0;	// ba -  êîëè÷åñòâî â îäíîì îòðÿäå
	word *NINDEX, *AMOUNT;
	word prio=0xFFFF;
