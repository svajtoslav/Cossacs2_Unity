...
void SetTexFactInPoint(int x,int y,int R1,int R2,int ter){
	...
	if(R2<=0)
		goto End;
	if(TP->Terr.GetAmount()<=ter)
		goto End;
	int NRules=TP->Terr[ter]->Rules.GetAmount();
	if(NRules==0)
		goto End;
	int TW,WC,WW1;
	int* LIST=new int[NRules*2+2];
	int* Texture=new int[NRules*2+2];
	int* TextureWeight=new int[NRules*2+2];
	int* Facture=new int[NRules*2+2];
	int* FactureMaxWeight=new int[NRules*2+2];
	int vx=x>>5;
	if(vx&1)y+=16;
	int vy=y>>5;
	int vR1=R1>>5;
	int vR2=R2>>5;
	float w0=TP->Terr[ter]->GlobalWeight;
	for(int yy=vy-vR2;yy<vy+vR2;yy++){
		for(int xx=vx-vR2;xx<vx+vR2;xx++)if(xx>=0&&xx<VertInLine&&yy<MaxPointIndex/VertInLine&& 
		                                   yy>=0&&((xx-vx)*(xx-vx)+(yy-vy)*(yy-vy)<vR2*vR2)){
			int V=yy*VertInLine+xx;
			int x1=xx<<5;
			int y1=yy<<5;
			if(x1&1)y1-=16;
		    int H=THMap[V];
			int A=GetAngleInPoint(x1,y1);
			float w=256*w0*TP->Terr[ter]->WeightVariation.GetFValue(x1,y1);
			...
			LIST[0]=0;
				NTX++;
			if(TexMap[V]==TexMapEx[V])
				TextureWeight[0]=255-WW1;
			else{
				TextureWeight[0]=((255-WTexMapEx[V])*(255-WW1))>>8;
				Texture[1]=TexMapEx[V];
				TextureWeight[1]=(WTexMapEx[V]*(255-WW1))>>8;
				LIST[1]=1;
				NTX++;
			}
			int NTex=0;
			for(int rule=0;rule<NRules;rule++){
				WC=TP->Terr[ter]->Rules[rule]->Condition.GetResult(x1,y1,H,A,TexMap[V],FactureMap[V],FactureWeight[V]);
				if(WC>0){
					ExTexResult tr;
					TP->Terr[ter]->Rules[rule]->Action.GetResult(x1,y1,WC,tr);
					if(tr.t1!=tr.t2){
						Texture[NTex+NTX]=tr.t1;
						Texture[NTex+NTX+1]=tr.t2;
						TextureWeight[NTex+NTX]=(tr.w1*WW1)>>8;
						TextureWeight[NTex+NTX+1]=(tr.w2*WW1)>>8;
						Facture[NTex+NTX]=tr.f1;
						Facture[NTex+NTX+1]=tr.f2;
						FactureMaxWeight[NTex+NTX]=tr.fWeight1;
						FactureMaxWeight[NTex+NTX+1]=tr.fWeight2;
						LIST[NTex+NTX]=NTex+NTX;
						LIST[NTex+NTX+1]=NTex+NTX+1;
						NTex+=2;
					}else{
						Texture[NTex+NTX]=tr.t1;
						TextureWeight[NTex+NTX]=(tr.w1*WW1)>>8;
			...
			if(NTex>0){
				int NTX1=NTX;
				for(int i=0;i<NTX1+NTex-1;i++){
					for(int j=i+1;j<NTX1+NTex;j++){
						if(i!=j && Texture[j]>=0 && Texture[i]==Texture[j]){
								TextureWeight[i]+=TextureWeight[j];
								Facture[i]=Facture[j];
								FactureMaxWeight[i]=FactureMaxWeight[j];
								Texture[j]=-1;
						}
						if(j==i+1 && j>=NTX1 && Texture[j]>=0){
							LIST[NTX]=j;
							NTX++;
						}
					}
				...
				do{
					change=0;
					for(int i=1;i<NTX;i++){
						if(TextureWeight[LIST[i-1]]<TextureWeight[LIST[i]]){
							swap(LIST[i-1],LIST[i]);
							change=1;
						}
					}
				}while(change);
				if(NTX>1 && TextureWeight[LIST[0]]>0){
					if(Texture[LIST[0]]!=TexMap[V]){
						TexMap[V]=Texture[LIST[0]];
						FactureMap[V]=Facture[LIST[0]];
					}
					int WW=255*FactureMaxWeight[LIST[0]]*TextureWeight[LIST[0]]/(TextureWeight[LIST[1]]+TextureWeight[LIST[0]]);
					if(WW>255)WW=255;
					FactureWeight[V]=WW;
					TexMapEx [V]=Texture[LIST[1]];
					WTexMapEx[V]=255*TextureWeight[LIST[1]]/(TextureWeight[LIST[1]]+TextureWeight[LIST[0]]);
					void MarkPointToDraw(int Vert);
					MarkPointToDraw(V);
				}
			}
		}
...
void ExLandsScope::Generate(){
	if(GenerateRelief)Relief.GenerateHeight();
	addmr(101);
	if(GenerateTextures){
		bool isDz=false;
		DynArray<int,100000> Bounds;
		int* NewHMap=new int[MaxPointIndex];
		for(int i=0;i<MaxPointIndex;i++){
			if(!(i&1023))SetCurrentProgress("Painting",i,MaxPointIndex);
			int vx=i%VertInLine;
			int vy=i/VertInLine;
			int x=vx<<5;
			int y=vy<<5;
			...
			addmr(i);
			addmr(H);
			bool Bound=GetResult(x,y,H,A,TexMap[i],FactureMap[i],FactureWeight[i],tr);
			addmr(tr.TreesGrp);
			if(Bound) Bounds.Add(i);
			TexMap[i]=tr.t1;
			TexMapEx[i]=tr.t2;
			if(tr.w1==0)tr.w1=1;
			WTexMapEx[i]=255*tr.w2/(tr.w2+tr.w1);
			FactureMap[i]=tr.f1;
			if(tr.f1<0)tr.f1=0;
			if(tr.f2<0)tr.f2=0;
			if(tr.f1>39)tr.f1=0;
			if(tr.f2>39)tr.f2=0;
			int FW=int(255.0f*tr.fWeight1*tr.w1/(tr.w1+tr.w2));
			if(FW<0)FW=0;
			if(FW>255)FW=255;
			FactureWeight[i]=FW;
			if(tr.tdz1>500) tr.tdz1=500;
			if(tr.tdz2>500) tr.tdz2=500;
			if(tr.tdz1<-500) tr.tdz1=-500;
			if(tr.tdz2<-500) tr.tdz2=-500;
			if(tr.tdz1 || tr.tdz2)
				isDz=true;
			if(tr.w1>0 || tr.w2>0)
				NewHMap[i]=(int)((tr.w1*tr.tdz1+tr.w2*tr.tdz2)/(tr.w1+tr.w2));
			else
				NewHMap[i]=0;
			addmr(200);
			int trees=tr.TreesGrp;
			int tw=int(tr.TreesDensity*256.0f*256.0f);
			addmr(trees);
			if(trees>=0&&trees<255&&mrand()<tw){
				addmr(207);
				int weicv=TSC.TGroup.GetAmount();
				int ns=TSC.TGroup[trees]->SGS.GetAmount();
				int idx=(ns*int(mrand()))>>15;
				addmr(idx);
				addmr(weicv);
				if(TSC.TGroup[trees]->SGS[idx]->Objects[idx]->ModelManagerID<=0){
					addmr(333);
					void addSprite(int x,int y,SprGroup* SG,word id,int PrefRadius);
					int R=int(tr.TreesDist*300)+1;
					int NA=1+(60/R);
					if(NA>8)NA=8;
					addmr(202);
					for(int q=0;q<NA;q++){
						addmr(204);
						addSprite(x+(mrand()&31)-15,y+(mrand()&31)-15,TSC.TGroup[trees]->SGS[idx],TSC.TGroup[trees]->Indices[idx],R);
						addmr(205);
						extern int LastSpriteIndex;
						if(LastSpriteIndex!=-1){
							Sprites[LastSpriteIndex].Locking=tr.LockMask;
							if(tr.LockMask&1)Sprites[LastSpriteIndex].Locking|=8;
						}
						addmr(206);
					}
					addmr(203);
		...
		if(isDz){
			int sumdz;
			int* NewHMap1=new int[MaxPointIndex];
			for(int c=0;c<3;c++){
				for(int i=VertInLine;i<MaxPointIndex-VertInLine;i++){
					if(i%VertInLine>0 && i%VertInLine<VertInLine-1){
						sumdz=NewHMap[i-VertInLine]+NewHMap[i+VertInLine]+NewHMap[i-1]+NewHMap[i+1]+3;
						if((i+(int)(i/VertInLine))%2>0)
							sumdz+=NewHMap[i-VertInLine-1]+NewHMap[i-VertInLine+1];
						else
							sumdz+=NewHMap[i+VertInLine-1]+NewHMap[i+VertInLine+1];
						NewHMap1[i]=sumdz/6;
					}
				...
				for(int i=0;i<MaxPointIndex;i++){
					if(c==2) THMap[i]+=NewHMap[i];
					else NewHMap[i]=NewHMap1[i];
				}
			}
			delete NewHMap1;
		}
		...
		if(PostProcessType==2){
			int ns=min(PostStagesAmount,3)*(MaxPointIndex-VertInLine*2);
			SetCurrentProgress("Textures smoothing",0,ns);
			int pp=0;
			for(int j=0;j<PostStagesAmount&&j<3;j++){
				int fin=MaxPointIndex-VertInLine;
				for(int i=VertInLine;i<fin;i++){
					void PerformTexSmoothingInPoint(int vx,int vy,int x0,int y0,int r);
					PerformTexSmoothingInPoint(i%VertInLine,i/VertInLine,0,0,100000);
					pp++;
					if((pp&8191)==0)SetCurrentProgress("Textures smoothing",pp,ns);
				}
			}
		}
		addmr(104);
		if(PostProcessType==1){
			extern bool LockMarkPoint;
			LockMarkPoint=1;
			int ns=min(PostStagesAmount,3)*Bounds.GetAmount();
			SetCurrentProgress("Textures smoothing",0,ns);
			int pp=0;
			for(int j=0;j<PostStagesAmount&&j<3;j++){
				int fin=Bounds.GetAmount();
				for(int i=0;i<fin;i++){
					int idx=Bounds[i];
					void PerformTexSmoothingInPoint(int vx,int vy,int x0,int y0,int r);
					PerformTexSmoothingInPoint(idx%VertInLine,idx/VertInLine,0,0,100000);
					pp++;
					if((pp&8191)==0)SetCurrentProgress("Textures smoothing",pp,ns);
				}
			}
		...
		addmr(105);
		int ns=2*(MaxPointIndex-VertInLine*2);
		SetCurrentProgress("Edges smoothing",0,ns);
		int pp=0;
		for(int q=0;q<2;q++){								
			int N=MaxPointIndex-VertInLine;
			for(int i=VertInLine;i<N;i++){
				int ix=i%VertInLine;
				int iy=i/VertInLine;
				if(ix>0&&ix<VertInLine-1){
					int H0=iy*16-THMap[i-VertInLine];
					int H1=iy*16+32-THMap[i+VertInLine];
					if(H1<H0){
						int sumdz=THMap[i-VertInLine]+THMap[i+VertInLine]+THMap[i-1]+THMap[i+1]+3;
						if((i+(int)(i/VertInLine))%2>0)
							sumdz+=THMap[i-VertInLine-1]+THMap[i-VertInLine+1];
						else
							sumdz+=THMap[i+VertInLine-1]+THMap[i+VertInLine+1];
						THMap[i]=sumdz/6;
					}
				...
				pp++;
				if((pp&8191)==0)SetCurrentProgress("Edges smoothing",pp,ns);
			}
		}
		...
		switch(ItemChoose){
			...
			case 2://Save
				...
				if(ProcessSavingFile("XmlMaps\\*.xml",dst,3,1)){
					...
					sd="XmlMaps\\";
					sd+=dst;
					if(!strstr(sd.str,".xml"))sd+=".xml";
                    xml3.WriteToFile(sd.str);
				}
				break;
			case 3://Generate
				TP->GenerateRelief=1;
		...
		for(int j=0;j<StartPieces.GetAmount();j++){
			...
			for(int p=0;p<StartPieces[j]->AmountInNearZone;p++){
				...
				if(temp->Pieces.FindRandomPos(xx,yy,r0,r1,0,temp->Voronoi,i)){
					char* s=StartPieces[j]->Samples[mrand()%StartPieces[j]->Samples.GetAmount()]->Name.str;
					extern bool ImmVis;
					ImmVis=true;
					byte cc=MyNation;
				...
				for(int i=0;i<FarPieces.GetAmount();i++){
					if(FarPieces[i]->Name.str){
						...
						if(temp->Pieces.Get(x,y)==0){
							...
							RM_Load(FarPieces[i]->Name.str,x*scv,y*scv);
							for(int q=-st_sz;q<=st_sz;q++)
								for(int p=-st_sz;p<=st_sz;p++)
...
const char* GetExtraOptionElementName(int idx,int eidx){
	...
	switch(idx){
		case 0:
			return GetTextByID(RMDESC.StartPositions[eidx]->Name.str);
		case 1:
			if(eidx)return GetTextByID("MS_Islands");
			else return GetTextByID("MS_Land");
		case 2:
			return GetTextByID(RMDESC.StartRes[eidx]->Name.str);
		case 3:
			...
			if(TP->Terr[i]->Enabled){
				...
				for(int j=0;j<EXT->Rules.GetAmount();j++){
					...
					if(ORL->Enabled){
						...
						if(R>8&&ORL->Description.str){
							char cc[64];
							sprintf(cc,"[%s: %d%%] ",ORL->Description.str,R*100/255);
							dest+=cc;
						}
					}
				}
			...
			if(ItemChoose==10){//new preset
				...
				if(PRESET){
					...
					for(int i=0;i<CL.GetAmount();i++){
                        PRESET->AddLine(CL[i]->str);
						if(!strcmp(CL[i]->str,pname)){
							PRESET->CurLine=i;
						}
					}
				}
	...
	virtual void OnDraw(SimpleDialog* SD){
		...
		if(CB){
			if(CB->TestMode){
                ...
                FindAllPresets(CL);
				for(int i=0;i<CL.GetAmount();i++){
					CB->AddLine(GetTextByID(CL[i]->str));
				}
			}
		}
	}
...
void SetTextureInPointUsingPreset(int Vertex,int Texture){
	if(Vertex>=0&&Vertex<MaxPointIndex){
		if(CurPreset){
			...
			int A=GetAngleInPoint(x,y);
			ExTexResult tr;
			CurPreset->GetResult(x,y,H,A,TexMap[Vertex],FactureMap[Vertex],FactureWeight[Vertex],tr);
			if(tr.w2+tr.w1){
				TexMap[Vertex]=tr.t1;
				TexMapEx[Vertex]=tr.t2;
				WTexMapEx[Vertex]=255*tr.w2/(tr.w2+tr.w1);
				FactureMap[Vertex]=tr.f1;
				int FW=int(255.0f*tr.fWeight1*tr.w1/(tr.w1+tr.w2));
				if(FW<0)FW=0;
				if(FW>255)FW=255;
				FactureWeight[Vertex]=FW;
			}
		}else if(Texture!=-1)TexMap[Vertex]=Texture;
	}
}
