	...
	if(F){
		...
		do{
			...
			if(S1[0]&&S2[0]){
				...
				PLLIST[NPLR-1]=znew(char,strlen(S1)+1);
				strcpy(PLLIST[NPLR-1],S1);

				EMAILS[NPLR-1]=znew(char,strlen(S2)+1);
				strcpy(EMAILS[NPLR-1],S2);

				PASWRD[NPLR-1]=znew(char,strlen(S3)+1);
...
void CurrentGame::CreateGame(char* pGameName,char* pMap,char* pNick){
	if(LOGIN.ProfileID){
		...
		NPL=NPlayers;
		GameType=GSets.CGame.PL_INFO[0].VictCond;
		...
		if(no){
			for(int q=0;q<no;q++){
				...
				if(NATIONS[GSets.CGame.cgi_NatRefTBL[ord[q]]].AI_Enabled){
					Active=0;
					DeleteThisGameFromFile();
					return;
				};
				byte c=GSets.CGame.cgi_NatRefTBL[ord[q]];
				int v=ord[q];
				if(!(ms&prevms)){
					CurTm++;
				};
				int PIDIDX=-1;
				for(int w=0;w<NPlayers;w++)if(GSets.CGame.PL_INFO[w].ColorID==ord[q])PIDIDX=w;
				if(PIDIDX==-1){
					...
					strcpy(PLAYERS[q].Nick,GSets.CGame.PL_INFO[PIDIDX].name);
					PLAYERS[q].Rank=GSets.CGame.PL_INFO[PIDIDX].Rank;
					PLAYERS[q].Profile=GSets.CGame.PL_INFO[PIDIDX].ProfileID;
				};
				...
				if(GSets.CGame.PL_INFO[q].PlayerID==MyDPID){
					strcpy(InGameNick,GSets.CGame.PL_INFO[q].name);
				};
...
void LOOSEANDEXITFAST(){
	if(CURIGAME.Active){
		for(int i=0;i<NPlayers;i++)if(GSets.CGame.PL_INFO[i].ColorID==MyNation){
			CURIGAME.UpdateGame();
			CURIGAME.AssignDefeat(GSets.CGame.PL_INFO[i].name);
			CURIGAME.SaveGameToFile();
			CURIGAME.Active=0;
		};
	};
