	...
	for(int j=0;j<NUNITS;j++){
		...
		if(UIDS==0xFFFF){
			...
			if(GO->newMons->IconID!=0xFFFF){
				TOOL_PARAM.addGPPicture(NULL,6+xx*(IconLX+4),yy,0,GO->newMons->IconID);
...
void CreateTOOL_PARAM_SPRITE(byte ID){
	...
	if(CURSG->Sections.GetAmount()){
		...
		TP_CSECT->Center-=5;
		for(int i=0;i<CURSG->Sections.GetAmount();i++)TP_CSECT->AddLine(CURSG->Sections[i]->SectionName.str);
	...
	for(int i=0;i<=n;i++){
		...
		if(OC&&OC->FixDx){
			...
			int Lx=GPS.GetGPWidth(OC->FileID_forPreview,OC->SpriteID_forPreview&4095);
			int Ly=GPS.GetGPHeight(OC->FileID_forPreview,OC->SpriteID_forPreview&4095);
			int dxx,dyy;
			Rct F;
		...
		if(sp<CURSG->GetNSprites()){
			...
			if((GetKeyState(VK_TAB)||InteriorType==3)&&mouseX<RealLx-256-20&&Shifter==5){
				...
				if(OC->ViewType==1){
					...
					IMM->Render(OC->ModelManagerID,&M4);
				}else
				if(OC->HaveAligning){
					OneSprite OS;
					OS.x=x;
