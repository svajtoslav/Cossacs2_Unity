...
void	CBigMapQuest::GetQuestsData()
{
	...
	do
	{
		...
		// skip line
		if( a_DS2.str[0] == 47 /*"/"*/ ) { a_DS.ExtractLine(a_DS2); line++; continue; }   //if( a_DS2 == "/" || a_DS2 == "//"  || a_DS2 == "///" ) { a_DS.ExtractLine(a_DS2); continue; }
		// ports
		if( a_DS2 == "ports" ) {
			a_DS.ExtractWord(a_DS2);
			int num = atoi(a_DS2.str);
			for(int i=0; i < num && i < 50 ; i++) {
				a_DS.ExtractWord(a_DS2);
				this->ports[i] = atoi(a_DS2.str) ;
			}
		}
		// next item
		if( a_DS2 == "[QUEST]" ) {
			...
			if( pQ ) {
				...
				if( a_DS2 == "recipient" ) {
					...
					for(int i=0; i < num && i < MAX_PLAYERS ; i++) {
						a_DS.ExtractWord(a_DS2);
						pQ->taker[i] = atoi(a_DS2.str) ;
					}
				}
				// probability
				if( a_DS2 == "probability" ) {
					a_DS.ExtractWord(a_DS2);
					pQ->prob = atoi(a_DS2.str);
				}
				//turn_limit
				if( a_DS2 == "turn_limit" ) {
					a_DS.ExtractWord(a_DS2);
					pQ->turn_limit = atoi(a_DS2.str);
					pQ->wait = pQ->turn_limit/2 ? pQ->turn_limit/2 : 1 ; 
				}
				// gold
				if( a_DS2 == "gold" ) {
					a_DS.ExtractWord(a_DS2);
					pQ->gold = atoi(a_DS2.str);
				}
				// main
				if( a_DS2 == "main" ) {
					a_DS.ExtractWord(a_DS2);
					pQ->main = atoi(a_DS2.str);
				}
			}
		}
		// end
		if( a_DS2 == "[END]" || a_DS2.str[0] == 0 || line > 0xFFFF )  quit = true;
