...
bool	CSectStatData::GetData(DString* pDS){
	...
	if (a_dsWR=="#SECTOR#"){
		...
		if (a_dsWR=="SityXY"){
			pDS->ExtractWord(a_dsWR);
			m_SityX = atoi(a_dsWR.str);
			pDS->ExtractWord(a_dsWR);
			m_SityY = atoi(a_dsWR.str);
		}
		else{
		...
		if (a_dsWR=="FortXY"){
			pDS->ExtractWord(a_dsWR);
			m_FortX = atoi(a_dsWR.str);
			pDS->ExtractWord(a_dsWR);
			m_FortY = atoi(a_dsWR.str);
		}
		else{
		...
		if (a_dsWR=="Population"){
			pDS->ExtractWord(a_dsWR);
			m_inPopulation = atoi(a_dsWR.str);
		...
		if (a_dsWR=="SectorName"){
			pDS->ExtractWord(a_dsWR);
			m_chSectorName = GetTextByID( a_dsWR.str );
		}
		else{
		...
		if (a_dsWR=="Neighbors"){
			pDS->ExtractWord(a_dsWR);
			m_inNON = atoi(a_dsWR.str);
		//	m_pinNIDS = new int[m_inNON];
			for (int i=0; i<m_inNON; i++){
				pDS->ExtractWord(a_dsWR);
		//		m_pinNIDS[i] = atoi(a_dsWR.str);
				m_pinNIDS.Add( atoi(a_dsWR.str) );
			}
		}
		else{
		...
		//	m_pinArrowD = new int[m_inNON*3];
			for (int i=0; i<m_inNON*3; i++){
				pDS->ExtractWord(a_dsWR);
		//		m_pinArrowD[i] = atoi(a_dsWR.str);
				m_pinArrowD.Add( atoi(a_dsWR.str) );
			}
		}
		else{
...
void	CSectStatData::CreateHint2()
{
	_str	a_Hint;
	a_Hint = "";
	DYNHINT_RA( m_psdSec , a_Hint.str );
}
...
void	CSectStatData::CreateHint()
{
	...
	_str	a_Hint;
	a_Hint = "{FM}{CR}";	
	if( m_chSectorName.str )  a_Hint += m_chSectorName;//very new!!! 8/12/04
	...
	DYNHINT_RA( m_psdSec , a_Hint.str );
	//DYNHINT_RA( m_psdSity , a_Hint.str );
}
//void	CSectStatData::CreateHint()
...
CSectData::CSectData(char* DataFileName){
	...
	if (a_DS2=="#MAPSIZE#"){
		a_DS.ExtractWord(a_DS2);
		m_fWM = atoi(a_DS2.str);
		a_DS.ExtractWord(a_DS2);
		m_fHM = atoi(a_DS2.str);
	}
	else{
...
void	CSectorMenu::CreatePictureAndButton( DialogsSystem* psdMenu,LocalGP* plgpPic,LocalGP* pFlags )
{
	...
	_str	a_Hint;
	a_Hint = "{FS}{C FF502515}"; a_Hint += GetTextByID("#CHINT_UpgDef"); DYNHINT_RA(m_pgpbDefence,a_Hint.str);
	...
	_str	a_HintRP;
	a_HintRP = "{FS}{C FF502515}"; a_HintRP += GetTextByID("#CHINT_SectIncomeRes"); DYNHINT_RA(m_pgppResPict,a_HintRP.str);
	...
	_str	a_Hint2;
	a_Hint2 = "{FS}{C FF502515}"; a_Hint2 += GetTextByID("#CHINT_Diversion"); DYNHINT_RA(m_pgpbDiversion,a_Hint2.str);
...
bool	CSectorMenu::CheckDefenceButton( CDiplMenu* pDiplMenu , CPlayer* pPlayer, int sectID , int playerNation ) //grey
{
	...
	// SetNewHint
	if(!DefDisabled)  a_Hint += a_Cost.str;
	//a_Hint += ")";
	DYNHINT_RA( m_pgpbDefence , a_Hint.str );
	...
	if( DefPressed ) {
		if( DefDisabled ) {
			...
			if( EnoughRes ) {
				sprintf(text, GetTextByID("#CWM_UpgradeDef") , a_Cost.str );
...
bool	CSectorMenu::CheckDiversionButton( CDiplMenu* pDiplMenu , CPlayer* pPlayer, int sectID , int playerNation )
{
	...
	DYNHINT_RA(m_pgpbDiversion,a_Hint2.str);
	//
...
bool	CSectorMenu::ShowDiversionActions( DialogsSystem* pMenu , int x0, int y0 )
{
	...
	a_Hint2 = "{FS}{C FF502515}";  a_Hint2 += GetTextByID("#CHINT_Diversion0");  DYNHINT_RA( m_ptbDText[0] ,a_Hint2.str);
	...
	a_Hint2 = "{FS}{C FF502515}";  a_Hint2 += GetTextByID("#CHINT_Diversion1");  DYNHINT_RA( m_ptbDText[1] ,a_Hint2.str);
	...
	a_Hint2.DString::Clear();
	a_Hint2 = "{FS}{C FF502515}";  a_Hint2 += GetTextByID("#CHINT_Diversion2");  DYNHINT_RA( m_ptbDText[2] ,a_Hint2.str);
	...
	a_Hint2.DString::Clear();
	a_Hint2 = "{FS}{C FF502515}";  a_Hint2 += GetTextByID("#CHINT_Diversion3");  DYNHINT_RA(m_ptbDText[3],a_Hint2.str);
...
void	CSectorMenu::SetDiversionTexts( int def )
{
	...
	for(int i=0; i < m_iDTN ; i++)
	{
		...
		sprintf( sTMP, GetTextByID( textID ) , txt.str ,  pt );		// - ( 5 + 3 * 0 ) * def  ) ;
		_str	txt2;
		txt2 = " ";
		txt2 += sTMP;
		if( m_ptbDText[i] ) {
			m_ptbDText[i]->SetMessage( txt2.str ); //sTMP ) ;
...
bool	CUnit::operator =(CUnit& other){
	if (other.m_chName.str!=NULL){
		m_chName.operator =(other.m_chName.str);					/*strcpy(,other.m_chName);*/
	}
	else{
...
void	CHero::CreateEmptyHint()
{
	_str	a_Hint;
	a_Hint = "";
	DYNHINT_RA(m_psdMapPic,a_Hint.str);
}
//----------------------------------------------
void	CHero::CreateHint()
{
	...
	_str  a_Hint;
	a_Hint  = "{FM}{CR}" ; 					a_Hint += m_chName.str ;	//a_Hint += "{C}" ;
...
char*	CArmyLimit::GetNSqdID    ( int rankID, int countryID, int n ){
	if ( 0<=rankID && rankID<MAX_RANKS && 0<=countryID && countryID<MAX_COUNTRYS ) {
		if ( 0<=n && n<HERO_RANKS[rankID].TYPES_CONT[countryID].TN ) {
			return HERO_RANKS[rankID].TYPES_CONT[countryID].TYPES[n].unitName.str;
		}
	}
	return	NULL;
}
...
bool	CArmyLimit::Load(char* fName){
	...
	for (int i=0; i<MAX_RANKS; i++){
		...
		HERO_RANKS[i].rankID=atoi(a_DS3.str);
		HERO_RANKS[i].rankName = GetTextByID( a_DS2.str );
		
		for (int ii=0; ii<MAX_COUNTRYS; ii++){
			a_DS.ExtractWord(a_DS2);
			HERO_RANKS[i].TYPES_CONT[ii].country = GetTextByID( a_DS2.str );
			a_DS.ExtractWord(a_DS2);
			HERO_RANKS[i].TYPES_CONT[ii].maxSquards=atoi(a_DS2.str);

			if (i!=0){
				HERO_RANKS[i].TYPES_CONT[ii]=HERO_RANKS[i-1].TYPES_CONT[ii];
				for (int iii=0; iii<HERO_RANKS[i].TYPES_CONT[ii].TN; iii++){
					a_DS.ExtractWord(a_DS2);
					HERO_RANKS[i].TYPES_CONT[ii].TYPES[iii].maxSquards=atoi(a_DS2.str);
				}
			}
			
			a_DS.ExtractWord(a_DS2);
			for (int iii=0; iii<atoi(a_DS2.str); iii++){
				SSquardData	dsT;
				a_DS.ExtractWord(a_DS3);
				dsT.unitName = a_DS3;
				a_DS.ExtractWord(a_DS3);
				dsT.maxSquards=atoi(a_DS3.str);
...
void	CHERO_MINI_MENU::UpdateMenu()
{
	...
	if( lpszName.str && lpszName != "NONAME" ) {
		BigMapSetMessage( pgptbName , lpszName.str ) ;
	}
	if( lpszRank.str && lpszRank != "NORANK" ) {
		BigMapSetMessage( pgptbRank , lpszRank.str ) ;
	}
	...
	DYNHINT_RA( ptbSQNum , a_Hint.str );
	...
	if( m_pHero && ptbVictN ) {
		...
		a_Hint += GetTextByID("#CHINT_Victories");			DYNHINT_RA( ptbVictN , a_Hint.str );
	}
	...
	DYNHINT_RA( ptbExpa	 , a_Hint.str );
	DYNHINT_RA( pgppExpBack , a_Hint.str );
}
//----------------------------------------------
...
void	CHERO_MINI_MENU::SetStat( int stat ) {
	...
	if( 0 <= stat && stat <= MAX_H_STATUS ) {
		...
		a_Hint += tRed;		a_Hint += inStat;							DYNHINT_RA(pgppStat[0], a_Hint.str);
	}
}
//----------------------------------------------
void	CHERO_MINI_MENU::SetFlag( int flag ) {
	if( 0 <= flag && flag <= MAX_PLAYERS+1 ) {
		...
		if( PID >= 0 && PID < MAX_PLAYERS && ROOT_SCENARIO.m_pPlayer[ PID ] ) { 
			...
			DYNHINT_RA(pgppFlag[0], a_Hint.str);
		}
	}
}
//----------------------------------------------
					...
					for(int i=0; i < nn ; i++) { // просмотр секторов-соседей. 
						...
						{ 
							...
							if( sabID >= 0 ) { // if( done )
								...
								//ShowBigMapGlobalMessage(1,2, 11, txt.str , 0);
								done = true;
...
void	CROOTDRIVER::AddStatisticsInfo(CResource_BM& OldResAmount)
{
	...
	m_pDipl->AddMessage( -100, 0, picID , "_" , pMessHeads->Get( 11 ) , text.str );
}
//----------------------------------------------
 		...
 		if( this->m_bSaveLogs && ( LastKey == 'X' || LastKey == 'x' || m_CheatsStage > start0 ) ) {		
			...
			if( m_CheatsStage < start0 ) { // enter
				...
				if( m_CheatsStage == start0+5 && ((LastKey >= 0x30 && LastKey < 0x30 + MAX_RANKS) || LastKey == VK_BACK ) ) //+			
				{
					...
					text += "\\ {CG}done.";						
					if(m_ptbText)	m_ptbText->SetMessage( text.str );
				}
...
void	CPERSONAL::setFarme( DialogsSystem* pMenu, LocalGP* pFR , CArmyLimit* pAL )
{
	...
	if ( pFR!=NULL ) 
	{
		...
		for(int i=0; i < m_nBT ; i+=2 ) {
			...
			DYNHINT_RA( m_pgpbButton[i] , a_HintDel.str );
			...
			DYNHINT_RA( m_pgpbButton[i+1] , a_HintAdd.str );
		...
		DYNHINT_RA( m_pgpbPages[0] , a_Hint.str );
		a_Hint.DString::Clear();		
		a_Hint = "{FS}{C FF502515}";				a_Hint += GetTextByID( "#CPT_Cavalry" );		
		DYNHINT_RA( m_pgpbPages[1] , a_Hint.str );
		a_Hint.DString::Clear();
		a_Hint = "{FS}{C FF502515}";				a_Hint += GetTextByID( "#CPT_Artillery" );		
		DYNHINT_RA( m_pgpbPages[2] , a_Hint.str );
	}
	...
	if( pH ) {
		for(int sq=0 ; sq < pH->ARMY.GetAmountOfElements() ; sq++)
			if( pH->ARMY[ sq ]->getTypeGlobID() == NIndex ) {
				...
				//a_Hint += tRed;
				//a_Hint += "     ";
				//a_Hint += pH->ARMY[sq]->TypeName.str; 
				//a_Hint += "\\ ";									// a_Hint += " \\  \\ " ;
				a_Hint  = tBlack;
				a_Hint += a_MonChar;
				a_Hint += " \\ ";
...
{ 
	...
	if( ptbHeroDesc && m_CurHeroId != id ) {
		...
		ptbHeroDesc->SetMessage( text.str ) ;
...
void		CTYPENUM::SetAll(const _str& TypeName, const int n ){
	setTypeName( TypeName );
	setTypeIndx( g_pALL_UNIT_TYPE->Get( TypeName.str ) );
	setNSquard ( n );
}
//----------------------------------------------
...
void	CSAVE_MIS_STRUCT::operator +=( const CMISSIONINIT& MIS_INT ){
    ...
    if (n>0) {
		for ( int i=0; i<n; i++ ){
			...
			str = (pCT->getTypeName())->str;
			pTE = createSquard( str.str, pCT->getNSquard() );
...
void	CHeroPersDesk::Refresh( CHero* pHero , int num )
{
	...
	if( ROOT_SCENARIO.m_pPlayer[i] && ROOT_SCENARIO.m_pPlayer[i]->m_Alive && ROOT_SCENARIO.m_pPlayer[i]->m_phrHero[ 0 ].m_Alive )
	{
		...
		a_Hint += GetTextByID("#CHINT_Victories");			DYNHINT_RA( ptbVictN[i] , a_Hint.str );
		...
		a_Hint += GetTextByID("#SquadsNum");	a_Hint += " / ";		a_Hint += GetTextByID("#MaxSquadsNum");	  
		DYNHINT_RA( ptbSQNum[i] , a_Hint.str );
		...
		DYNHINT_RA( ptbExpa[i] ,	 a_Hint.str );
		DYNHINT_RA( pgppExpBack[i] , a_Hint.str );
		...
		DYNHINT_RA( ptbFlag[i] , a_Hint.str);
		// awards hint
		a_Hint.DString::Clear();	
		a_Hint = "{FS}";	
		a_Hint += tBrown;	a_Hint += GetTextByID("#CWT_Quantity");	a_Hint += " ";	
		a_Hint += tRed;		a_Hint += SNN;				
		DYNHINT_RA( ptbStat[i] , a_Hint.str);
	}
	SetCoords( num ) ;
}
//----------------------------------------------------------------------------------
...
void	CResPanel_BM::CreateElements()
{
	...
	{ 
		...
		DYNHINT_RA( pgppResPic[ curR ] , a_Hint.str );		DYNHINT_RA( pgppResTxt[ curR ] , a_Hint.str );
	}
}
//----------------------------------------------------------------------------------
...
void	CMarketDesk::Refresh( CPlayer* pPlayer[MAX_PLAYERS] , int Nation )
{
	...
	if( NI >= 0 && NI < MAX_PLAYERS && m_ResSell >= 0 && m_ResSell <= COAL )
	{
		...
		if( m_SellAmount > pPlayer[ NI ]->GetResource( m_ResSell ) ) {
			...
			m_ShowActText = true;	
			ptbActionMess->SetMessage( a_text.str );
		}
		//
	...
	for(int i=0; i < MAX_PLAYERS ; i++)
		if( i != Nation && pPlayer[i]->m_Alive ) {
			...
			if( add && pPlayer[i] ) {
				text += "." ;
				sprintf( tmp, text10 , pPlayer[i]->GetPlayerName() , text.str );
				mess += tmp ;
				//mess +=  " \\  \\" ; 
				find = true ;
			}
	...
	if( find )   
		ptbMarketInfo->SetMessage( mess.str ) ;
}
//----------------------------------------------------------------------------------
...
bool	CBigMapHelp::Refresh( int page )
{
	...
	ptbHelpText->SetMessage( mess.str ) ;
