...
//		a_ptbName[i] = a_pddDesk->addGP_TextButton(NULL,dX0,dY0+35*i/*27*i*/,SAVE.m_pPlayer[i]->m_chName.str,a_lgpNameButton.GPID,4/*0*/,&WhiteFont,&YellowFont /*&BlackFont*/);//NEED NEW FONTS
...
//					//SAVE.m_pPlayer[0].m_iNation     = ActiveNation;	//SAVE.m_pPlayer[0].m_iDifficulty = ActiveDiff;	//SAVE.m_pPlayer[0].m_iFirstHeroId     = heroId;	//SAVE.m_pPlayer[0].m_chName.str = znew(char,strlen(NewName)+1);	//strcpy( SAVE.m_pPlayer[0].m_chName.str, NewName );
//
...
//			a_ptbNameNew->Enabled = false; a_ptbNameDel->Enabled = false; a_ptbNameOk->Enabled = false; a_ptbNameCancel->Enabled = false; a_pddDesk->Enabled = false; for(int i=0; i<Names; i++) { a_ptbName[i]->Enabled = false;; }
//
//			if(PlayerIntrfDeletePlayer(&a_dsMenu, SAVE.m_pPlayer[Active]->m_chName.str, SAVE.m_pPlayer[Active]->m_iNation)) 
//			{
