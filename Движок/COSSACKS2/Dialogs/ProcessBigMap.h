...
char* CutStrForMaxLen(char* St, int MaxLen, RLCFont* font)
{
	...
	int cutL = 0, TLen=0;
	do {
		TLen = GetRLen( text.str , font ) - 1;
		cutL = strlen( text.str ) -1 ; // -5 -4; 
...
bool	CstrID::operator==( const char*	UID ){
	...
	return ( strcmp( UID.strID.str, UID ) == 0 );
};
//------------------------------------------------------------------------
bool	CstrID::operator!=( const char*	UID ){
	return !( *this == UID );
...
bool	CstrID::GetEmpty(){ 
	if ( strID.str == NULL )	return true;
	return ( strcmp( strID.str, EMPTY_str ) == 0 );
};
//////////////////////////////////////////////////////////////////////////
...
bool	ProcessBigMap(int campainID)
{
	...
	for(int i=0; i < 5 ; i++) {	
		...
		a_Hint.DString::Clear();  a_Hint+="{FS}{C FF502515}";  a_Hint+=a_chDBHint[i];  DYNHINT(a_pgptbWM,a_Hint.str);
		//if( i == 4 ) {
...
BMlabel:
	...
	//_str testtxt;
	//testtxt = CutStrForMaxLen( pSAVE->m_chName.str , 100 , ptbBigMapHead->PassiveFont );

	int   HeadLimit = 535; //450-550;
	int   cutL = 0, Tlen=0, Flen = ptbBigMapFlag->x1 - ptbBigMapFlag->x;
	char  head[256], name[256];												
	strcpy( name , pSAVE->m_chName.str );
	do {
	...
	{ _str	a_Hint , a_Hint2 ;	
	 a_Hint  += "{FS}{C FF502515}";  a_Hint  += GetTextByID("#CHINT_EndOfTurn");	DYNHINT(pgpbOK, a_Hint.str );
	 a_Hint2 += "{FS}{C FF502515}";  a_Hint2 += GetTextByID("#CHINT_Quit");			DYNHINT(pgpbNO, a_Hint2.str); }
...
void	ProcessMissionForPlayer(int _campainID_, bool needsave )
{
	...
	if( a_inASID != -1 ) 
	{
		...
		for(int i=0; i < EndMissionData.AttackerArmy.GetAmountOfElements() ; i++) {
			RegisterUnitType( &Type, EndMissionData.AttackerArmy[i]->TypeName_sID.str );
		...
		for(int i=0; i < EndMissionData.DefenderArmy.GetAmountOfElements() ; i++) {
			RegisterUnitType(&Type, EndMissionData.DefenderArmy[i]->TypeName_sID.str );
...
bool	ProcessBigMapBriefing()
{
	...
	do {
		...
		text += a_DS2.str ;
		if( a_DS2 == "[END]" || a_DS2.str[0] == 0 )  quit = true; // end
	...
	a_Hint1+=GetTextByID("#CHINT_StartMis"); a_Hint1+=" "; a_Hint1+=GetTextByID("#CHINT_ENTERKEY"); DYNHINT(pgpbOK,a_Hint1.str);
	a_Hint2+=GetTextByID("#CHINT_StepBack"); a_Hint2+=" "; a_Hint2+=GetTextByID("#CHINT_ESCKEY");   DYNHINT(pgpbNO,a_Hint2.str);
	...
	//CHK->Message = znew(char,strlen( a_text.str )+1);
	//strcpy(CHK->Message,a_text.str); //"ARCADE"
...
bool	ProcessBigMapStatistics()
{
	...
	TextButton* ptbAttackName  = pddDesk1->addTextButton(NULL, 78, 368, a_name.str ,&fonMenuTitle2[1],&fonMenuTitle2[1],&fonMenuTitle2[1],0);//NEED NEW FONTS!!! &fonMenuTitle[1]
	...
	if( pDefendHero ) {
		...
		TextButton* ptbDefendName  = pddDesk2->addTextButton(NULL, 78, 368, a_name2.str ,&fonMenuTitle2[1],&fonMenuTitle2[1],&fonMenuTitle2[1],0);//NEED NEW FONTS!!!
	...
	ptbExpVal1->SetMessage( ExpsTxt.str ) ;
	...
	if( pDefendHero ) {
		...
		ptbExpVal12->SetMessage( ExpsTxt2.str ) ;
	...
	{ 
		...
		pgppIncRes->SetMessage( incRes.str );
		//			- out
		...
		pgppOutRes->SetMessage( outRes.str );
		//			- res amount
		_str  Total;
		...
		if( MissionInitData.AttackerNation_ID == gi_bmSAVE_Nation || 
			(MissionInitData.AttackerNation_ID != gi_bmSAVE_Nation && tR > 999 ) )  pgppTotal->SetMessage( Total.str );
		//pgppTotal->SetMessage( Total.str );
		...
		if( pDefendHero ) {
			...
			pgppIncRes2->SetMessage( incRes2.str );
			//			- out
			...
			pgppOutRes2->SetMessage( outRes2.str );
			//			- res amount
			_str  Total2;
			...
			if(  MissionInitData.DefenderNation_ID == gi_bmSAVE_Nation  || 
				(MissionInitData.DefenderNation_ID != gi_bmSAVE_Nation && tR > 999 ) ) pgppTotal2->SetMessage( Total2.str );
			//pgppTotal2->SetMessage( Total2.str );
		}
	}
	...
	for(int br=0; br < 3 ; br++) 
		...
		if( sqN0 > sqN1 && uid >=0 ) {	/*find &&*/
			...
			if( fid != 0xFFFF && sid != 0xFFFF ) { 
				...
				a_hint += "{FS}{CR}";   a_hint += NATIONS[0].Mon[uid]->Message;   DYNHINT(UPic, a_hint.str );	
				GPPicture*   UFrame	= pddKill->addGPPicture(NULL, UX +1 , UY +1 , lgpElems.GPID, 8 ) ;
				_str  SQNTxt;	
				SQNTxt += ( sqN0 - sqN1 ) ;
				TextButton*	 SQNum	= pddKill->addTextButton(NULL, UFrame->x+(UFrame->x1-UFrame->x+4)/2, UFrame->y1 -14 ,SQNTxt.str,Rfont,Rfont,Rfont, 1 );
				//TextButton*	 SQNum	= pddKill->addTextButton(NULL,UX +32 /*31*/, UFrame->y1 - 14 /*15*/,SQNTxt.str,Rfont,Rfont,Rfont, 1 );
				UN++;
				UX = UFrame->x1 + 17 ;
	...
	TextButton* ptbTotalLost = pddDesk1->addTextButton(NULL, 916, 368 , LostTxt.str ,Rfont,Rfont,Rfont,2);
	...
	if( pDefendHero ) {
		...
		for(int br=0; br < 3 ; br++) 
			...
			if( sqN0 > sqN1 && uid >=0 ) {
				...
				if( fid != 0xFFFF && sid != 0xFFFF ) { 
					...
					TextButton*	 SQNum	= pddKill2->addTextButton(NULL, UFrame->x+(UFrame->x1-UFrame->x+4)/2, UFrame->y1 -14 ,SQNTxt.str,Rfont,Rfont,Rfont, 1 );
					//TextButton*	 SQNum	= pddKill2->addTextButton(NULL,UX +32, UFrame->y1 - 14 ,SQNTxt.str,Rfont,Rfont,Rfont, 1 );
					UN++;
					UX = UFrame->x1 + 17 ;
					if( UN > 4 )  { UN = 0 ;  UX = 15 ;  UY += 130 ; }
					TotalLost += ( sqN0 - sqN1 ) ;
		...
		TextButton* ptbTotalLost = pddDesk2->addTextButton(NULL, 916, 368, LostTxt2.str ,Rfont,Rfont,Rfont,2);
	}
	...
	a_hint += pgpbOK->Message;  a_hint+=" ";  a_hint += GetTextByID("#CHINT_ESCKEY");  DYNHINT(pgpbOK,a_hint.str);
...
void	ProcessBigMapPlayVideo( int id /*= -1*/ )
{
	...
	KeyPressed = LastKey = 0;   //Global
	ItemChoose = - 1;			//Global
	PlayFullscreenVideo( name.str , 0 , 0.17 ); // bik resolution = 800x448
	UnPress();					//Global
	KeyPressed = LastKey = 0;   //Global
	ItemChoose = - 1;			//Global
...
{	
	...
	if(!a_DS.str) return;//VERY NEW //FOR FUNTASIA
	...
	do
	{
		...
		// skip line
		if( a_DS2.str[0] == 47 /*"/"*/ ) { a_DS.ExtractLine(a_DS2); line++; continue; }
		...
		for(sec=0; sec < 24 ; sec++) { 
			...
			if( a_DS2 == name ) {
				for(int dir=0; dir < 8 ; dir++)	{	//BriefDIR_XY[24][8][4]; // [secid][dir][xy]
					a_DS.ExtractWord(a_DS2);
                    BriefDIR_XY[ sec ][dir][0] = atoi(a_DS2.str) ; // x0  A
					a_DS.ExtractWord(a_DS2);
                    BriefDIR_XY[ sec ][dir][1] = atoi(a_DS2.str) ; // y0  A
					a_DS.ExtractWord(a_DS2);
                    BriefDIR_XY[ sec ][dir][2] = atoi(a_DS2.str) ; // x0  D
					a_DS.ExtractWord(a_DS2);
                    BriefDIR_XY[ sec ][dir][3] = atoi(a_DS2.str) ; // y0  D
					line++;
				}
				break;
			}
...
{	
	...
	do
	{
		...
		// start resources
		if( a_DS2 == "#FOOD_RATIO" )   { 
			a_DS.ExtractWord(a_DS2);	ROOT_SCENARIO.m_iBigMap_FOOD_RATIO = atoi(a_DS2.str); 
		}
		if( a_DS2 == nameAI )   { 
			a_DS.ExtractWord(a_DS2);	ROOT_SCENARIO.m_iBigMap_AI___RESOURCES	= atoi(a_DS2.str); 
		}
		if( a_DS2 == namePL )   { 
			a_DS.ExtractWord(a_DS2);	ROOT_SCENARIO.m_iBigMap_User_RESOURCES	= atoi(a_DS2.str); 
		}
		// делитель для дохода ресурсов - f(diff)
		for(int i=0; i < DIFF_LEVELS_N ; i++) {
			sprintf(txt, "#Res_Div_%d_f" , i );
			if( !strcmp( a_DS2.str , txt ) ) {
				a_DS.ExtractWord(a_DS2);	ROOT_SCENARIO.UpdateResDiv[i] = atof(a_DS2.str);
			}
		}
		// доход от сектора на каждом ходу
		if( a_DS2 == "#SECT_INCOME_WOOD" )   { 
			a_DS.ExtractWord(a_DS2);	SECT_INCOME[ WOOD ]	= atoi(a_DS2.str); 
		}
		if( a_DS2 == "#SECT_INCOME_FOOD" )   {
			a_DS.ExtractWord(a_DS2);	SECT_INCOME[ FOOD ] = atoi(a_DS2.str); 
		}
		if( a_DS2 == "#SECT_INCOME_STONE" ) {
			a_DS.ExtractWord(a_DS2);	SECT_INCOME[ STONE] = atoi(a_DS2.str);
		}
		if( a_DS2 == "#SECT_INCOME_GOLD" ) {
			a_DS.ExtractWord(a_DS2);	SECT_INCOME[ GOLD ] = atoi(a_DS2.str);
		}
		if( a_DS2 == "#SECT_INCOME_IRON" ) {
			a_DS.ExtractWord(a_DS2);	SECT_INCOME[ IRON ] = atoi(a_DS2.str);
		}
		if( a_DS2 == "#SECT_INCOME_COAL" ) {
			a_DS.ExtractWord(a_DS2);	SECT_INCOME[ COAL ] = atoi(a_DS2.str);
		}
		// доход от основного ресурса сектора
		if( a_DS2 == "#SECT_ADDING_FOOD" ) {				
			a_DS.ExtractWord(a_DS2);	SECT_ADDING[ FOOD ] = atoi(a_DS2.str);
		}
		if( a_DS2 == "#SECT_ADDING_GOLD" ) {
			a_DS.ExtractWord(a_DS2);	SECT_ADDING[ GOLD ] = atoi(a_DS2.str);
		}
		if( a_DS2 == "#SECT_ADDING_IRON" ) {
			a_DS.ExtractWord(a_DS2);	SECT_ADDING[ IRON ] = atoi(a_DS2.str);
		}
		if( a_DS2 == "#SECT_ADDING_COAL" ) {
			a_DS.ExtractWord(a_DS2);	SECT_ADDING[ COAL ] = atoi(a_DS2.str);
		}
		// макс.кол-во рекрутов в секторе
		if( a_DS2 == "#SECT_MAX_RECRTS" ) {
			a_DS.ExtractWord(a_DS2);	SECT_MAX_RECRTS		= atoi(a_DS2.str);
		}
		// восстановление рекрутов в секторе
		if( a_DS2 == "#SECT_REG_RECRTS" ) {
			a_DS.ExtractWord(a_DS2);	SECT_REG_RECRTS		= atoi(a_DS2.str);
		}
		// затраты на defence сектора
		if( a_DS2 == "#SECT_DEFENCE_WOOD" ) {
			a_DS.ExtractWord(a_DS2);	SECT_DEFENCE[ WOOD  ] = atoi(a_DS2.str);
		}
		if( a_DS2 == "#SECT_DEFENCE_FOOD" ) {
			a_DS.ExtractWord(a_DS2);	SECT_DEFENCE[ FOOD  ] = atoi(a_DS2.str);
		}
		if( a_DS2 == "#SECT_DEFENCE_STONE" ) {
			a_DS.ExtractWord(a_DS2);	SECT_DEFENCE[ STONE ] = atoi(a_DS2.str);
		}
		if( a_DS2 == "#SECT_DEFENCE_GOLD" ) {
			a_DS.ExtractWord(a_DS2);	SECT_DEFENCE[ GOLD  ] = atoi(a_DS2.str);
		}
		if( a_DS2 == "#SECT_DEFENCE_RECRTS" ) {
			a_DS.ExtractWord(a_DS2);	SECT_DEFENCE[ RECRT ] = atoi(a_DS2.str);
		}
		if( a_DS2 == "#SECT_DEF_mult2_f" ) {
			a_DS.ExtractWord(a_DS2);	SECT_DEF_multiplier2 = atof(a_DS2.str);
		}
		if( a_DS2 == "#SECT_DEF_mult3_f" ) {
			a_DS.ExtractWord(a_DS2);	SECT_DEF_multiplier3 = atof(a_DS2.str);
		}
		...
		for(int i=0; i < 4 ; i++) {
			...
			if( !strcmp( a_DS2.str , txt ) ) {
				...
				for(int k=0; k < 4 && !done ; k++) {
					a_DS.ExtractWord(a_DS2);	SECT_SABOTAGE[i][k] = atoi(a_DS2.str);		
					if( atoi(a_DS2.str) == 0 )  done = true; 
				}
			}
			...
			if( !strcmp( a_DS2.str , txt ) ) {
				bool  done = false;
				for(int k=0; k < 4 && !done ; k++) {
					a_DS.ExtractWord(a_DS2);	SECT_SAB_PERCENT[i][k] = atoi(a_DS2.str);		
					if( atoi(a_DS2.str) == 0 )  done = true; 
				}
			}
		}
		...
		if( a_DS2 == "LTN_EXPS_LIMIT" ) {
			a_DS.ExtractWord(a_DS2);	LTN_EXPS_LIMIT		= atoi(a_DS2.str);
		}
		if( a_DS2 == "CPT_EXPS_LIMIT" ) {
			a_DS.ExtractWord(a_DS2);	CPT_EXPS_LIMIT		= atoi(a_DS2.str);
		}
		if( a_DS2 == "MJR_EXPS_LIMIT" ) {
			a_DS.ExtractWord(a_DS2);	MJR_EXPS_LIMIT		= atoi(a_DS2.str);
		}
		if( a_DS2 == "COL_EXPS_LIMIT" ) {
			a_DS.ExtractWord(a_DS2);	COL_EXPS_LIMIT		= atoi(a_DS2.str);
		}
		if( a_DS2 == "GEN_EXPS_LIMIT" ) {
			a_DS.ExtractWord(a_DS2);	GEN_EXPS_LIMIT		= atoi(a_DS2.str);
		}
		if( a_DS2 == "MAR_EXPS_LIMIT" ) {
			a_DS.ExtractWord(a_DS2);	MAR_EXPS_LIMIT		= atoi(a_DS2.str);
		}
		// Exp. in Miss 
		if( a_DS2 == "DisbandSquadExp" ) {
			a_DS.ExtractWord(a_DS2);	DisbandSQExp		= atoi(a_DS2.str);
		}
		if( a_DS2 == "SectCaptureExp" ) {
			a_DS.ExtractWord(a_DS2);	SecCaptExp			= atoi(a_DS2.str);
		}
		if( a_DS2 == "HeroDefeatExp" ) {
			a_DS.ExtractWord(a_DS2);	HeroDefExp			= atoi(a_DS2.str);
		}
		// quest exp.
		if( a_DS2 == "QuestDisbSquadExp" ) {
			a_DS.ExtractWord(a_DS2);	QuestDisbSquadExp	= atoi(a_DS2.str);
		}
		if( a_DS2 == "QuestCompliteExp" ) {
			a_DS.ExtractWord(a_DS2);	QuestCompliteExp	= atoi(a_DS2.str);
		}
		if( a_DS2 == "MISS_FRAG_LIMIT" ) { // BigMap NKills Limit for squads
			a_DS.ExtractWord(a_DS2);	gi_bmMISS_FRAG_LIMIT = atoi(a_DS2.str);
		}
		...
		for(int i=0; i < MAX_PLAYERS+1 ; i++) {
			...
			if( !strcmp( a_DS2.str , txt ) ) {
				a_DS.ExtractWord(a_DS2);
				ROOT_SCENARIO.m_iNationID[i] = atoi(a_DS2.str);
			}
		}
		...
		for(int i=0; i <= MAX_DEFENCE ; i++) {
			...
			if( !strcmp( a_DS2.str , txt ) ) {
				a_DS.ExtractWord(a_DS2);
				ROOT_SCENARIO.m_iGarrSqN[i] = atoi(a_DS2.str);
			}
			sprintf(txt, "SectGarrisonAdd%d" , i );
			if( !strcmp( a_DS2.str , txt ) ) {
				a_DS.ExtractWord(a_DS2);
				ROOT_SCENARIO.m_iGarrAdd[i] = atoi(a_DS2.str);
			}
			sprintf(txt, "SectRevoltedSquads%d" , i );
			if( !strcmp( a_DS2.str , txt ) ) {
				a_DS.ExtractWord(a_DS2);
				ROOT_SCENARIO.m_iRebelAdd[i] = atoi(a_DS2.str);
			}
		}
		if( a_DS2 == "MinSectDefSquadPower" ) {
			a_DS.ExtractWord(a_DS2);	ROOT_SCENARIO.m_iMinSecDefPower = atoi(a_DS2.str);
		}
		if( a_DS2 == "AddSectDefSquadPower" ) {
			a_DS.ExtractWord(a_DS2);	ROOT_SCENARIO.m_iAddSecDefPower = atoi(a_DS2.str);
		}
		...
		//if( a_DS2 == "#" ) {
		//	a_DS.ExtractWord(a_DS2);	 = atoi(a_DS2.str);
		//}
...
bool	OnDiversionClicked(SimpleDialog* pSD)
{
	...
	if( ROOT_SCENARIO.m_pPlayer[ gi_bmSAVE_Nation ]->GetResource( GOLD ) >= money ) {
		...
		//sprintf(text, GetTextByID("#CWM_NoEnoughRes") , a_Cost.str );
