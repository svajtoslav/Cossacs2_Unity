...
public:
	...
	void		FlushBatches		() { ISM->FlushBatches(); FlushLines(); }
	void		FlushLines			();
	...
	int			SetCurrentShader	( int shaderID	)			{ return ISM->SetCurrentShader( shaderID ); }
	...
	void 		EnableZBuffer		( bool bEnable = true )		{ ISM->EnableZBuffer( bEnable ); rsEnableZ( bEnable ); }
