...
int carr_CapturableZone::ExpandWith(const char* ElmName,void* base){	
	...
	if(id!=-1 && EditMapMode/*&&(PlayGameMode||EditMapMode)*/){
		...
		if(arr){			
			...
			ActiveZone* AZ=NULL;			
			for(int i=0;i<AZones.GetAmount();i++)if(AZones[i]->Name==z->str){
				AZ=AZones[i];				
				break;
			}
			if(!AZ){
				...
				int y=(mapy<<5)+RealLy;
				int zid=CreateNewZone(z->str,x,y,500); //1000+id*200,1000+id*200
				AZ=AZones[zid];
				if(AZ){
...
void VC_TheKingOfTheHill::InitOnCurrentMap(){
	...
	Reset();
	for(int i=0;i<AZones.GetAmount();i++)if(!stricmp(AZones[i]->Name.str,"KINGOFTHEHILL")){
		xc=AZones[i]->x;
		yc=AZones[i]->y;
		r=AZones[i]->R;
	}
