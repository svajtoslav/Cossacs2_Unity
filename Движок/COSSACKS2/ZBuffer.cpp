...
void AddAnimation(int x,int y,int z,NewAnimation* ANM,int Frame,float Dir,DWORD Diffuse,OneObject* OB,float Scale,float fiDir,float fiOrt,DWORD Handle){
	...
	int XL=(x>>zoomsh)-(mapx<<(5-zoomsh));	
	int YL=(y>>(1+zoomsh))-(mapy<<(4-zoomsh))+DYZBuf;
	if(YL<0){
...
void AddWorldPoint(int xc,int yc,int zc,int dx,int dy,OneObject* OB,word FileID,word SpriteID,DWORD Diffuse,bool AllowRotation,float ScaleX,float ScaleY,float RotateAngle){
	if(NZElm>=MAXZELM)return;
...
void AddOptPoint(byte Method,short XL,short YL,short x,short y,OneObject* OB,word FileID,word Sprite,int Options){
	...
	case 2:
		...
		break;
	default:
		AddPoint(XL,YL,x,y,OB,FileID,Sprite,Param1,Param2,0);
		break;
	};
};
...
void ShowZElement(ZElement* ZEL)
{
	...
	if(ZEL->ShowAnimation){
		...
		ENDPROF;
		OneObject* OB=ZEL->OB;
		if(OB&&CINFMOD&&OB->NewBuilding&&!LMode){
