	...
	if(gMessagesMap.GetNMess() > 0) {
		...
		if( pBaseMes && pBaseMes->GetNMess() > 0 ) 
		{
			...
			for(int i=0; i<pBaseMes->GetNMess() && MN < 0 && ParentName ;i++) 
				if( !strcmp(pBaseMes->Messages[i]->Object_Name.str,ParentName) )  MN = i;
			...
			if( pParentMes ) 
			{
				...
				return  gMessagesMap.AddMessage( pMess , pBaseMes->Object_Name.str /*"Base"*/ ) ; //return  gMessagesMap.AddMessage( pMess , ParentName ) ;
			}
		}
	}
	return  false;
...
//	pLocalParent->AddMessage(pMessage);
	if (ParenName!=NULL&&strcmp(Object_Name.str,ParenName)==0){
		AddMessage(pMessage);
		return true;
	};
...
bool			BaseMesMgrST::CompareName(char* ObjectName){
	return (strcmp(ObjectName,Object_Name.str)==0);
};
//////////////////////////////////////////////////////////////////////////
...
void	C2CampMesMgr::ClearDeleted(){
	...
	if (TypeObj==0) {
		...
		while (empty&&N--) {
			...
			if( !strcmp( Messages[N]->Object_Name.str , this->Object_Name.str ) )	empty=false;//grey
...
void	C2CampMesMgr::ProcessMessActions()
{
	switch(ID) {
		case 0xADD:		// Ress (add res/info)
			ShowPageParam( PageName.str , text.str );
			break;
		case 0xDEF:		// Quest
