	...
	if(OB->DestX>0){
		...
		if(DIS>64){
			...
			if(abs(ddir)<2){
				...
				OB->RealVx=((OB->DestX-OB->RealX)*spf/DIS);
				OB->RealVy=((OB->DestY-OB->RealY)*spf/DIS);
...
bool CalculateMotion2Discret(OneObject* OB){
	NewMonster* NM=OB->newMons;
	int MaxV=NM->MotionDist;
	GeneralObject* GO=OB->Ref.General;
	AdvCharacter* ADC=GO->MoreCharacter;
	...
	if(OB->DestX>0){
		...
		if(DIS>64){
			...
			char DIR1=char(GetDir(OB->DestX-OB->RealX,OB->DestY-OB->RealY));
			int NR=NM->GetAnimation(anm_MotionL)->Rotations;
			if(NR>1&&NR&1){
				int oct=64/(NR-1);
				int oct2=oct<<1;
				DIR1=(((DIR1+oct+256)/oct2)*oct2)&255;
			}else{
			};
...
CalcMotion:
		if(OB->Speed){
			int NSp=NMN->GetAnimation(anm_Stand)->NFrames;
			int NewX;
			int NewY;
			...
			if(OB->GLock)MFI->ClearRound(OldLockX,OldLockY,LX);
			if(OB->LLock)UnitsField.ClearRound(OldLockX,OldLockY,LX);
			...
			if(AllowGMotion&&AllowLMotion){
				...
				MoveObject(OB);				
				if(!OB->UnlimitedMotion)UnitsField.BSetBar(NewLockX,NewLockY,LX);
				OB->x=NewLockX;
				OB->y=NewLockY;
				...
				if(MRes){
					...
					if(AllowGMotion){
						...
						if(!OB->UnlimitedMotion)UnitsField.BSetBar(OldLockX,OldLockY,LX);
						OB->LLock=true;
					}else{
						///if(FirstTime){
						//	CanCrashLimit=CalculateMotion2Limited(OB);
