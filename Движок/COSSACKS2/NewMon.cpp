...
void InitNewMonstersSystem(){
	...
	CBB_GPFILE=GPS.PreLoadGPImage("Interf3\\elements\\combo");
	LoadMapInterface();
...
NewAnimation* NewMonster::LoadNewAnimationByName(char* gx,int Index){
	...
	DWORD V=E->Get(gx+1);
	if(V==0xFFFFFFFF)return NULL;
	NewAnimation* NA=Animations.Get(V,Index);	
	return NA;
	/*
...
void NewMonster::InitNM(char* name){
	...
	if(!strcmp(Message,gx))Message="";
	LongMessage=GetTextByID(gx);
	if(!strcmp(LongMessage,gx))LongMessage="";
	DstZPoint=30;
				...
				if(gx[0]=='$'){
					...
					case 'B':						
						...
						if(!strcmp(gx,"BREAKANIMATION")){
							...
							if(z!=1)IncPar(name,Line,gx);
							{
								NewAnimation* NANM=LoadNewAnimationByName(gx,0);
								if(NANM){
									NANM->CanBeBroken=1;
								};
							}
					...
					case 'C':
						...
						if(!strcmp(gx,"CRAFTPOINTS2")){
							...
							for(int i=0;i<p1;i++){
								z=Gscanf(f1,"%d%d",&p2,&p3);
								if(z!=2)IncPar(name,Line,"CONCENTRATOR2");
								CraftPtX.Add(p2);
								CraftPtY.Add(p3<<1);
							};
							NLine(f1);
					...
					case 'E':
						if(!strcmp(gx,"EXTRALOCK")){
							...
							bool ExtraLock(word& NLockPt,byte*& LockX,byte*& LockY);
					...
					case 'M':
						...
						if(!strcmp(gx,"MAXINDEPO")){
							z=Gscanf(f1,"%d%d",&p1,&p2);
							if(z!=2)NEPar(name,Line,gx,1);
							if(p1>4)InvPar(name,Line,gx,"first parameter must be<5");
							NInArtDepot=p2;
							ArtSet=p1+1;
			                NLine(f1);
							Line++;
							...
							if(z==5){
								{
									...
									if(NA){
										...
										if(!AP->Particle)pErr("%s : PARTICLES : Unknown node %s for animation %s",name,node,anm);
                                        AP->Phase=Phase;
										AP->AverageDensity=Avr;
										AP->LinkedTo=LinkTo;
										NA->Particles.Add(AP);
									}else{
										pErr("%s : PARTICLES : Unknown animation %s",name,anm);
									}
								}
					...
					case 'S':
						...
						if(!strcmp(gx,"SETHOTFRAME")){
							...
							if(z!=3)IncPar(name,Line,gx);
							{
								...
								if(NANM&&NANM->AnimationID>0){
									...
									if(NANM->NFrames)IMM->Animate(NANM->ModelID,NANM->AnimationID,float(f)*AT/NANM->NFrames);
									else IMM->Animate(NANM->ModelID,NANM->AnimationID,0);
						...
						if(!strcmp(gx,"SOUND")){
							...
							if(z!=3)IncPar(name,Line,gx);
							{
								NewAnimation* NAN=LoadNewAnimationByName(gy,0);
								if(NAN){
									NAN->SoundID=SearchStrBack(SoundID,Fn,NSounds);
									if(NAN->SoundID==-1){
...
bool CreateGOByName(GeneralObject* GO,char* name,char* newName){
	...
	char ddd[128];
	Log.Error( "Could not create GeneralObject by name: %s",name);
	return false;
};
...
int  SprGroup::GetIndexByName(char* Name){
	...
	for(int i=0;i<GetNSprites();i++){
		if(Objects[i]->Name.str&&!strcmp(Objects[i]->Name.str,Name))return i;
	};
	return -1;
};
int GetIndexByName2(char* Name,int NSpr,ObjCharacter** ObjChar){	
	...
	for(int i=0;i<NSpr;i++){
		if(ObjChar[i]->Name.str&&!strcmp(ObjChar[i]->Name.str,Name))return i;
	};
	return -1;
};
					...
					case 21://[MODEL]
						{
							...
							if(z1!=-1){
								...
								if(idx!=-1){
									...
									sprintf(str,"%s : Unknown model ID :%s",fn,gy);
									ErrM(str);
								}
							}else{
								sprintf(str,"%s : Unknown sprite ID :%s",fn,gx);
...
void ShowVisualLess(int yend){
	...
	do{
		...
		if(ylog<yend){
            ...
            switch(par1){
            case AV_NORMAL:
				if(LocTrans)GPS.ShowGP(smapx+UnitX[fu],smapy+UnitY[fu],FID,spr,nat);
				else GPS.ShowGP(smapx+UnitX[fu],smapy+UnitY[fu],FID,spr,nat);
                break;
            case AV_PULSING:
                {
                    int NNN=5+int(8*(sin(double(GetTickCount())/100)+2));
					GPS.ShowGP(smapx+UnitX[fu],smapy+UnitY[fu],FID,spr,nat);
                };
                break;
            case AV_TRANSPARENT:
				GPS.ShowGP(smapx+UnitX[fu],smapy+UnitY[fu],FID,spr,nat);
	...
	if(NFir){
		...
		for(int i=0;i<FIN->RealFires;i++){
			...
			if(NANM&&NANM->NFrames){
				...
				PlayAnimation(NANM,csp<<8,x+(mapx<<5),y+(mapy<<4));
				...
				if(!OB->Sdoxlo){
					...
					if(ofst>=0&&ofst<VAL_MAXCIOFS){
						...
						if(k<MAXINCELL-1){
							MCount[ofst]++;
							TmpMC[ofst]++;
							if(!OB->Sdoxlo)SetNMSL_FAST(ofst1+k,OB->Index);
						}
					}
				}
			}
...
void TryToStand(OneObject* OB,bool rest){
	...
	if(OB->NewState){
		...
		//if(rest)
		if(OB->newMons->MotionStyle==7||OB->NewAnm&&OB->FrameFinished){
...
int TryToMove(OneObject* OB,byte NewDir,bool Dirc){
	...
	OB->CheckState();
	int nf=NMN->GetAnimation(anm_MotionL)->NFrames>>SpeedSh;
	...
	if(OB->MaxAIndex){
		if(Dirc){
			if(OB->LeftLeg){
				GO=NMN->GetAnimation(anm_PMotionLB+OB->CurAIndex);
			}else{
				GO=NMN->GetAnimation(anm_PMotionRB+OB->CurAIndex);
			};
			...
			if(OB->LeftLeg){
				GO=NMN->GetAnimation(anm_PMotionL+OB->CurAIndex);
			}else{
				GO=NMN->GetAnimation(anm_PMotionR+OB->CurAIndex);
			};
	...
	if(OB->NewState&&!GO){
		if(Dirc){
			//Back motion
			GO=NMN->GetAnimation(anm_PMotionLB+OB->NewState-1);
				if(GO->Enabled){
				if(!OB->LeftLeg)GO=NMN->GetAnimation(anm_PMotionRB+OB->NewState-1);
			}else GO=NULL;
		}else{
			GO=NMN->GetAnimation(anm_PMotionL+OB->NewState-1);
			if(GO->Enabled){
				if(!OB->LeftLeg)GO=NMN->GetAnimation(anm_PMotionR+OB->NewState-1);
			}else GO=NULL;
		};
	};
	if(OB->LocalNewState&&!GO){
		NewAnimation* NANM=NMN->GetAnimation(anm_UAttack+OB->LocalNewState-1);
		if(NANM->Enabled){
			OB->NewAnm=NANM;
			OB->SetZeroFrame();
			OB->InMotion=false;
	...
	if(!GO){
		if(Dirc){
			if(OB->LeftLeg)GO=NMN->GetAnimation(anm_MotionLB);
			else GO=NMN->GetAnimation(anm_MotionRB);
		}else{
			if(OB->LeftLeg)GO=NMN->GetAnimation(anm_MotionL);
			else GO=NMN->GetAnimation(anm_MotionR);
		};
		if(!GO->Enabled)GO=NMN->GetAnimation(anm_Stand);
	};
	//OB->Speed=16;
	if(retval==0){
		//bdx>>=1;
				...
				if(UMID!=0xFFFF){
					...
					if(OB&&!OB->Illusion){
						...
						if(!OB->Sdoxlo){
							...
							if(OB->ActivityState==1&&OB->StandTime>10){//peaceful
								if(OB->BrigadeID!=0xFFFF){
									if(!OB->StandGround){
										Brigade* BR=CITY[OB->NNUM].Brigs+OB->BrigadeID;	
							...
							if(OB->NewState!=5&&OB->FrameFinished&&OB->StandGround&&OB->BrigadeID!=0xFFFF&&OB->newMons->SitInFormations&&OB->BrIndex>=NBPERSONAL){
                                Brigade* BR=CITY[OB->NNUM].Brigs+OB->BrigadeID;
							...
							if(OB->ActivityState==2){//aggressive
								if(OB->BrigadeID==0xFFFF){
									OB->StandGround=0;
								}else{
									if(OB->StandGround){
...
int CheckMinePosition(NewMonster* NM,int* xi,int* yi,int r,bool Manual=true);
...
int CheckCreationAbility(byte NI,NewMonster* NM,int* x2i,int* y2i,word* BLD,int NBLD,bool Manual){	
	word TotBLD[512];
    ...
    if(NM->Building){
		...
		if(NM->ProdType){
			int xx=x2;
			int yy=y2;
            ID=CheckMinePosition(NM,&xx,&yy,r,Manual);
			*x2i=xx;
			*y2i=yy;
			x2=xx;
			y2=yy;
...
int CheckSmartCreationAbility(byte NI,NewMonster* NM,int* x2i,int* y2i){
	...
	int v=CheckCreationAbility(NI,NM,x2i,y2i,TotBLD,Nb,1);
	if(NM->Port)return v;
	if(v==-1){
		int x0=((*x2i)>>8)<<8;
		int y0=((*y2i)>>8)<<8;
		for(int r=1;r<10;r++){//10
			...
			for(int j=0;j<N;j++){
				...
				int xc0=xc;
				int yc0=yc;
				v=CheckCreationAbility(NI,NM,&xc,&yc,TotBLD,Nb,0);
				if(v!=-1){
					*x2i=xc;
					*y2i=yc;
					return v;
...
int CheckAISmartCreationAbility(byte NI,NewMonster* NM,int* x2i,int* y2i){
	...
	int Nb=0;
	if(NM->Building)Nb=CreateBLDList(NI,TotBLD,512,*x2i,*y2i);
	int v=CheckCreationAbility(NI,NM,x2i,y2i,TotBLD,Nb,0);
	if(NM->Port)return v;
	if(v==-1){
		int x0=((*x2i)>>8)<<8;
		int y0=((*y2i)>>8)<<8;
	...
	for(int r=0;r<50;r++){
		...
		if(rando()<16384){
			for(int j=0;j<N;j++){
				...
				if(!CheckCreationAbility(0xFF,NM,&x1,&y1,NULL,0,1)){
					*x=x1;
					*y=y1;
					return;
				};
...
int Nation::CreateNewMonsterAt(int rx,int ry,int n,bool Anyway,short Dir){
	...
	if(!(Anyway||NM->Wall)){
		...
		ID=CheckCreationAbility(NNUM,GO->newMons,&rx,&ry,NULL,0,1);
		addrand(ID);
		x=rx;
		y=ry;
        if(ID==-1)return -1;
...
//		MFIELDS[1].BSetBar(PortBuiX-1,PortBuiY+LL-1,3);
	...
	*/
	...
	G->UnitSpeed=64;
	G->CurUnitSpeed=64;
	G->GroupSpeed=G->newMons->MotionDist;//(NM->MotionDist*G->Ref.General->MoreCharacter->Speed)/100;
...
bool CanIStandHere(OneObject* OB)
{
	...
	{
		UnitsField.BClrBar(OB->x,OB->y,OB->Lx);
		if(UnitsField.CheckBar(OB->x,OB->y,OB->Lx,OB->Ly))
			rez=false;
...
void OneObject::NewMonsterPreciseSendTo(int x,int y,byte Prio,byte OrdType){
	...
	*/
	...
	Order1* Or1=CreateOrder(OrdType==0?3:OrdType);
	if(!int(Or1))return;
	//if(!(newMons->LockType==1||Sdoxlo||UnlimitedMotion||OPT)){
	...
	if(ActivityState==2&&!(Prio&127)){
		if(newMons->GetAnimation(anm_PMotionL)->Enabled&&!newMons->MaxAIndex)GroundState=1;
		addrand(332);
	}else{
		bool GS0=1;
		addrand(333);
							...
							bool	NotInForm )
{
	...
	if (abs( wr ) < 16 && abs( hr ) < 16)
	{
		...
		if(OB&&OB->newMons&&(!((OB->ImSelected&GM(NI))||OB->Hidden||OB->Sdoxlo||OB->UnlimitedMotion))
								&&((!FN)||FN(OB,NN)))
		{
			if(GetObjectVisibilityInFog(OB->RealX/16,OB->RealY/16,OB->RZ,OB)){
	...
	for(int i=0;i<MAXOBJECT;i++)
	{
		...
		//if (OB->NNUM != NIX) continue;		
		if (FN && !FN( OB, NN )) continue;		

		if((OB->BrigadeID!=0xFFFF&&InForm)||(OB->BrigadeID==0xFFFF&&NotInForm))
		{
			
			if (rctSel.Overlap( GetRect( OB ) ) && GetObjectVisibilityInFog(OB->RealX/16,OB->RealY/16,OB->RZ,OB) )
			{
				if (OB->NNUM != NIX) EnemyPresent=true;
				else MyPresent=true;
...
void BSetPt(int x,int y){
	MFIELDS->BSetPt(x,y);
	for(int i=2;i<NMFIELDS;i++) MFIELDS[i].BSetPt(x,y);
};
void BClrPt(int x,int y){
	MFIELDS->BClrPt(x,y);
	for(int i=2;i<NMFIELDS;i++) MFIELDS[i].BClrPt(x,y);	
};
		...
		if(abs(ddir)<rot2){
			...
			if(WP){
				...
				if(udir<uoc2){
					...
					y0=(OBJ->RealY>>4)+(UNAM->ActivePtY[udir1]+NF->dy+NMN->SrcZPoint)*2;
				}else{
...
bool OneObject::AttackObj(word OID,int Prio1,byte OrdType,word NTimes){
	...
	if(RE>=Ref.General->MoreCharacter->MaxR_Attack){
	...
	if(Ref.General->MoreCharacter->MaxR_Attack<160){
...
#endif
	StandTime=0;
	if(Ref.General->MoreCharacter->MaxR_Attack<180)ATTLIST.Add(this,EOB);
	return true;
};	
void NewAttackPointLink(OneObject* OBJ);
...
void AttackByBuildingLink(OneObject* OBJ){
	...
	if(OBJ->NewBuilding){
		...
		if(NMN->ShotDir){
			if(OBJ->NInside==0&&OBJ->Ref.General->MoreCharacter->MaxInside){
				OBJ->DeleteLastOrder();
				return;
			};
			bool UseEvro=0;
...
void AttackObjLink(OneObject* OBJ){
	...
	if(nst&&!DELAY){
		...
		addrand(Anm->Enabled);
		if(Anm->Enabled&&dstx<ADC->MaxR_Attack+150+OB->newMons->AddShotRadius&&dst>ADC->MinR_Attack-80){
			addname(OBJ->Index);
						...
						for(int q=0;q<NWP;q++){
							...
							if(NR&1||!NR){
								...
								if(udir<uoc2){
									...
									y0=(OBJ->RealY>>4);
									z0-=UNAM->ActivePtY[udir1+SWS]+NF->dy;
								}else{
									int udir1=uoc2-uocts+udir;
									x0=(OBJ->RealX>>4)+(UNAM->ActivePtX[udir1+SWS]+NF->dx);
									y0=(OBJ->RealY>>4);
								...
								z0-=UNAM->ActivePtY[udir+SWS]+NF->dy;
							};
							//----------
						...
						if(OB->BrigadeID!=0xFFFF){
							...
							if(HOB){
								if(HOB->Life<HOB->MaxLife){
									HOB->Life+=OBJ->Ref.General->MoreCharacter->MaxDamage[0];
									if(HOB->Life>=HOB->MaxLife){
	...
	if(OBJ->FrameFinished||(OBJ->NewAnm->CanBeBroken&&OBJ->NewAnm->Code!=anm_Attack+max(int(OBJ->NewState)-1,0))){//to optimise
		...
		if(!(OBJ->FrameFinished||(OBJ->NewAnm->CanBeBroken&&OBJ->NewAnm->Code!=anm_Attack+OBJ->NewState-1)))return;//to optimise
		...
		if(dst1>=ADC->AttackRadius1[NeedState]&&dstx<OBJ->GetMaxAttackRadius(NeedState)+DR2&&dst<RMAX){
			addrand(1);
			//can attack at once
				...
				if(OBJ->LocalNewState==NeedState+1||(OBJ->LockType==1&&OBJ->NewState==NeedState+1)){
					...
					if(!DELAY){
						...
						if(WP){
							...
							int WRes=OBJ->LocalNewState==3||NMN->MotionStyle==2?-1:CheckBreaksBetweenPoints(x0,y0,z0,x1,y1,z1,OBJ->Index);
...
DoShot1:;
						...
						if(!DELAY){
							//if((tmtmt&15)==3){
								...
								if(OBJ->LockType==1)OBJ->NewAnm=NMN->GetAnimation(anm_Attack+OBJ->NewState-1);
			...
			if(dst>=OBJ->GetMaxAttackRadius(NeedState)+DR2||dst>=RMAX){
				...
				if(!OBJ->StandGround){
					int R0=(OBJ->GetMaxAttackRadius(NeedState)*3+ADC->AttackRadius1[NeedState])/4;
...
void MakeOneShotLink(OneObject* OBJ){
	...
	if(cf1>=hf){
		...
		if(WP){
			...
			if(ST<0)ST=0;
			OBJ->delay=OBJ->Ref.General->MoreCharacter->AttackPause[ST];
			OBJ->MaxDelay=div(int(OBJ->delay)*OBJ->PersonalDelay,100).quot;
			OBJ->MaxDelay=int(OBJ->MaxDelay)*100/OBJ->GetAttackSpeedBonus();
			OBJ->delay=OBJ->MaxDelay;
		...
		if(dst>=ADC->AttackRadius1[AttMethod]&&dst<=ADC->AttackRadius2[AttMethod]){
			{
				...
				if(NGoodWp==0||NGoodWp<NMN->MaxZalp){
                    if(mindir>0)OBJ->RealDirPrecise+=(GameSpeed*OB->newMons->MinRotator)>>5;
					else OBJ->RealDirPrecise-=(GameSpeed*OB->newMons->MinRotator)>>5;
...
void OneObject::ClearBuildPt(){
	...
	int y0=(RealY+PicSY)>>8;
	for(int i=0;i<Npt;i++){
		if(!MFIELDS[3].CheckPt(x0+px[i],y0+py[i]))MFIELDS[0].BClrPt(x0+px[i],y0+py[i]);
	}
};
void BuildObjLink(OneObject* OBJ);
...
bool OneObject::BuildObj(word OID,int Prio,bool LockPoint,byte OrdType){
	...
	if(OB->FindPoint(&xx,&yy,FP_NEAREST_POINT|FP_FIND_WORKPOINT|FP_UNLOCKED_POINT)){
		...
		Or1->PrioryLevel=Prio&127;
		if(LockPoint)MFIELDS[0].BSetPt(xx,yy);
		//PrioryLevel=Prio&127;	
...
void OneObject::DeleteLastOrder(){
	...
	if(NewEnemy&&!InCloseObjects){
		static bool InNewEnemy=0;
		extern bool InClOrders;
		if(!(InNewEnemy||InClOrders)){		
			InNewEnemy=1;
			addrand(Index);
			SearchVictim();
			InNewEnemy=0;
...
void SearchVictimForAI_Archer(OneObject* OB){
	...
	//NewMonster* NM=OB->newMons;
	////int rr=NM->VisRange;
	//int rr=OB->Ref.General->MoreCharacter->VisRange;
	//int rx1=(rr>>11)+1;
	//int rx2=rx1+rx1+1;
	//byte nmask=~OB->NMask;
...
void OneObject::SearchVictim(){
	...
	if(NM->WSECT){
		...
		NM->WSECT->RMax=Ref.General->MoreCharacter->VisRange>>4;
	...
	int REALY=RealY;
	//int rr=NM->VisRange;
	int rr=Ref.General->MoreCharacter->VisRange+(addr<<4);
	...
	if(Zombi){
	    ...
	    NewMonster* NM=newMons;
	    //int rr=NM->VisRange;
		int rr=Ref.General->MoreCharacter->VisRange;
		int rx1=6;
		byte nmask=~NMask;
	    byte mmask=NM->KillMask;
		OneObject* DestObj=NULL;
    ...
    int dist;
#ifdef CONQUEST
	if(TempDelay&&(NInside||Ref.General->MoreCharacter->MaxInside==0)){
		word VICTIMS[1024];
		int NVICT=0;
		int rx2=rx1+rx1+1;
		...
		if(mindist<(rr+dr)){
			...
			if(ac2!=1||DestObj->NMask&NMask){
				...
				if(BrigadeID!=0xFFFF&&RifleAttack&&delay==0){
					...
					if((!newMons->ArmAttack)||RifleAttack||Norma(RealX-DestObj->RealX,RealY-DestObj->RealY)<100*16)AttackObj(DestObj->Index,1);
				}
			}
		}
...
void PlayAnimationEx(OneObject* OB,NewAnimation* NA,int Frame,int x,int y){
	...
	if(NA->HotFrame==0xFF)DoPlay=true;
		...
		if(Frame<=HF){
			OB->SoundWasEmit=false;
		}
		if(!OB->SoundWasEmit){
			if(Frame>HF){
				OB->SoundWasEmit=true;
				DoPlay=true;
			}
		}
	}else{
...
void NewMonsterSmartSendToLink(OneObject* OBJ){
	...
	if(NextNextTop==FinalTop||FinalTop==NextTop){
		...
		OBJ->NewMonsterPreciseSendTo(x<<4,y<<4,prio,1);
		return;
	};
	if(NextNextTop!=0xFFFF){
		...
		OBJ->NewMonsterPreciseSendTo(rx<<4,ry<<4,OR1->PrioryLevel,1);
		return;
	}else{
		OBJ->DeleteLastOrder();
		return;
...
void OneObject::NewMonsterSmartSendTo(int x,int y,int dx,int dy,byte Prio,byte OrdType){
	...
	if(TD<80&&Norma(x-(RealX>>4),(y-(RealY>>4))>>1)<1100){
			if(ActivityState==2||!(Prio&127)){
				if(newMons->GetAnimation(anm_PMotionL)->Enabled&&!newMons->MaxAIndex)GroundState=1;
		}else{
			GroundState=0;
...
void MotionHandlerForSingleStepObjects(OneObject* OB){
	...
	NewMonster* NMN=OB->newMons;
//#ifndef NOBOIDS
	if(OB->Attack&&!OB->FrameFinished/*&&OB->BrigadeID==0xFFFF*/&&!NMN->RotationAtPlaceSpeed)
	{
...
//#endif //NOBOIDS
	void RotUnit(OneObject* OB,char Dir,byte OrdType);
	void RotUnitLink(OneObject* OB);
	bool LO=OB->LocalOrder;
	if(LO&&OB->LocalOrder->DoLink==RotUnitLink)return;
	if(OB->FrameFinished){
	...
	if(OB->BrigadeID==0xFFFF&&!(OB->LocalOrder||OB->LockedInBuilding!=0xFFFF))OB->DestX=-1;
	...
	if(CANNOWMOVE){		
		if(OB->DestX>0){
			...
			R=(R*NM->SpeedScale)>>8;
			R=(R*OB->Ref.General->MoreCharacter->Speed)/100;
			if(OB->ActiveAbility)OB->ActiveAbility->ActiveAbilities.modifyMotionSpeed(R,R);
			addrand(R);
			addrand(OB->RealDir);
			/*
			//checking TEXTURE
...
DoRot:;
					if(HaveRotAnm&&Nrm>120*16&&(!isLock)){
						...
						if(RotSpeed){							
							...
							if(mr>8){
								...
								if(ddir<0)OB->RealDirPrecise-=mr<<5;
								else OB->RealDirPrecise+=mr<<5;
								addrand(OB->RealDir);
								OB->RealDir=OB->RealDirPrecise>>8;
								addrand(OB->RealDir);
							}
				...
				if(OB->LocalNewState==OB->NewState){
					...
					if(!FixEnd){
						...
						if(!OB->BackMotion){
							rx1=OB->RealX+((RS*TCos[OB->RealDir])>>8);
							ry1=OB->RealY+((RS*TSin[OB->RealDir])>>8);
						}else{
							rx1=OB->RealX+((RS*TCos[byte(OB->RealDir+128)])>>8);
							ry1=OB->RealY+((RS*TSin[byte(OB->RealDir+128)])>>8);
						}
						CHKXY;
						addrand(OB->RealDir);
						addrand(RS);
			...
			if(CANNOWMOVE){
				if(OB->NewAnm->NFrames){
					...
					if(code==anm_RotateR){
						...
						int rf=RInFrame;
						if(!rf)rf=1;
						OB->CurrentFrameLong=((abs(OB->TotalPath+100000)<<8)/rf)%(int(nf)<<8);
						OB->NewCurSpritePrev=OB->NewCurSprite;
						OB->NewCurSprite=OB->CurrentFrameLong>>8;
					}
...
int GetDamage(word ShooterID,word VictimID,int AttType){
	...
	if((AttType)&&(R>Shooter->Ref.General->MoreCharacter->AttackRadius2[AttType]))return 0;
...
void MotionHandlerForFlyingObjects(OneObject* OB){
	APPLY_TIRING(OB);
	if(OB->Sdoxlo){		
            if(OB->OverEarth)OB->OverEarth-=4;		
		return;
	}
	if(OB->DestX<=0)OB->StandTime++;
	...
	int SP0=(OB->newMons->MotionDist*OB->Ref.General->MoreCharacter->Speed)/100;
	bool Pike=false;
	bool nogrow=false;
	if(OB->DestX>0){		
		int R=Norma(OB->RealX-OB->DestX,OB->RealY-OB->DestY);        
		if(Pike&&R<16*512&&R>16*128){
			nogrow=true;
			int dd=GameSpeed*6;
			int dd0=dd>>8;
			dd-=dd0<<8;
			if((rando()&255)<dd)dd0++;
			if(OB->OverEarth>20)OB->OverEarth-=dd0;			
		}
		if(R>1000)R=1000;		
		int maxs=((SP0*R/1000)*GameSpeed)>>8;
		N+=GameSpeed<<2;
		if((N>>8)>maxs)N=int(maxs)<<8;		
		char D0=GetDir(OB->DestX-OB->RealX,OB->DestY-OB->RealY);
		char dd=D0-OB->RealDir;
		if(abs(int(dd))<NMN->MinRotator){
			OB->RealDir=D0;
		}else{
			if(dd>0){
				OB->RealDir+=NMN->MinRotator;
				SlowFrame=1;
			}else{
				OB->RealDir-=NMN->MinRotator;
				SlowFrame=1;
	...
	OB->GraphDir=OB->RealDir;
	OB->RealVx=(N*TCos[OB->RealDir])>>8;
	OB->RealVy=(N*TSin[OB->RealDir])>>8;
	OB->ForceX=(OB->ForceX*63+(OB->RealVx-vx0)*10)/64;
	OB->ForceY=(OB->ForceY*63+(OB->RealVy-vy0)*10)/64;
	...
	if(OB->NewAnm->NFrames){
		...
		if(OB->FrameFinished){
			OB->SetZeroFrame();
			if(!OB->Sdoxlo)OB->NewAnm=NMN->GetAnimation(anm_MotionL);
		}
	}else{
		OB->SetZeroFrame();
		if(!OB->Sdoxlo)OB->NewAnm=NMN->GetAnimation(anm_MotionL);
...
void CheckVitality(OneObject* OB){
	if(OB->LockType!=1&&!OB->NewBuilding){
		if(MFIELDS[OB->LockType].CheckBar(OB->x,OB->y,OB->Lx,OB->Ly)){
			OB->Die();
		}
	}
}
...
int OneObject::GetAttackSpeedBonus(){
	int sp=Ref.General->MoreCharacter->AttackSpeed;
	if(BrigadeID!=0xFFFF){
	...
	if(ActiveAbility){
		...
		ActiveAbility->ActiveAbilities.modifyAttackRate(LocalNewState?LocalNewState-1:0,sp,sp,V);
	}
	return sp;
}
int OneObject::GetMaxAttackRadius(int AttType){
	int R=Ref.General->MoreCharacter->AttackRadius2[AttType];
	if(R>180){
		if(BrigadeID!=0xFFFF){
...
int OneObject::GetMaxAttackRadius(){
	int R=Ref.General->MoreCharacter->MaxR_Attack;
	if(R>180){
		if(BrigadeID!=0xFFFF){
			R+=CITY[NNUM].Brigs[BrigadeID].AddAttackRange;
		}
...
bool OneObject::notSelectable(){
	return NotSelectable&&!EditMapMode;
}
...
bool ExtraLock(word& NLockPt,byte*& LockX,byte*& LockY)
{
	...
	if(sz)
	{
		...
		byte* newLX = (byte*)malloc(NA);
		byte* newLY = (byte*)malloc(NA);
