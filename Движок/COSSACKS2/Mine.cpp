...
bool CheckPossibilityToComeInside(OneObject* Transport,OneObject* Passanger){
	if(Transport->newMons->InviteMask&Passanger->newMons->VisitorMask){
		...
		if(NP==0)return true;
		AdvCharacter* ADC=Transport->Ref.General->MoreCharacter;
...
void GoToMineLink(OneObject* OBJ){
	...
	if(!MiNM->ShowInsideUnits)OB->RealDir=-64;
	int COS=TCos[byte(OB->RealDir+64)];
	int SIN=TSin[byte(OB->RealDir+64)];	
...
GGG1:
			...
			for(int i=ncp-1;i>0;i--){
				...
				OBJ->NewMonsterPreciseSendTo(xx,yy,128+16,1+128);
			}
...
void MSpinf(OneSprite* OS){
	...
	if(OC->IntResType!=0xFF){
		...
		//if(MS&MineMask){
			...
			NewAnimation* DN=NEWMON->GetAnimation(anm_StandLo);
...
extern int tmtmt;
int CheckMinePosition(NewMonster* NM,int* xi,int* yi,int r,bool Manual=1);
void HandleMines(){
	if(!(tmtmt&255)){
