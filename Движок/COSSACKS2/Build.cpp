...
//	byte NI=OBJ->NNUM;
	...
	if(ID!=-1){
		...
		{			
			...
			if(!Immediate){
				for(int j=1;j<nc;j++){
					if(j==nc-1){
						...
						OB->NewMonsterPreciseSendTo(((xx<<4)+PTX[j])<<4,((yy<<4)+PTY[j])<<4,16,2+128);
					}
					else OB->NewMonsterPreciseSendTo(((xx<<4)+PTX[j])<<4,((yy<<4)+PTY[j])<<4,16,2+128);
				};
			}
			...
			if(OBJ->DstX<0&&!Immediate)OB->NewMonsterSendTo((((xx<<4)+PTX[nc-1])<<4)+(rando()%4096)-2048,(((yy<<4)+PTY[nc-1])<<4)+(rando()%4096)-2048,16,2+128);
