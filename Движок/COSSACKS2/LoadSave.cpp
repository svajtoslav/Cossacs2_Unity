...
void SaveActiveObjects(SaveBuf* f1){
	...
	int sz1=strlen(D1.str);
	int sz2=strlen(D2.str);
	xBlockWrite(f1,&sz1,4);
	xBlockWrite(f1,D1.str,sz1);
	xBlockWrite(f1,&sz2,4);
	xBlockWrite(f1,D2.str,sz2);	
};
...
void PreSaveGame(SaveBuf* SB,char* Messtr,int ID){
	...
	int L=strlen(src.str)+1;
	xBlockWrite(SB,&L,4);
	xBlockWrite(SB,src.str,L);
};
...
void SendLogsToServer(){
	if(UseGSC_Login&&BattleLog.str&&BattleLog.str[0]){
        _str temp;
		char GMID[64]="";
		GetGameID(GMID);
		char* s=strstr(GMID,"@");
		if(s){
			...
			if(s2){
				s2[0]=0;
				temp.print("GW|btlog&%s&%s",s+1,BattleLog.str);
				ExplorerOpenRef(0,temp.str);
			}
		}		
	}else{
		//if(BattleLog.str&&BattleLog.str[0]){
		//	FILE* F=fopen("btl.log","a");
		//	if(F){
		//		fprintf(F,"%s",BattleLog.str);
		//		fclose(F);
		//	}
		//}
	}
...
void ClearBattleLogs(){
	//if(BattleLog.str&&BattleLog.str[0])BattleLog.DString::WriteToFile("blog.txt");
	SendLogsToServer();
    BattleLog.Clear();
}
char* GetBattleLog(){
	return BattleLog.str;
}
