...
bool AIScripsEditorProcess(ClassEditor* CE,BaseClass* BC,int Options){
	...
	if(Options==2){
		...
		if(GetTickCount()>RefTime){
			...
			int i=MyNation;
			for(int j=0;j<GScripts.Scripts[i].Units.GetAmount();j++)if(GScripts.Scripts[i].Units[j]->ShortName.str){
				EU->Add(GScripts.Scripts[i].Units[j]->ShortName.str,GScripts.Scripts[i].Units[j]->UnitID);
			}
			for(int j=0;j<GScripts.Scripts[i].Buildings.GetAmount();j++)if(GScripts.Scripts[i].Buildings[j]->ShortName.str){
				EU->Add(GScripts.Scripts[i].Buildings[j]->ShortName.str,GScripts.Scripts[i].Buildings[j]->UnitID);
			}
		}
		if(NI!=MyNation){
...
void PointStaticInfo::CreateForPoint(int x,int y){	
	...
	int xx=x>>4;
	int yy=y>>4;
	Locked=CheckPt(xx-20,yy)||CheckPt(xx+20,yy)||CheckPt(xx,yy-20)||CheckPt(xx,yy+20)||CheckPt(xx,yy);
	if(Locked)return;
	//roads
	int NR=GetNRoads();
	...
	DistanceToStone=ns>5?RStone:100000;
}
class MapInfoHash{
public:
...
int CheckCreationAbility(byte NI,NewMonster* NM,int* x2i,int* y2i,word* BLD,int NBLD,bool);
...
int TryToStandBuildingAnywhere(int x,int y,byte NI,int Type,BuildRequirements* Req){
	...
	if(Req->NearToEnemy||Req->FarFromEnemy){
		...
		for(int i=0;i<MAXOBJECT;i++){
			...
			if(OB&&(!OB->Sdoxlo)&&OB->NNUM!=7){
                totdx+=(OB->RealX>>4)-cx0;
				totdy+=(OB->RealY>>4)-cy0;
				totw++;
			}
		}
		if(totw){
		...
		for(int j=0;j<N;j++){
			...
			if(tx>0&&ty>0&&tx<TopLx-1&&ty<TopLx){
				...
				if(tz>=0&&tz<0xFFFE&&TopIslands[tz]==ISLAND){					
					PointStaticInfo* PS=MHash.GetPointInfo(xr>>4,yr>>4);
					if((!PS->Locked)&&PS->DistanceToRoad>250){
						int res=CheckCreationAbility(NI,NM,&xr,&yr,TotBLD,Nb,0);
						if(res!=-1){
							...
							if(Req->NearToEnemy!=Req->FarFromEnemy){
								char dd=GetDir(xr-cx0,yr-cy0)-EnemyDir;
...
bool EnemyNearBase::Check(){
	...
	for(int i=0;i<MAXOBJECT;i++){
		...
		if(OB&&!(OB->Sdoxlo||OB->NMask&Mask)){
			...
			if(R<Radius){
				if(OB->NewBuilding){
					if(OB->Ref.General->MoreCharacter->MaxDamage){
                        if(Towers)N++;
					}else{
						if(Buildings)N++;
					}
...
void cv_AI_ScriptExt::OnGameStart(){
	pLastTime=0;
	for(int i=0;i<7;i++){
	...
	if(OB&&OB->Serial==obSerial&&!OB->Sdoxlo){
		...
		if(OB->Usage==MineID){
			if(OB->Ready){			
				...
				Inv->RefreshUnits();
				int maxIn=OB->Ref.General->MoreCharacter->MaxInside+OB->AddInside;
...
const char*	AI_Folder::GetThisElementView(const char* LocalName){
	if(Description.str){
		static char cc[256];
		sprintf(cc,"%s %s",LocalName,Description.str);
		return cc;
	}
	return LocalName;
}
