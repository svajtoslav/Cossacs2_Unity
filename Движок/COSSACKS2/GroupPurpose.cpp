...
DLLEXPORT
void SGP_TakeResourcesZone(GAMEOBJ* Grp,byte Type,byte NI,int ResType,GAMEOBJ* Zone){
	if(Grp->Type=='UNIT'&&Grp->Index<SCENINF.UGRP.GetAmount()){
...
bool GroupAttackInZone(UnitsGroup* UGR, byte NI, int x, int y, int r){
	...
	GA->y=y;
	GA->r=r;	
	return GroupAttackInZoneLink(UGR);
}
//
...
DLLEXPORT
int GetZonesDist2(int Start,int Fin,byte TopType){
...
DLLEXPORT bool AttackEnemyInZone3(GAMEOBJ* Grp,GAMEOBJ* Zone,byte NI){
			...
			if(XX>=0&&YY>=0&&XX<VAL_MAXCX-1&&YY<VAL_MAXCX-1){
				...
				if(NMon){
					...
					for(int i=0;i<NMon;i++){
						...
						if(MID!=0xFFFF){
							OneObject* OB=Group[MID];
							if(OB&&NEN<1600&&!((OB->NMask&MASK)||OB->Sdoxlo||OB->Hidden||OB->newMons->Immortal)){
				...
				if(BO&&NEN<1600&&!((BO->NMask&MASK)||BO->Sdoxlo||BO->Hidden||BO->newMons->Immortal)){
					word BID=BO->Index;
					bool exist=false;
	...
	UnitsGroup* UG=SCENINF.UGRP+Grp->Index;
	void SetUnitsState(GAMEOBJ* GRP,bool SearchEnemy,bool StandGround,bool ArmAttack,bool FriendlyFire);
	if((NEN==0||NEN<vMinEnemy)&&NEB==0){
		SetUnitsState(Grp,0,1,0,0);
		//SetUnitsState(Grp,1,0,0,0); // very stupid
		return false;
	}else{
