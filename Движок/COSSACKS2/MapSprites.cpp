...
{
	if(EditableModel){
		Vector3D ax(cos(RotX),cos(RotY),sin(RotX)*sin(RotY));
		Matrix4D M40;
		M40.scaling(ObjCharacter::Scale);
		M4=M40;
		M40.rotation(Vector3D::oZ,RotFI*2);
		M4*=M40;
		M40.rotation(Vector3D::oY,RotX);
		M4*=M40;
		M40.rotation(Vector3D::oX,-RotY);
		M4*=M40;		
		M40=GetSkewTM();	
		M4*=M40;
		M40=Matrix4D::identity;
		M40.translation(Vector3D(x,y,EditableModel?ObjCharacter::ModelDZ:z));
		M4*=M40;
	}else M4.translation(x,y,z);
}
...
void addSprite(int x,int y,SprGroup* SG,word id,int PrefRadius){
	...
	if(CheckIfNewTerrain()){
		...
		if(OC->ModelManagerID!=-1){
			Matrix4D M4;
			OC->GetMatrix4D(M4,x,y,0);	
			const char* name=IMM->GetModelFileName(OC->ModelManagerID);
			if(name){
				extern UniversalMap g_UniversalMap;
                //g_UniversalMap.AddModelByName((char*)name,M4);
				g_UniversalMap.CombineModelByName((char*)name, M4);
				return;
			}
		}
	}
...
void addSpriteAnyway(int x,int y,SprGroup* SG,word id){
	SpriteSuccess=false;
	if(id>=SG->Objects.GetAmount())return;
	if(SG->Objects[id]->OnGround!=0xFFFF){
		AddGPiece(x,y,SG->Objects[id]->OnGround);
		return;
	};	
...
public:
	...
	void init(){
		...
		BM->setShader(IRS->GetShaderID("FastSprites"));
	...
	void add(Vector3D& pos,Vector2D<float>& tc,Vector2D<float>& left,Vector2D<float>& size,float scale,float angle){
		init();
...
void AddFastSprite(const OneSprite& OS)
{
	...
	FS.add( Vector3D( OS.x, OS.y, OS.z ),
            Vector2D<float>( OC.uC, OC.vC ),
            Vector2D<float>( OC.uL, OC.vL ),
            Vector2D<float>( OC.uR, OC.vR ),
            1.0f, ang );
} // AddFastSprite
...
//x: 16units=1 pix
//y: 16units=1 pix
int CheckMinePosition(NewMonster* NM,int* xi,int* yi,int r,bool Manual){
    int x=(*xi)>>4;
    int y=(*yi)>>4;
	int x0=x;
	int y0=y;
...
void OneSprite::CreateMatrix()
{
	...
	{	
		//horizontal aligning
			...
			OC->P3.wy-=TBiasY;
			OC->P3.wz-=TBiasZ;
			Vector2Df PP1,PP2,PP3;
			
			PP1.set(OC->P1.xp,OC->P1.yp);
			PP2.set(OC->P2.xp,OC->P2.yp);
			PP3.set(OC->P3.xp,OC->P3.yp);
			Vector3D W1,W2,W3;
			...
			if(!M4)M4=new Matrix4D;
			M4->TransformationFromPlaneToWorld(PP1,PP2,PP3,W1,W2,W3);
	}
		...
		if(NM1->BornPtX.GetAmount()){
			if(OBJ->UnlimitedMotion){
				...
				for(int j=np-1;j>=0;j--){
					OBJ->NewMonsterSendTo(((x0<<4)+NM1->BornPtX[j])<<4,((y0<<4)+NM1->BornPtY[j])<<4,128+127,1+128);
				};
				...
				int y0=((OB->RealY+PicSY)>>8);
				int np=NM1->BornPtX.GetAmount();
				int dsx0=x0+(NM1->BornPtX[np-1]>>4);
				int dsy0=y0+(NM1->BornPtY[np-1]>>4);
				int NRM=Norma(dsx0-OBJ->x,dsy0-OBJ->y);
				if(NRM<5){
					//OBJ->ClearOrderedUnlimitedMotion(1);
					OBJ->UnlimitedMotion=0;
					for(int j=0;j<np;j++){
						OBJ->NewMonsterPreciseSendTo(((x0<<4)+NM1->BornPtX[j])<<4,((y0<<4)+NM1->BornPtY[j])<<4,128+127,1);
					};
					OBJ->UnlimitedMotion=1;
					//addrand(CheckNtIntegrity(OBJ->NNUM));
					return;
...
void TimeReq::Handle(){
    for(int i=0;i<NMembers;i++){
        ...
        if(!Kind){
            ...
            if(OC->TimeAmount&&OS->TimePassed>=OC->TimeAmount){
...
*/
float ObjCharacter::Scale=1.0f;
float ObjCharacter::RotX=0.0f;
float ObjCharacter::RotY=0.0f;
float ObjCharacter::ModelDZ=0.0;
float ObjCharacter::RotFI=0.0;
