...
float GetAttSpped(OneObject* OB,int AttType)
{
	...
	if(NA&&NA->NFrames)
	{
		int AF=max(NA->NFrames,OB->Ref.General->MoreCharacter->AttackPause[AttType]);
		//rez=GSets.SVOpt.RequiredMsPerFrame*AF*OB->Ref.General->MoreCharacter->AttackSpeed/1000;
		...
		if(NSL[i]>0)
		{
			for(int k=0;k<NSL[i];k++)
			{
				...
				if(OB&&!OB->Sdoxlo)
				{
					...
					{
						if(OB->Ref.General->MoreCharacter->MaxDamage[j])
						{
							AttType=j;
							Damage=OB->Ref.General->MoreCharacter->MaxDamage[j];
							break;
						}
					}
					...
					{
						Deff[e]=OB->Ref.General->MoreCharacter->Protection[e]+OB->AddShield;
					}
					C.AddUnit(OB->newMons->Name,OB->Index,i,OB->Ref.General->MoreCharacter->Life,
