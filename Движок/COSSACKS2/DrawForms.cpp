...
void DrawRect4(int x0,int y0,int x1,int y1,Corners* CR,int GP_File){
	...
	PopWindow(&TEMP);
	IntersectWindows(x0+LX3,(y1+y0)/2+1,x1-LX3-1,y1+LLY2);//LLY2);
	N=(x1-x0+1-DLLX)/DNLX;
	for(i=0;i<=N+2;i++){
	...
	PopWindow(&TEMP);
	IntersectWindows(x0-LX3,(y1-LDY2+y0+LLY2)/2,(x0+x1)/2-1,y1+LLY2);
	if(CR->CLD!=-1)GPS.ShowGP(x0-LX3,y1-LDY2,GP_File,CR->CLD,MyNation);

	PopWindow(&TEMP);
	IntersectWindows((x0+x1)/2,(y1-LDY2+y0+LLY2)/2,x1+LX3,y1+LLY2);
