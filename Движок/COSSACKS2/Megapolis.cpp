...
void City::RegisterNewUnit(OneObject* OB){
	...
	if(OB){
		...
		if(OB->newMons->Peasant){
			...
			if(OB->newMons->LockType==1&&OB->newMons->MaxInside){
				OB->Borg=1;
...
void City::EnumUnits(){
	...
	Nat->NArtdep=0;
	for(i=0;i<4;i++)Nat->NArtUnits[i]=0;
	int xc=CenterX;
	int yc=CenterY;
	...
	for(i=0;i<MAXOBJECT;i++){		
		OB=Group[i];
		if(OB&&/*(!OB->Illusion)&&*/OB->NNUM==NI&&(OB->Hidden||!OB->Sdoxlo)){
			addrand(OB->Index);
			addrand(OB->NIndex);
	...
	*/
	...
	if(Nat->NFarms>GSets.CGame.UnitLimit)Nat->NFarms=GSets.CGame.UnitLimit;
	InCEnum=0;
};
...
bool City::FindPreciseBuildingPlace(GeneralObject* GO){
	...
	if(CheckCreationAbility(NI,GO->newMons,&xx,&yy,NULL,0,0)!=-1){
		BPR.x=xx;
		BPR.y=yy;
		return true;
	};
					...
					for(int p=0;p<NU;p++){
						...
						if(MID!=0xFFFF){
							...
							if(OB&&OB->Serial==SNS[p]){
								int dl=OB->Ref.General->MoreCharacter->Life-OB->Life;
								if(dl){
									DL+=dl;
									NDL++;
								}
		...
		for(int i=0;i<NMines;i++){
			...
			if(OSP&&OSP->Enabled){
				...
				if(usefog&&!NATIONS[GSets.CGame.cgi_NatRefTBL[MyNation]].Geology){
					...
					vis=GetInterpFOW(OSP->x,OSP->y/2)>600;
				}
				if(vis){
	...
	if(F){
		do{
			...
			if(S[0]!='/'&&S[0]){
				...
				if(NAbils>=MaxAbils){
					MaxAbils+=64;
					Abils=(OneAbility*)realloc(Abils,MaxAbils*sizeof OneAbility);
				};
				OneAbility* ABL=Abils+NAbils;
				memset(ABL,0,sizeof OneAbility);
				NAbils++;
				char sp[64];
				bool done=0;
				int v;
