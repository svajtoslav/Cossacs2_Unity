	...
	for(int i=0;i<N;i++){
		...
		if(mid!=0xFFFF){
			...
			if(OB&&OB->Serial==MembSN[i]){
				OB->UnitSpeed=64;
				OB->CurUnitSpeed=64;
				OB->GroupSpeed=OB->newMons->MotionDist;//(OB->newMons->MotionDist*OB->Ref.General->MoreCharacter->Speed)/100;
	...
	for(int i=0;i<ni;i++)
	{
		...
		if(tOB&&(!tOB->Sdoxlo))
		{
			if(tOB->Life>EnemyLife[rn]&&((EnemyFireCount[rn]<5&&tOB->Life<101)||tOB->Life>100))
			{
				...
				int sea=OB->Ref.General->MoreCharacter->VisRange>>4;
				if(nd<dist&&nd<=sea)
				{
					Coor3D C3D;
...
void RotUnit(OneObject* OB,char Dir,byte OrdType){	
	if(OB->Transport||OB->newMons->MotionStyle==5||OB->NewBuilding||OB->newMons->ShowInsideUnits)return;	
	addrand(1029);
	int RotSpeed=OB->newMons->RotationAtPlaceSpeed;
	if(RotSpeed&&OB->BrigadeID!=0xFFFF){
		Brigade* BR=CITY[OB->NNUM].Brigs+OB->BrigadeID;
...
CEXPORT
void RotUnit(int Index,char Dir,byte OrdType)
{
	if(Index!=0xFFFF)
	{
				...
				if(MID!=0xFFFF){
					...
					if(OB&&!(OB->InMotion||OB->Sdoxlo||(!CheckIfPossibleToBreakOrder(OB)))){
						if(Norma((OB->RealX>>4)-BR->posX[p],(OB->RealY>>4)-BR->posY[p])>5){
							...
							bool FN=0;
							if(!(OB->LocalNewState==State&&(FN=(OB->FrameFinished||OB->NewAnm->CanBeBroken)))){
								Ready=0;
								if(!FN){
									OB->SetNextFrame();
								};
...
void Brigade_Bitva::AddEnemXY(int x,int y,int MyTop,byte mask){
	...
	if(cell>0&&cell<MAXCIOFS){
		...
		for(int i=0;i<NMon;i++){
			...
			if(MID!=0xFFFF){
				OneObject* OB=Group[MID];
				if(OB&&!(OB->NMask&mask||OB->Sdoxlo||OB->newMons->LockType==1)){
					int id=OB->Index;
					int ms=1<<(id&7);
					int of=id>>3;
					if(!(BitMask[of]&ms)){
