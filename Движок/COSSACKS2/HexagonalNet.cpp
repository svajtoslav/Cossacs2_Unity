...
public:
	...
	virtual short GetValue(int x,int y){
		...
		return max(H,temp::GetValue(x,y));
	}
	...
	for(int ix=x0;ix<=x1;ix++){
		for(int iy=y0;iy<=y1;iy++){
			...
			if(tx>0&&ty>0&&tx<VertInLine&&ty<MaxTH){
				...
				DWORD T1=TexDiffuse[TexMap[ofs]];
				DWORD T2=TexDiffuse[TexMapEx[ofs]];
				int W=WTexMapEx[ofs];
				DWORD D=MixDWORD(T1,T2,255-W,W);
                WMAP.TexR.SetValue(ix,iy,(D>>16)&255);
				WMAP.TexG.SetValue(ix,iy,(D>>8)&255);
				WMAP.TexB.SetValue(ix,iy,D&255);
			}
        }
	}
}
