...
void UnitLight(OneObject* OB){
	//if(OB->NNUM==7){
	//	if(OB->Ref.General->MoreCharacter->MaxDam==0)return;
	//}
	...
	int VT=OB->Ref.General->MoreCharacter->VisionType;
	switch(VT){
	case 0:
		FogSpot(xx,yy);
		break;
	...
	default:
		{
			...
			for(int i=0;i<N;i++){
				byte A=i*256/N;
                FogSpot(xx+(R*TCos[A]>>8),yy+(R*TSin[A]>>8));
			}
			if(VT>20){
				R>>=1;
				int N=6*R/250;
...
int GetInterpFOW2(int x,int y){//x,y - pixel coordinates
	...
	int F3=fmap[ofs+FMSX];
	int F4=fmap[ofs+FMSX+1];
	int FF=F1+(((F2-F1)*dx)>>7)+(((F3-F1)*dy)>>6)+(((F4+F1-F3-F2)*dx*dy)>>13);
