...
extern	tpDrawOnMapCallback* SetDrawOnMapCallback(tpDrawOnMapCallback* dc);
...
public:
	...
	int			vGrpID;		// Группа за которой движется камера.
    	
	bool		useMapXY;

	int			TimeD;		// Время перемешения.
	int			LastTime;	// Время последнего перемещения.
	enum		enMoveType {
