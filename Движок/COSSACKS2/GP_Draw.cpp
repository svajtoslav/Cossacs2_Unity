...
{
	if (!OB) return;
	if( (OB->NotSelectable&&OB->NewBuilding) || OB->newMons->NotSelectable )return;
    int nVGP = g_VisGP.size();
	...
	try{
		...
		for(int i = nVGP - 1; i >= 0; i--)
		{
			...
			if (vgp.m_ModelID == 0xFFFFFFFF)
			{
				...
				{
					cx      = (x - (int)vgp.m_TM.e30) << (5 - Shifter);
					cy      = (y - (int)vgp.m_TM.e31) << (5 - Shifter);
					gpID    = vgp.m_GPID;
					sprID   = vgp.m_Frame;
			...
			{
				float dist = 0.0f;
				for (int ddx=-3;ddx<=3;ddx+=3)
				{
					for (int ddy=-3;ddy<=3;ddy+=3)
					{
						if (IMM->Intersects( x+ddx, y+ddy, vgp.m_ModelID, vgp.m_TM, dist ))
						{
							if (dist < minDist) 
							{
								minDist     = dist;
