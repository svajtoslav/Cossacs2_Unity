...
void VideoLayer::ShowRGB_Buffer(byte* src,int lx,int ly,int x,int y,int SizeX,DWORD Diffuse,float sx,float sy){
	if(!Texture){
		TextureDescr td;
		td.setValues(TXSZ,TXSZ,cfXRGB8888,mpManaged,1,tuProcedural);
		Texture=IRS->CreateTexture("BinkTempTexture",td);
	}
	if(true){//lx>127+256||ly>256-64){
		...
		if(sx>1.05)BM->setShader(IRS->GetShaderID("video"));
		else BM->setShader(IRS->GetShaderID("hud"));
...
void PlayFullscreenVideo(char* name,float px,float py){
	...
	if(BPL.Open(name)){
		...
		KeyPressed=false;
		int t0=GetTickCount();
		DialogsSystem* DS=GetShowAboutDS();
		do{
			if(BPL.GetCurrentFrame()){
				int rlx=RealLx*(1-px*2);
				int rly=RealLx*(1-px*2);
				int rly1=rlx*BPL.Ly/BPL.Lx;
				int rlx1=rly*BPL.Lx/BPL.Lx;
				if(rly>rly1){
                    rly=rly1;
				}else{
                    rlx=rlx1;
				}
				float SX=float(rlx)/BPL.Lx;
				float SY=float(rly)/BPL.Ly;
				IRS->ClearDeviceTarget();
                VL.ShowRGB_Buffer(BPL.OutBuffer,BPL.Lx,BPL.Ly,(RealLx-rlx)/2,(RealLy-rly)/2,BPL.Lx,0xFFFFFFFF,SX,SY);
				if(GSets.EnableSubtitres){
					void DrawSubtitlesInWindow(int x,int y,int Lx,int Ly,float TimePassed);
					DrawSubtitlesInWindow((RealLx-rlx)/2,(RealLy-rly)/2,BPL.Lx*SX,BPL.Ly*SY,float(GetTickCount()-t0)/1000.0f);
				}
				if(FullscreenVideoCredits){					
					if(DS->DSS.GetAmount()){
						ProcessMessages();
						DS->ProcessDialogs();
						DS->RefreshView();
...
CEXPORT
void ShowRGBBuff(byte* Buf,int x,int y,int Lx,int Ly,int SizeX,int xs,int ys){
