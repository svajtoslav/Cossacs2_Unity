...
void Operand::GetAssembledView(DString& ST,bool SetURL)
{
	...
	if(DMask.L)
	{
		...
		for(int i=0;i<n;i++)
		{
			...
			char* fp=strstr(DMask.str,Rep.str);
			if(fp)
			{
				DString Spas;
				Spas.Add("\\");
				for(char* k=fp-1;k>=DMask.str;k--)
				{
					if(k[0]==' ')
					{
						Spas.Add(" ");
...
const char* Operand::GetThisElementView(const char* LocalName)
{
	static char s[256];
	DString ST;
	GetAssembledView(ST,true); 
	if(ST.L)
	{
		char* s1=strstr(ST.str,"\\");
		if(s1)s1[0]=0;
		strncpy(s,ST.str,200);
	}
	s[200]=0;
	return s;
}
...
{
	bool rez=false;
	BoolType* NT = dynamic_cast<BoolType*> (BT);
	if(NT)
	{
		NT->Value=Value;
		rez=true;
...
{
	bool rez=false;
	ArrayType* DA = dynamic_cast<ArrayType*> (BT);
	if(DA)
	{
		DA->Clear();
		int n=GetNElements();
...
void VariablesList::RefreshEnumerator()
{
	...
	for(i=0;i<n;i++)
	{
		if(Variables[i]->Name.L)
		{
			...
			{
				Enumerator* En = ENUM.Get(Variables[i]->VarType.Get()->GetID());
				En->Add(Variables[i]->Name.str,(DWORD)(Variables[i]));
			}
		}
	}
}
...
void BrigadeVariablesList::RefreshEnumerator()
{
	...
	En->Add(FormID.Name.str,(DWORD)(&FormID));
	En->Add(UnitID.Name.str,(DWORD)(&UnitID));
	En->Add(FinalStateOrder.Name.str,(DWORD)(&FinalStateOrder));
	En->Add(NShooters.Name.str,(DWORD)(&NShooters));
	En->Add(NMelee.Name.str,(DWORD)(&NMelee));
	En->Add(NDying.Name.str,(DWORD)(&NDying));
	En->Add(NMoving.Name.str,(DWORD)(&NMoving));
	En->Add(NStanding.Name.str,(DWORD)(&NStanding));
	En->Add(NAttMoving.Name.str,(DWORD)(&NAttMoving));
	En->Add(NAttStanding.Name.str,(DWORD)(&NAttStanding));
	En->Add(Panic.Name.str,(DWORD)(&Panic));
	En->Add(Tired.Name.str,(DWORD)(&Tired));
	En->Add(NRecharging.Name.str,(DWORD)(&NRecharging));
	En->Add(NPAttack.Name.str,(DWORD)(&NPAttack));
	En->Add(NUAttack.Name.str,(DWORD)(&NUAttack));
	En->Add(NFireReady.Name.str,(DWORD)(&NFireReady));
	En->Add(DistToMainTarget.Name.str,(DWORD)(&DistToMainTarget));
	En->Add(IsStandGround.Name.str,(DWORD)(&IsStandGround));
	En->Add(Arsenal.Name.str,(DWORD)(&Arsenal));
	En->Add(Morale.Name.str,(DWORD)(&Morale));//GEC
	En->Add(Food.Name.str,(DWORD)(&Food));//GEC
...
bool Procedure::SetArguments(Operand* ArgumentList)
{
	...
	if(ArgumentList)
	{
		OperandParamList* OPL = dynamic_cast<OperandParamList*>(ArgumentList);
		if(OPL)
		{
...
bool ForEachNumerical::Execute()
{
	...
	NumericalArray* NA = dynamic_cast<NumericalArray*>(NAv->VarType.Get());
	NumericalType* NV = dynamic_cast<NumericalType*>(NVv->VarType.Get());
...
bool ForFromTo::Execute()
{
	...
	if(From.Get()&&To.Get()&&OB.GetNArguments())
	{
		...
		NumericalType* NV = dynamic_cast<NumericalType*>(Varr->VarType.Get());
		NumericalType NFrom;
		NumericalType NTo;
		From.Get()->Get(&NFrom);
		To.Get()->Get(&NTo);
...
bool Logical::Get(BaseType* BT)
{
	...
	if(Op1.Get()&&Op2.Get())
	{
		BoolType* Boo = dynamic_cast<BoolType*> (BT);
		if(Boo)
		{
			BoolType Boo1, Boo2;
...
bool OneStringExpressions::Get(BaseType* BT)
{
	...
	if(Op.Get())
	{
		NumericalType* Boo = dynamic_cast<NumericalType*> (BT);
		if(Boo)
		{
			rez = Op.Get()->Get(Boo);
		}
...
//==================================================================================================================//
int UserFriendlyNumericalReturner::Get()
{
	NumericalType NT;
	if(Value.Get(&NT))
		return NT.Value;
		...
		for(int i=0;i<n;i++)
		{
			if(NewWord)
			{
				...
				else
				{
					...
					Wor->Allocate(i-nwi);
					Wor->Type=2;
					strncat(Wor->str,DS->str+nwi,i-nwi);
					Wor->L=i-nwi;
					Words.Add(Wor);
					NewWord=false;
				}
			}
			if((!NewWord)&&isalpha(DS->str[i]))
			{
				nwi=i;
				NewWord=true;
			}
			...
			if(NewDigit)
			{
				if(!isdigit(DS->str[i]))
				{
					...
					Wor->Allocate(i-ndi);
					Wor->Type=1;
					strncat(Wor->str,DS->str+ndi,i-ndi);
					Wor->L=i-ndi;
					Words.Add(Wor);
					NewDigit=false;
				}
			...
			if((!NewDigit)&&isdigit(DS->str[i])&&(!NewWord))
			{
				ndi=i;
				NewDigit=true;
				if(NewCom)
			...
			bool enn=false;
			if(DS->str[i]=='('||DS->str[i]==')')
			{
				enn=true;
			}
			else
			if((!isalnum(DS->str[i]))&&(!isspace(DS->str[i])))
			{
				if(DS->str[i]=='\"')
				{
					char* st = strstr(DS->str+i+1,"\"");
					if(st)
					{
						int ni = st-(DS->str+i);
						licsema* Wor = new licsema();
						Wor->Allocate(ni);
						Wor->Type=4;
						strncat(Wor->str,DS->str+i+1,ni-1);
						Wor->L=ni-1;
						Words.Add(Wor);
						i=i+ni+1;
					}
			...
			if(enn)
			{
				if(NewCom)
				{
					...
					Wor->Allocate(i-nci);
					Wor->Type=3;
					strncat(Wor->str,DS->str+nci,i-nci);
					Wor->L=i-nci;
					Words.Add(Wor);
					NewCom=false;
				}
			...
			if(DS->str[i]=='('||DS->str[i]==')')
			{
				...
				Wor->Allocate(1);
				Wor->Type=3;
				strncat(Wor->str,DS->str+i,1);
				Words.Add(Wor);
			}

			if(isspace(DS->str[i]))
			{
				if(DS->str[i]==13&&Words.GetAmount()>0)
				{
					if(!(Words.End()->L&&Words.End()->str[0]==13))
					{
						licsema* Wor = new licsema();
						Wor->Add("\n");
						Wor->Type=0;
...
bool OneScript::ReadVariablesList(VariablesList* VL)
{
	...
	bool haveend=false;
	BaseType* T=NULL;
	if(strstr(vart.str,Words[Point]->str))
	{
		rez=false;
		Point++;
		for(int i=Point;i<n;i++)
		{
			if(strstr(begint.str,Words[i]->str)||strstr(Brigvar.str,Words[i]->str))
			{
				Point=i;
				haveend=true;
				break;
...
void OneScript::MarkVariables()
{
	...
	for(int i=0;i<n;i++)
	{
		for(int j=0;j<m;j++)
		{
			if(Words[j]->Type==2)
			{
				if(!strcmp(Words[j]->str,VAR.Variables[i]->Name.str))
				{
...
bool OneScript::ParseOperand(Operand* Op,int& point)
{
	...
	if(Op)
	{
		...
		if(intp<maxintp)
		{
			OperandBlock* OB = dynamic_cast<OperandBlock*> (Op);
			Procedure* PR = dynamic_cast<Procedure*> (Op);
			NumericalReturner* NR = dynamic_cast<NumericalReturner*> (Op);
			if(OB)
			{
				...
				if(nob==1)
				{
					...
					if(Mas.L>0)
					{
						if(strstr(Mas.str,Words[intp]->str))
						{
							intp++;
						...
						do 
						{
							...
							for(int j=0;j<m;j++)
							{
								...
								if(OCS&&OCS->OneMemb)
								{
									...
									Procedure* On = dynamic_cast<Procedure*> (BC);
									if(On)
									{
			...
			VariableReference* VR = dynamic_cast<VariableReference*> (Op);
			if(VR)
			{
				NumericalVariableReference* NVR = dynamic_cast<NumericalVariableReference*> (Op);
				if(NVR)
				{
			...
			StringConst* SC = dynamic_cast<StringConst*> (Op);
			if(SC)
			{
				if(Words[intp]->Type==4)
				{
					SC->Value.String.Add(Words[intp]->str);
					intp++;
					if(intp<maxintp)
						point=intp;
					else
			...
			NumericalConst* NC = dynamic_cast<NumericalConst*> (Op);
			if(NC)
			{
				if(Words[intp]->Type==1)
				{
					NC->Value.Value=atoi(Words[intp]->str);
					intp++;
					if(intp<maxintp)
						point=intp;
					else
			...
			NumericalArrayElement* NAE = dynamic_cast<NumericalArrayElement*> (Op);
			if(NAE)
			{
				return false;
			}
			
			If_Else* IfE = dynamic_cast<If_Else*> (Op);
			if(IfE)
			{
				...
				ParseDString(D3,&L3);
				int p=0;
				if(!strcmp(L1[0]->str,Words[intp]->str))//If
				{
					intp++;
					if(!strcmp(L1[1]->str,Words[intp]->str))//(
					{
						intp++;
						if(L1[2]->Type==99)// <0>
						{
							...
							{
								if(!strcmp(L1[3]->str,Words[intp]->str))//)
								{
									intp++;
									if(!strcmp(L1[4]->str,Words[intp]->str))//\n
									{
										intp++;
										if(ParseOperand((Operand*)(&IfE->OB1),intp))// Block
										{
											if(!strcmp(L2[0]->str,Words[intp]->str))// else
											{
												intp++;
												if(!strcmp(L2[1]->str,Words[intp]->str))// \n
												{
													intp++;
											...
											if(!strcmp(L3[0]->str,Words[intp]->str)) // endif
											{
												intp++;
												point=intp;
												return true;
			...
			BVariables* BVR = dynamic_cast<BVariables*> (Op);
			if(BVR)
			{
				ClassArray<licsema> LL;
				ParseViewMask(Op,&LL);
				if(!strcmp(LL[0]->str,Words[intp]->str))
				{
					intp++;
					if(!strcmp(LL[1]->str,Words[intp]->str))
					{
						intp++;
						static Enumerator* E_BVR = ENUM.Get("BrigadeVariablesListEnum");
						int nen=E_BVR->Get(Words[intp]->str);
						if(nen!=-1)
						{
							intp++;
							BVR->VarPoint=nen;
			...
			Op_Add* OpA = dynamic_cast<Op_Add*> (Op);
			Logical* Log = dynamic_cast<Logical*> (Op);
			CompareFun* CF = dynamic_cast<CompareFun*> (Op);
			//if(PR)
			{
				bool secses=true;
				ClassArray<licsema> LL;
				...
				for(int i=0;i<nll&&secses;i++)
				{
					...
					{
						if(strcmp(LL[i]->str,Words[intp]->str))
						{
							if(OpA)
							{
								Enumerator* Enm = ENUM.Get("ARIPHMETICALOPS");
								int en = Enm->FindStr(Words[intp]->str);
								if(en!=-1)
								{
									OpA->Op=en;
								}
		...
		for(int i=0;i<Mask.L;i++)
		{
			if(NewWord)
			{
				...
				else
				{
					...
					Wor->Allocate(i-nwi);
					Wor->Type=2;
					strncat(Wor->str,Mask.str+nwi,i-nwi);
					Wor->L=i-nwi;
					W->Add(Wor);
					NewWord=false;
				}
			}
			if((!NewWord)&&isalpha(Mask.str[i]))
			{
				nwi=i;
				NewWord=true;
			}
			...
			if(NewDigit)
			{
				if(!isdigit(Mask.str[i]))
				{
					licsema* Wor = new licsema();
					Wor->Allocate(i-ndi);
					Wor->Type=1;
					strncat(Wor->str,Mask.str+ndi,i-ndi);
					Wor->L=i-ndi;
					W->Add(Wor);
					NewDigit=false;
				}
			}
			if((!NewDigit)&&isdigit(Mask.str[i])&&(!NewWord)&&(!NewIdent))
			{
				ndi=i;
				NewDigit=true;
			}
			...
			bool endcom=false;
			bool slesh=false;
			if((!isalnum(Mask.str[i]))&&(!isspace(Mask.str[i])))
			{
				if(Mask.str[i]=='<')
				{
					ni=i;
				...
				if(Mask.str[i]=='\\')
				{
					...
					if((!NewCom)&&(!NewIdent))
					{
						if(Mask.str[i]=='('||Mask.str[i]==')')
						{
							licsema* Wor = new licsema();
							Wor->Allocate(1);
							Wor->Type=3;
							strncat(Wor->str,Mask.str+i,1);
							W->Add(Wor);
						}
						else
						{
					...
					if(NewIdent&&Mask.str[i]=='>')
					{
						licsema* Wor = new licsema();
						Wor->Allocate(i-ni);
						Wor->Type=99;
						strncat(Wor->str,Mask.str+ni+1,i-ni-1);
						Wor->L=i-ni-1;
						W->Add(Wor);
						NewIdent=false;
					}
			...
			if(NewCom&&endcom)
			{
				...
				Wor->Allocate(i-nci);
				Wor->Type=3;
				strncat(Wor->str,Mask.str+nci,i-nci);
				Wor->L=i-nci;
				W->Add(Wor);
				NewCom=false;
			}
	...
	if(Op)
	{
		...
		if(Arg->GetEditableElementExpansionList(Ep,0,ArgPtr,7))
		{
			...
			for(int j=0;j<m&&(!rez);j++)
			{
				...
				{
					BaseClass* BC = OCS->OneMemb->new_element();
					Operand* On = dynamic_cast<Operand*> (BC);
	...
	if(DS&&WordList)
	{
		...
		for(int i=0;i<n;i++)
		{
			...
			if(DS->str[i]=='('||DS->str[i]==')')
			{
				...
				if(isspace(DS->str[i]))
				{
					if(DS->str[i]==13&&WordList->GetAmount()>0)
					{
						if(!(WordList->End()->L&&WordList->End()->str[0]==13))
						{
							licsema* Wor = new licsema();
							Wor->Add("\n");
							Wor->Type=0;
		...
		if(intp<maxintp)
		{
			...
			if(OB)
			{
				...
				if(nob==1)
				{
					...
					if(Mas.L>0)
					{
						if(strstr(Mas.str,(*WordList)[intp]->str))
						{
							intp++;
			...
			if(VR)
			{
				...
				if(NVR)
				{
					if((*WordList)[intp]->SecondName=="NumericalVariable")
					{
						...
						if(Enm)
						{
							int n=Enm->Get((*WordList)[intp]->str);
							if(n!=-1)
							{
								NVR->VarPoint=n;
								intp++;
			...
			if(SC)
			{
				...
				{
					SC->Value.String.Add((*WordList)[intp]->str);
					intp++;
					if(intp<maxintp)
						point=intp;
					else
			...
			if(NC)
			{
				...
				{
					NC->Value.Value=atoi((*WordList)[intp]->str);
					intp++;
					if(intp<maxintp)
						point=intp;
					else
			...
			if(IfE)
			{
				...
				ParseDString_(D3,&L3);
				int p=0;
				if(!strcmp(L1[0]->str,(*WordList)[intp]->str))//If
				{
					intp++;
					if(!strcmp(L1[1]->str,(*WordList)[intp]->str))//(
					{
						intp++;
						if(L1[2]->Type==99)// <0>
						{
							...
							{
								if(!strcmp(L1[3]->str,(*WordList)[intp]->str))//)
								{
									intp++;
									if(!strcmp(L1[4]->str,(*WordList)[intp]->str))//\n
									{
										intp++;
										if(ParseOperand_((Operand*)(&IfE->OB1),intp,WordList))// Block
										{
											if(!strcmp(L2[0]->str,(*WordList)[intp]->str))// else
											{
												intp++;
												if(!strcmp(L2[1]->str,(*WordList)[intp]->str))// \n
												{
													intp++;
											...
											if(!strcmp(L3[0]->str,(*WordList)[intp]->str)) // endif
											{
												intp++;
												point=intp;
												return true;
			...
			if(BVR)
			{
				...
				if(!strcmp(LL[0]->str,(*WordList)[intp]->str))
				{
					intp++;
					if(!strcmp(LL[1]->str,(*WordList)[intp]->str))
					{
						intp++;
						static Enumerator* E_BVR = ENUM.Get("BrigadeVariablesListEnum");
						int nen=E_BVR->Get((*WordList)[intp]->str);
						if(nen!=-1)
						{
							intp++;
							BVR->VarPoint=nen;
			...
			//if(PR)
			{
				...
				for(int i=0;i<nll&&secses;i++)
				{
					...
					{
						if(LL[i]->str&&WordList->GetAmount()>intp&&strcmp(LL[i]->str,(*WordList)[intp]->str))
						{
							if(OpA)
							{
								Enumerator* Enm = ENUM.Get("ARIPHMETICALOPS");
								int en = Enm->FindStr((*WordList)[intp]->str);
								if(en!=-1)
								{
									OpA->Op=en;
								}
