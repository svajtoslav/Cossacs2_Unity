...
void InputBox_LMousePressed(SimpleDialog* SD, int x, int y){
	...
	x+=IB->totdx;
	int length=strlen(IB->Str?IB->Str:IB->_Str->str);
	char base[2048];
	if(IB->Anonim)
...
bool InputBox_OnDraw(SimpleDialog* SD){
	...
	if(IB->Str || IB->_Str){
		...
		if(IB->_Str&&!IB->_Str->str) *IB->_Str="";
		strcpy(base,IB->Str?IB->Str:IB->_Str->str);
		int length=strlen(IB->Str?IB->Str:IB->_Str->str);
...
bool InputBox_OnKeyDown(SimpleDialog* SD){
	...
	if(LastKey==65 && (GetKeyState(VK_CONTROL)&0x8000)){ // Ctrl A
		...
		char cc[2048];
		strcpy(cc,(IB->Str?IB->Str:IB->_Str->str)+min(IB->Selected,IB->CursPos));
		...
		int size=IB->StrMaxLen-strlen(IB->Str?IB->Str:IB->_Str->str)+end_ins-start_ins;
		char cc[2048];
		cc[0]=0;
		...
		ccc[start_ins]=0;
		strcat(ccc,cc);
		strcat(ccc,(IB->Str?IB->Str:IB->_Str->str)+end_ins);
		...
		/*if(GetKeyState(VK_SHIFT)&0x8000){
			...
			IB->Selected=-1;
		IB->CursPos+=8;
		if(IB->CursPos>strlen(IB->Str?IB->Str:IB->_Str->str))
			IB->CursPos=strlen(IB->Str?IB->Str:IB->_Str->str);*/
		KeyPressed=0;
		LastKey=0;
		return true;
	...
	if(LastKey==VK_BACK){
		...
		if(IB->CursPos>0 || IB->Selected>0){
			...
			if(IB->Multiline){
				char cc[2048];
				strcpy(cc,IB->Str?IB->Str:IB->_Str->str);
				if(cc[start_back]==10){
	...
	if(LastAsciiKey){
		if(LastAsciiKey>=32){
			...
			if(IB->Selected>=0 && IB->Selected!=IB->CursPos)
				...
				start_ins=IB->CursPos;
			int end_ins=max(IB->CursPos,IB->Selected);
			if(strlen(IB->Str?IB->Str:IB->_Str->str)+1<DWORD(IB->StrMaxLen+end_ins-start_ins)){
				char ccc[2048];
				strcpy(ccc,IB->Str?IB->Str:IB->_Str->str);
				ccc[start_ins]=0;
				strcat(ccc,xx);
