...
void vmCheckVictory(){
	...
	if(vmVictory){
		if(CurEW2miss>=CurPlayer->c_NDone){
...
void cvi_mfGame::Play(){
	...
	strcpy(g->cgi_CurrentMap,MissFileName.str);	
	...
	if(Scroll){
		...
		for(int i=0;i<aN;i++){			
			...
			if(BP){
				...
				if(i<n||cyc){
					if(cyc){
						BP->SetFileName(Pics[i-2]->str);
					}else{
						BP->SetFileName(Pics[i]->str);
					}					
					BP->Setx(x);
		...
		BP->FileName.Assign(Picture.str);
	}	
	ov_Play(Sound.str,0);
	...
	if(Scroll&&ScrollLx&&GetTickCount()>vov_Time){
		if(ScrollSpeed==0){
			...
			if(t>0){
				...
				if(n){
					for(int i=0;i<n;i++){
						Lx+=mv_FrescoWidth[i];//GetBitPicWidth(Pics[i]->str);
					}	
					ScrollLx=-Lx;
...
void cvi_mfFrescos::Play(){
	if(Frescos.GetAmount()>0){
		...
		*/
		//
		//ov_SetVolume(25);
		//ov_Play("Sound\\interface\\music.ogg",0);
		//ov_Cyclic(true,0);		
		vfVol=GetCDVolume();
...
void cvi_mfMovie::Play(){
	...
	PlayFullscreenVideo( Movie.str , px , py ); //0.17
	SetCDVolumeEx(vol);
}
//////////////////////////////////////////////////////////////////////////
...
#else
				id=vmSinglePlayerData.NDoneMission[vmCampID];
				if(id>=nm){
					id=nm-1;
				}
	...
	for(int i=0;i<nGP;i++){
		...
		if(GP){
			...
			if(M&&M->MapX>0&&i==cva_Camp_StartMission::MissionID&&i<=vmSinglePlayerData.NDoneMission[vmCampID]){
				//i!=cva_Camp_StartMission::MissionID
				GP->Visible=true;
...
void cva_CampDiff::SetFrameState(SimpleDialog* SD){
	...
	if(CB){
		...
		if(tbMD){
			...
			if(mid>=0){
				int n=min(vmSinglePlayerData.NDoneMission[vmCampID]+1,vmCamp->Missions.GetAmount());
				if(n&&mid>=n){
					mid=n-1;
...
void rcm_InitStatistic(){
	if(vGameMode==gmCamp&&gExpMan()&&vmCamp&&vmCampID>=0){
...
void cva_HeroesUpgrades::SetFrameState(SimpleDialog *SD)
{
	...
	hu_Controls.tbLPriceAA->SetMessage(T.str);
	hu_Controls.tbLPriceAB->SetMessage(T.str);
	hu_Controls.tbLPriceAC->SetMessage(T.str);
	...
	hu_Controls.tbLPriceBA->SetMessage(T.str);
	hu_Controls.tbLPriceBB->SetMessage(T.str);
	hu_Controls.tbLPriceBC->SetMessage(T.str);
	...
	hu_Controls.tbLPriceCA->SetMessage(T.str);
	hu_Controls.tbLPriceCB->SetMessage(T.str);
	hu_Controls.tbLPriceCC->SetMessage(T.str);
}
int dExp = 0;
