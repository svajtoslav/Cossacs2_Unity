	...
	if(NPlayers<=1){
		...
		if(GSets.SVOpt.RequiredMsPerFrame==120){
			...
			if(DTMT>8){
				...
				int GSpeed=GSP*256/GSets.SVOpt.RequiredMsPerFrame;
...
//	StopPF(19);
	...
	if(NOPAUSE){
		...
		*/
		TrueTime+=(GameSpeed*GSets.SVOpt.RequiredMsPerFrame)/256;
		AnimTime+=GameSpeed;
	...
	if(NOPAUSE){
		...
		*/
		...
		for(i=0;i<8;i++){
			...
			for(int j=0;j<8;j++){
				int R=NT->ResRem[j];
				R+=NT->ResSpeed[j]*10*GameSpeed/25;
				div_t dd=div(R,mult);
				R=dd.rem;
				addrand(XRESRC(i,j));
				addrand(dd.quot);
...
{	
	...
	SpecCmd=0;
	sfVersion=291;
	strcpy(BuildVersion,"V 1.1");//"V 1.46"); //CDV ASK GEC 13.03
	//Quality=2;
	RealLx=1024;
	RealLy=768;
	GSets.SVOpt.ScreenSizeX=1024;
	...
	*/
	...
	/*if(GSets.VideoList.GetAmount()){
		...
		for(int i=0;i<GSets.VideoList.GetAmount();i++){
            PlayFullscreenVideo(GSets.VideoList[i]->File.str,1.0-GSets.VideoList[i]->ScaleX,1.0-GSets.VideoList[i]->ScaleY);
		}
	}*/
	if(GSets.LeibzigDemo.Enabled){
...
void ShowProfInit(){
    IRS->ClearDeviceTarget(0xFF4040A0);
