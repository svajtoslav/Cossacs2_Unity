...
bool CMesh3D::FillNormals(BaseMesh &rNormals)
{
	...
	rNormals.setTexture(IRS->GetTextureID("wtexture.bmp"));
}
...
void CMesh3D::generatorSphere(CVertex3D c, float r, int db, int dl, CVertex3D Scale)
{
	Clear();
...
bool Sections2BaseMesh(const CSections3D &rSections, const CVertex3D &rOrigin, BaseMesh &rBaseMesh)
{
	...
	rBaseMesh.setTexture(IRS->GetTextureID("wtexture.bmp"));
}
...
{
	CMesh3D A, B;
	A.generatorSphere(CVertex3D(200, 900, 0), 200, 10, 10, CVertex3D(1.0f, 1.0f, 3.0f));
	B.generatorSphere(CVertex3D(200, 1000, 200), 200, 10, 10, CVertex3D(2.0f, 1.0f, 1.0f));

	CVertices3D Crosses;

	CMesh3D R, T;
	CSections3D Sections;
	CMesh3D::Difference(A, B, R, Crosses, T, Sections);
	...
	A.toBaseMesh(bmA, "WireS", "wtexture.bmp", c_Red - 0x80000000, CVertex3D::Null);
	B.toBaseMesh(bmB, "WireS", "wtexture.bmp", c_Green - 0x80000000, CVertex3D::Null);
	R.toBaseMesh(bmR, "WireS", "wtexture.bmp", c_Yellow, CVertex3D(500, 0, 0));
	T.toBaseMesh(bmT, "WireS", "wtexture.bmp", c_White, CVertex3D(500, 0, 0));
...
//		for(i = 0; i < Polygons.NValues; i++) Polygon2Mesh2D(Polygons[i], RMesh);
		...
		Mesh.setTexture(IRS->GetTextureID("wtexture.bmp"));
		IRS->Draw(Mesh);
