...
public:
	...
	virtual const char* GetThisElementView(){
		if(!Description.str)return NULL;
		static char cc[128];
		sprintf(cc,"Condition: %s",Description.str);
		return cc;
	};
	TCondition(){
		Enabled=0;
...
public:
	...
	virtual const char* GetThisElementView(){
		if(!Description.str)return NULL;
		static char cc[128];
		sprintf(cc,"Terrain: %s",Description.str);
		return cc;
	};
	SAVE(TerrParams)
	REG_AUTO(Description);
...
public:
	...
	virtual const char* GetThisElementView(){
		if(!Description.str)return NULL;
		static char cc[128];
		sprintf(cc,"Relief: %s",Description.str);
		return cc;
	};
	SAVE(OneReliefRule)
	REG_AUTO  (Description);
...
void PaintMapWith(){
	...
	LandsScope* TP=new LandsScope;	
	TP->RegisterMembers();
	xmlQuote xml;
	if(xml.ReadFromFile("Landscape.xml")){
		ErrorPager Error;
	...
	//testFilePathEditor* TPZ = new testFilePathEditor();
	static ClassEditor CE;
	static DialogsSystem GDS;
	GDS.CloseDialogs();
	int CED_DLG_LX=440;
	int CED_DLG_LY=500;
	DialogsDesk* DD=GDS.AddDialogsDesk(RealLx/2-220,RealLy/2-250,CED_DLG_LX,CED_DLG_LY,"StandartBorderNotScrollable");
	...
	const char* msgs[]={"Load","Save","Generate","PaintOnly","ReliefOnly","Cancel",""};
	TextButton* TBS[6];
	int Tl=0;
	...
	int xx=Tl;
	for(int i=1;i<NBTN;i++){
		TextButton* TB=DD->addTextButton(NULL,xx,CED_DLG_LY-BTDY,(char*)msgs[i-1],AF,PF,PF,0);
	...
	do{
		...
		if(MLayers->Maps.GetAmount()){
			...
			int NR=TP->Relief.GetAmountOfElements();
			for(int i=0;i<NR;i++){
				_str* s=&MLayers->Maps[TP->Relief.Rules[i].Layer.TopLayerIndex]->Desc;
				*s+=TP->Relief.Rules[i].Description;
				*s+=" ";
			}
			for(int i=0;i<MaxTerr;i++){
				for(int j=0;j<MaxCond;j++){
					if(TP->Terr[i].Condition[j].Mask.Enabled){
						_str* s=&MLayers->Maps[TP->Terr[i].Condition[j].Mask.Map.TopLayerIndex]->Desc;
						*s+=TP->Terr[i].Condition[j].Description;
						*s+=" ";
					}
				}
			}
	...
	if(LastKey==13){
		...
		if(!(GetKeyState(VK_CONTROL)&0x8000)){
			...
			DeleteAllSprites();
			xml2.WriteToFile("Landscape.xml");
		...
		if(TP->GenerateRelief)TP->Relief.GenerateHeight();
		if(TP->GenerateTextures){
			...
			for(int i=0;i<MaxPointIndex;i++){
				...
				TexResult tr;
				bool Bound=TP->GetResult(x,y,H,A,tr);
				...
				if(trees>=0&&trees<255&&mrand()<tw){
					...
					if(TSC.TGroup[trees]->SGS[idx]->Objects[TSC.TGroup[trees]->Indices[idx]]->ModelManagerID<=0){
			...
			if(TP->PostProcessType==2){
				for(int j=0;j<TP->PostStagesAmount;j++){
