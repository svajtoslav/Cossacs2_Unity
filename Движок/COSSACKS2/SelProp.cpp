...
void LoadMessages(){
	...
	LoadTextFromDir("Text\\");
	LoadTextFromDir("Missions\\Text\\");
	//
...
char* GetTextByID(char* id){
	...
	if(!GetHintDebugMode()) return id;
	///////////////////////////////////////////////////////////////////////////////
	static bool initMissed=false;
	if(!initMissed){
		initMissed=true;
		FILE* F;		
		F=fopen("Text\\ce_text.missed","w"); if(F)fclose(F);
		F=fopen("Text\\text.missed","w"); if(F)fclose(F);
		F=fopen("Text\\dialogs.missed","w"); if(F)fclose(F);
	}	
	if(GetHintDebugMode()&&ID[0]=='@'&&strstr(ID,"::")){
        int id=UnkIDS.find(ID);
		if(id==-1){
            UnkIDS.add(ID);
			FILE* F=fopen("Text\\ce_text.missed","a");
			if(F){
				fprintf(F,"%s  \n",ID);
				fclose(F);
			}
	...
	if(GetHintDebugMode()&&v_DialogsMissed){
		FILE* F=fopen("Text\\dialogs.missed","a");
		if(F){
			_str sid;
			if(ID[0]=='#'){
				fprintf(F,"%s \n",id);
	...
	//
	if(v_DialogsMissed){
		static StringsHash WrongTextID;
		int i=WrongTextID.find(ID);
		if(i==-1){
			WrongTextID.add(ID);
			FILE* F=fopen("Text\\text.missed","a");
			if(F){
				fprintf(F,"%s\n",ID);
				fclose(F);
			}
