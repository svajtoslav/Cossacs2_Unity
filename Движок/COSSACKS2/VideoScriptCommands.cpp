...
//void ov_Play(LPCSTR pcszFileName,DWORD dwStream=0);
//DWORD ov_StreamFinished(DWORD dwStream=0);
//void ov_Stop(DWORD dwStream=0);
//==================================================================================================================//
		...
		if(N)
		{
			...
			//bool played=false;
			{
				...
				for(int i=0;i<NOgg_Stream;i++)
				{
					if(ov_StreamFinished(FirstOgg_Stream+i))
					{
						Ogg_Stream_FileName[i]=(*CommList)[N]->str;							
						static int volume=-1;
						...
						//}						
						ov_Play((*CommList)[N]->str,FirstOgg_Stream+i);
	...
	if(Name.Get())
	{
		...
		if(Name.Get()->Get(&ST))
		{
			if(ActiveIndexses.NCom<64)
			{
				...
				ComListDS[ActiveIndexses.NCom].Add(ST.String.str);
				if(ActiveIndexses.ComList)
				{
					ActiveIndexses.ComList[ActiveIndexses.NCom]=ComListDS[ActiveIndexses.NCom].str;
					ActiveIndexses.NCom++;
				}
			}
			rez=true;
