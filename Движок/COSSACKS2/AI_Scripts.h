...
public:	
	...
	virtual void GetBriefing(_str& str){		
		...
		for(int i=0;i<Condition.GetAmount();i++){
			...
			if(strstr(str1.str," and "))Complex=true;
		}	
		_str str2;
		if(Complex){			
			str2=str1.str;
...
public:
	...
	virtual int Get(xmlQuote* xml){
		sprintf(Name,"%s=%s",U->ShortName.str,NATIONS->Mon[U->UnitID]->MonsterID);
		return true;
	}
	virtual OneElementEditor* _new(){
...
public:
		...
		BaseClass* Class,void* DataPtr,void* ExtraPtr, ControlParams* CParam)
	{
		...
		ai->GetBriefing(brief);
		Name[0]=0;
		if(ai->Description.str)strcpy(Name,ai->Description.str);
		if(Name[0])strcat(Name,", ");
		strcat(Name,brief.str);
		InputBox* IB=Base->addInputBox(NULL,x,y,Name,200,x1-x,y1-y,&CED_Font,&CED_Font);
		IB->Enabled=0;
		return y1;
	}
...
public:
		...
		BaseClass* Class,void* DataPtr,void* ExtraPtr, ControlParams* CParam)
	{
		...
		_str brief;
		brief=ai->Description.str?ai->Description.str:"";
		for(int i=0;i<ai->Action.GetAmount();i++){
            if(brief.str&&brief.str[0])brief+=", ";
			_str s1;
			ai->Action[i]->GetShortBriefing(s1);
			brief+=s1.str;
		}
		if(brief.str){
			strncpy(Name,brief.str,200);
			Name[200]=0;
		}
		InputBox* IB=Base->addInputBox(NULL,x,y,Name,200,x1-x,y1-y,&CED_Font,&CED_Font);
		IB->Enabled=0;
...
public:
	...
	virtual bool Perform(){
		...
		TryUpgradeEx(CurAI_Color,UpgradeID,CostPercent,100,100000,100000);	
		return true;
	};
	virtual void GetBriefing(_str& str){
