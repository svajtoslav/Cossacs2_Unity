...
byte* XYShift=NULL;//map of horisontal shifting
#ifdef _USE3D
byte* FactureMap;
byte* FactureWeight;
byte* FactureShift;
...
DWORD GetTexAverageColor(int x,int y){
	...
	int tid=WTexMapEx[ofs]>0x80?TexMapEx[ofs]:TexMap[ofs];
	if(tid>=0&&tid<64)return TexAverageColor[tid];
	return 0xFF000000;
}

int GetBmOfst(int i);
void Loadtextures(){
	for(int i=0;i<64;i++)TexDiffuse[i]=0xFF808080;
	...
	if(ReadBMP24("Textures\\GroundTex.bmp",&bm,&data)){
		for(int ix=0;ix<8;ix++){
			for(int iy=0;iy<8;iy++){
				int ofs0=(ix*64+iy*512*64)*3;
				int R=0;
				int G=0;
				int B=0;
				for(int dx=0;dx<64;dx++){
					for(int dy=0;dy<64;dy++){
                        int rofs=ofs0+(dx+(dy<<9))*3;
						B+=int(data[rofs  ]);
						G+=int(data[rofs+1]);
						R+=int(data[rofs+2]);
					}
				...
				G/=4096;
				B/=4096;
				TexAverageColor[ix+iy*8]=0xFF000000+(R<<16)+(G<<8)+B;
			}
		}
        free(data);
	}
