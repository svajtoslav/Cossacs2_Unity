		...
		for(int j=0;j<NNations;j++){
			if(strcmp(NatScripts[j],"-")&&(LOADNATMASK&m0)){
				...
				for(int i=0;i<MOptions.CustomNDS.GetAmount();i++)
					if(MOptions.CustomNDS[i]->OriginalNDS.str&&!stricmp(destscript,MOptions.CustomNDS[i]->OriginalNDS.str)){
					if(MOptions.CustomNDS[i]->CustomNDS.str)destscript=MOptions.CustomNDS[i]->CustomNDS.str;
                    break;
				}
