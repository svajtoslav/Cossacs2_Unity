...
void PutTexInPoint(int i,SelectionRequest* SR,int T=-1){
	...
	int R;
	if(SR->BrushType){
		R=tmax(abs(xp-SR->x+yp-SR->y),abs(xp-SR->x-yp+SR->y))-SR->r;
	}else{
...
void SetTexturedRound(int x,int y,int rx,byte Tex){
	...
	if(CheckIfNewTerrain() == true)
	{
		...
		DWORD CLLIST[128];
		for(int i=0;i<NTextures;i++)CLLIST[i]=TXCOLOR[TexList[i]];		
		// END: SILENT HILL!!!
		int nTexture = 0;
		nTexture = GetTexture();
		int r = 0, d = 0;
		...
		if(GetKeyState(VK_LCONTROL) & 0x8000){
			...
			if(GetTickCount() - tLast < 140) return;
			tLast = GetTickCount();
			g_UniversalMap.SmoothColors(x, y, r);
		}else g_UniversalMap.SetRandomColorsInRadius(x, y, r, d, CLLIST,NTextures);
		return;
	}
...
int GetMinimapTexture(){
	...
	if(MinimapTexture==0){
		...
		MinimapTexture=IRS->CreateTexture("MinimapTexture",td);
	}
...
int GetMinimapRoadsTexture(){
	...
	if(MinimapTexture==0){
		TextureDescr td;
		td.setValues(256,256,cfARGB4444,mpManaged,1,tuProcedural);
		MinimapTexture=IRS->CreateTexture("MinimapRoadsTexture",td);
	}
	return MinimapTexture;
}
void CreateMiniMap(){
...
void AddHi(int x,int y1,int r,int h){
	// BEGIN: NEW SURFACE CALLS
	bool CheckIfNewTerrain(void);
	if(CheckIfNewTerrain() == true)
	{
		extern UniversalMap g_UniversalMap;
		g_UniversalMap.ChangeHeightParabolic(x, y1, r, h);
		return;
	}
	// END: NEW SURFACE CALLS
...
void AddHiPlanar(int x,int y1,int r,int h){
	...
	{
		extern UniversalMap g_UniversalMap;
		g_UniversalMap.ChangeHeightPlanar(x, y1, r, h);
		return;
	}
	// END: NEW SURFACE CALLS
	int y=Prop43(y1);
...
void CreatePlane(int x,int y1,int r){
	...
	{
		extern UniversalMap g_UniversalMap;
		g_UniversalMap.EvenHeightInRadius(x, y1, r);
		return;
	}
	// END: NEW SURFACE CALLS
	int y=Prop43(y1);
...
void AverageHi(int x,int y1,int r){
	...
	{
		extern UniversalMap g_UniversalMap;
		g_UniversalMap.SmoothHeightInRadius(x, y1, r);
		return;
	}
	// END: NEW SURFACE CALLS
	int y=Prop43(y1);
...
void CreateLandLocking(int TAlp,bool ForVision){
	...
	BC=HorceLockBars.BC;
	for(i=0;i<N;i++){
		MFIELDS[2].BSetBar(int(BC[i].x)<<2,int(BC[i].y)<<2,4);
	};
	for(ix=1;ix<mxx;ix++)
...
extern int MinMapY;
extern int MaxMapY;
int LockCoef=120*64/56;
void CreateMapLocking(){
	int maxx=MaxMapX<<1;
	int maxy=MaxMapY<<1;
	ClearMaps();
	CreateLandLocking(LockCoef,true);	
	for(int f=2;f<NMFIELDS;f++) SmoothFields(MFIELDS+f,f);
	CreateFishMap();
	//Border setting
	int L1=32<<ADDSH;
...
void SHOWBARS(BlockBars* BB,DWORD c){
	...
	for(int i=0;i<BB->NBars;i++){
		...
		if(x>-32&&y>-32&&x<RealLx+32&&y<RealLy-32){
			GPS.ShowGP(x-16,y-16,idx,c,0);
			/*GPS.DrawLine(x,y-8,x,y+8,c);
			GPS.DrawLine(x-16,y,x+16,y,c);
			GPS.DrawLine(x,y-8,x+16,y,c);
			GPS.DrawLine(x-16,y,x,y+8,c);
			GPS.DrawLine(x,y-8,x-15,y,c);
			GPS.DrawLine(x,y+8,x+16,y,c);*/
		};
	};
};
