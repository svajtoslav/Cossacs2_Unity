...
public:
	...
	const char* GetThisElementView(const char* LocalName){
		if(NodeName.str)return NodeName.str;
        ...
        void EvaluateFunction(){
			...
			if(WE){
				...
				for(int i=0;i<N;i++){                    
					...
					int mnod=IMM->GetNodeID(WE->DestructedModelID,DP->NodeName.str);
					int anod=IMM->GetNodeID(WE->DestructedAnimationID,DP->NodeName.str);
					if(anod>0){						
						IGeometry* IG=IMM->GetGeometry(mnod);
                        float T=IMM->GetAnimTime(anod);
						if(T>0){
							WE->DestructionParts.Add(DP);
							float DT=T/500;
							IMM->Animate(mnod,anod,T-DT);
							Vector3D V1=IG->GetCenter();
							DP->Center=V1;
							Matrix4D M4=IMM->GetNodeTransform(mnod);
							M4.transformPt(V1);
							float z1=V1.z;
							IMM->Animate(mnod,anod,T-DT*2);
							Vector3D V2=IG->GetCenter();
							M4=IMM->GetNodeTransform(mnod);
							M4.transformPt(V2);
							float z2=V2.z;
                            float sp=(z2-z1)/DT;							
							float sp0=fabs(sp)/2;
							if(fabs(sp0)<0.0001)sp0=0.03;
							for(int j=497;j>=0;j--){
								IMM->Animate(mnod,anod,DT*j);
								Vector3D V1=IG->GetCenter();
								M4=IMM->GetNodeTransform(mnod);
								M4.transformPt(V1);									
								float z1=V1.z;
...
void SetTexFactInPoint(int V,int Texture0,int Facture0,float FactrureMaxWeight0,int Texture1,int Facture1,float FactrureMaxWeight1,int TW,int W){
	...
	for(int q=0;q<NTX;q++)WLIST[q]=(WLIST[q]*(255-W))>>8;
	bool T1P=0;
	for(int q=0;q<NTX;q++)if(TLIST[q]==Texture0){
		WLIST[q]+=TW1;
		T1P=1;
	}
	if(!T1P){
	...
	bool T2P=0;
	for(int q=0;q<NTX;q++)if(TLIST[q]==Texture1){
		WLIST[q]+=TW2;
		T2P=1;
	}
	if(!T2P){
	...
	do{
		ch=0;
		for(int q=1;q<NTX;q++){
			if(WLIST[q-1]<WLIST[q]){
				swap(WLIST[q-1],WLIST[q]);
				swap(TLIST[q-1],TLIST[q]);
				ch=1;
			}
		}
	}while(ch);
	if(NTX>1&&WLIST[0]>0){
		TexMap   [V]=TLIST[0];
		TexMapEx [V]=TLIST[1];
		WTexMapEx[V]=WLIST[1]*255/(WLIST[1]+WLIST[0]);
		if(TexMap[V]==Texture0){
			int WW=float(W)*FactrureMaxWeight0;
			if(WW>255)WW=255;
			FactureMap[V]=Facture0;
			FactureWeight[V]=WW;
		}
		if(TexMap[V]==Texture1){
			int WW=float(W)*FactrureMaxWeight1;
			if(WW>255)WW=255;
			FactureMap[V]=Facture1;
			FactureWeight[V]=WW;
		}
				...
				for(int t=0;t<Ditch.GetAmount();t++){
					...
					if(WDP&&((FRONT&&WDP->InFront)||(WDP->InBack&&!FRONT))){
						...
						if(dw>-WDP->InWidth&&dw<WDP->OutWidth){
							...
							if(DH>0)THMap[V]=min(THMap[V],H2);
							else if(DH<0)THMap[V]=max(THMap[V],H2);							
...
void OneWallPoint::Draw(float anmstage,OneObject* OB,WallTypeDescription* WT,OneWallSection* OWS){
	int T=LastWTaskT;
	Vector3D V=SkewPt(x,y,z);
    WorldToScreenSpace(V);	

	if(V.x<-100||V.x>RealLx+100||V.y<-100||V.y>RealLy+350)return;
	if(!GetObjectVisibilityInFog(x,y,z,OB))return;
	...
	if(CrashTime[0]!=0||DestructionMask!=0xFFFFFFFF
		...
		IMM->Animate(Type->DestructedModelID,Type->DestructedAnimationID,0);
		bool SOMETHING=0;
		if(ndp==0){
			...
			if(dt){
				if(dt<35000){
					...
					IMM->Animate(Type->DestructedModelID,Type->DestructedAnimationID,t);											
					SOMETHING=1;
				}
			}
		}
		for(int i=0;i<ndp&&i<MaxParts;i++){
			...
			if(OWDP){
				if(OWDP->NodeID<=0){
					OWDP->NodeID=IMM->GetNodeID(Type->DestructedModelID,OWDP->NodeName.str);
					OWDP->AnimNodeID=IMM->GetNodeID(Type->DestructedAnimationID,OWDP->NodeName.str);									
				}
				if(OWDP->NodeID<=0){
					LogWalls("Wrong node <%s> for model <%s>\n",OWDP->NodeName.str,IMM->GetModelFileName(Type->DestructedModelID));
				}
				if(OWDP->AnimNodeID<=0){
					LogWalls("Wrong node <%s> for animation <%s>\n",OWDP->NodeName.str,IMM->GetModelFileName(Type->DestructedAnimationID));
				}
				if(OWDP->NodeID>=0&&OWDP->AnimNodeID>=0){
					...
					if(dt){
						if(dt<35000){
							SOMETHING=1;
							float t=float(dt)*Type->TimeScale;
							IMM->Animate(OWDP->NodeID,OWDP->AnimNodeID,t);											
		...
		if(SOMETHING){
			...
			IMM->Render(Type->DestructedModelID,&M4);
			IMM->Render(Type->DestructedModelID,&M4);
		}
	}else if(Visible){
		if(Type&&Type->OpeningAnimationID>0){
			...
			if(T){
				IMM->Animate(Type->ModelID,Type->OpeningAnimationID,T*anmstage);
			}
		}
...
void AddWallSegment(byte NI,int Type,int x0,int y0,int x1,int y1){
	...
	maxEdge++;
	OneWallEdge* OWE0=OWS.GetNearEdge(x0,y0,min(300,WTD->MinWallLength-20));
	OneWallEdge* OWE1=OWS.GetNearEdge(x1,y1,min(300,WTD->MinWallLength-20));
	int ID1,ID2,ID3;
...
void TestWalls(){
	...
	if(WList.Walls.GetAmount()){		
		if(WallsEditMode){		
			...
			OneWallEdge* OWE=OWS.GetNearEdge(x,y,min(300,WTD->MinWallLength-20));
			...
			if(PrevClickX==0){
				...
				OneWallEdge* OWEP=OWS.GetNearEdge(PrevClickX,PrevClickY,min(300,WTD->MinWallLength-20));
...
void UpgradeWallObject(OneObject* OB,int NewType){
	...
	if(OWE){
		...
		if(ELM){
			OB->ClearOrders();
			OB->Ref.General=NATIONS[OB->NNUM].Mon[NewType];
	...
	for(int i=0;i<OWS.Edges.GetAmount();i++){
		...
		if(OWE&&!OWE->Dead){
			if(OWE->OwnerObjectID!=0xFFFF){
				...
				if(OB&&OB->Serial==OWE->OwnerObjectSN&&OB->newMons->UseLikeGate&&OB->NNUM==NI&&!OB->Sdoxlo){
					int dx=TCos[OB->RealDir];
					int dy=TSin[OB->RealDir];
					for(int q=-8;q<8;q++){
						int xx=(OB->RealX+dx*q)>>8;
						int yy=(OB->RealY+dy*q)>>8;
						if(State)BClrBar(xx-1,yy-1,3);
						else BSetBar(xx-1,yy-1,3);
					}
				}
			}
		}
...
bool TestDoubleWall(int x0,int y0, int x1, int y1){
    OneWallEdge* S=OWS.GetNearEdge(x0,y0,300);
	OneWallEdge* F=OWS.GetNearEdge(x1,y1,300);
	if(S&&F){
...
void SetGlobalClipPlane(Vector3D& pos,Vector3D& norm){
	pos=SkewPt(pos.x,pos.y,pos.z);
	//norm=SkewPt(norm.x,norm.y,norm.z);
    Plane P(pos,norm);
	IRS->SetClipPlane(0,P);
}
