...
void ExplodeAnmObject(AnmObject* AO,bool Landing){
	...
	if(CWP){
		...
		for(int i=0;i<NChild;i++){
			...
			addrand(79);
			if(BWpn){
				if(DebugWeapMode)LogWeap("%s_%d->%s\n",AO->Weap->Name.str,AO->ASerial,BWpn->Name.str);
...
bool Create3DAnmObjectEX(Weapon* Weap,int xs,int ys,int zs1,
	...
	if(OB&&OB->ActiveAbility){
		OB->ActiveAbility->ActiveAbilities.modifyWeapon(Weap,&Weap);
	}
	if(Weap->Modificator&&Weap->Modificator->Get())
	{		
		int Inn=0xFFFF;
		if(OB)Inn=OB->Index;
		CreateNewActiveWeapon(Weap,Inn,xs,ys,zs1,DestObj,xd,yd,zd,OB->Ref.General->MoreCharacter->MaxDamage[AttType],AttType);
		return true;
	}
	//
#ifdef _USE3D
...
void DrawWeaponUsing3D(byte Pass)
{
	...
	int shd=GPS.SetCurrentShader(wsh);
    
    Frustum frustum;
    ICam->GetFrustum( frustum );
...
int CheckImmediateAttackAbility(OneObject* Killer,OneObject* Victim,byte& Reason,bool Fast){
	...
	int d=dz*2;
	if(d>300)d=300;
	AdvCharacter* ADC=Killer->Ref.General->MoreCharacter;
...
public:
	...
	virtual DWORD GetIndexedValue (int idx){
		if(idx<nmbm)return (DWORD)GetWeaponByIdx(idx);
		else return (DWORD)GetNewWeaponPtr(idx-nmbm);
	}
