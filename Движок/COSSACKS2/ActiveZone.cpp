...
void ActiveZone::Draw(){
	...
	int L=GetRLCStrWidth(Name.str,&BigYellowFont);
	int L2=L>>1;
	...
	ShowString(xx-L2+1,yy-H-55+1,Name.str,&BigBlackFont);
	ShowString(xx-L2-1,yy-H-55+1,Name.str,&BigBlackFont);
	ShowString(xx-L2+1,yy-H-55-1,Name.str,&BigBlackFont);
	ShowString(xx-L2-1,yy-H-55-1,Name.str,&BigBlackFont);
	ShowString(xx-L2,yy-H-55,Name.str,&BigYellowFont);
};
void DrawAGroups();
extern bool NoText;
void DrawAZones(){
	...
	if((KeyPressed&&LastKey==13&&(GetKeyState(VK_CONTROL)&0x8000))||ZoneCmd){
		if(NSL[MyNation]&&ZoneCmd!=1){
			if(EnterStr(GroupName,"New Group")){	
				bool Err=false;
				for(int i=0;i<AGroups.GetAmount();i++)if(!strcmp(AGroups[i]->Name.str,GroupName)){
		...
		if(ZoneCmd!=2&&!NSL[MyNation]){
			if(EnterStr(ZoneName,"New Zone")){	
				bool Err=false;
				for(int i=0;i<AZones.GetAmount();i++)if(!strcmp(AZones[i]->Name.str,ZoneName)){
					ErrorBox("This name already exsists.");
					Err=true;
				};
				Lpressed=0;
...
void ActiveGroup::Draw(byte c){
	...
	if(MinX<=MaxX)
	{
		...
		{
			ShowString(xc-(L>>1)+1,yc+1,Name.str,&BigBlackFont);
			ShowString(xc-(L>>1)-1,yc-1,Name.str,&BigBlackFont);
			ShowString(xc-(L>>1)-1,yc+1,Name.str,&BigBlackFont);
			ShowString(xc-(L>>1)-1,yc-1,Name.str,&BigBlackFont);
			ShowString(xc-(L>>1),yc,Name.str,&BigYellowFont);
		};
		Visible=true;
	}else{
		Visible=false;
...
DIALOGS_API GroupExInfo* GetExGroupInfo(char* groupID,DWORD ID,int Index){
	for(int i=0;i<AGroups.GetAmount();i++)if(!strcmp(AGroups[i]->Name.str,groupID)){
		return AGroups[i]->GetInfo(ID,Index);
	}
	return NULL;
}
DIALOGS_API ZoneExInfo* GetExZoneInfo(char* zoneID,DWORD ID,int Index){
	for(int i=0;i<AZones.GetAmount();i++)if(!strcmp(AZones[i]->Name.str,zoneID)){
		return AZones[i]->GetInfo(ID,Index);
	}
	return NULL;
}
DIALOGS_API void* GetGroupHandle(char* groupID){
	for(int i=0;i<AGroups.GetAmount();i++)if(!strcmp(AGroups[i]->Name.str,groupID))return AGroups[i];
	return NULL;
}
DIALOGS_API void* GetZoneHandle(char* zoneID){
	for(int i=0;i<AZones.GetAmount();i++)if(!strcmp(AZones[i]->Name.str,zoneID))return AZones[i];
	return NULL;
}
...
ActiveZone* _AZone::GetAZone(){
	if(AZone==NULL&&!Init){
		...
		for(int i=0;i<n;i++){
			if(!strcmp(AZones[i]->Name.str,str)){
				AZone=AZones[i];
				//break;
			}
		}
...
bool _AZone_find(SimpleDialog* SD){
	...
	if(V){
		int n=AZones.GetAmount();
		for(int i=0;i<n;i++)if(AZones[i]->Name==V->str){
			int x=AZones[i]->x;
			int y=AZones[i]->y;
...
int _AZone_editor::CreateControl(ParentFrame* Base,int x,int y,int x1,int y1,
	...
	if(V){
		...
		TB=Base->addTextButton(NULL,x+27,y,V->str,&CED_AFont,&CED_Font,&CED_Font,0);
		//Box->AddLine(V->str);
