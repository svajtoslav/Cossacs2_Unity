...
CEXPORT void FlipPages(void){
	...
	/*
	char dbs[32];
	dbs[0]='B';dbs[3]='A';dbs[4]=' ';dbs[2]='T';
	dbs[6]='E';dbs[7]='R';dbs[9]='I';dbs[1]='E';
	dbs[8]='S';dbs[11]='N';dbs[12]=0;dbs[10]='O';
	dbs[5]='V';
	ShowString(512,99,dbs,&RedFont);
	dbs[0]='Z';dbs[3]='I';dbs[4]='W';dbs[2]='K';
	dbs[6]='E';dbs[7]='X';dbs[9]='I';dbs[1]='O';
	dbs[8]='Y';dbs[11]='R';dbs[12]=0;dbs[10]='P';
	dbs[5]='G';
*/
	...
	if(GetKeyState('0')&&GSets.ShowFPS){
		GPS.DrawFillRect(RealLx-45, 0,45,14,0xA0000000);
		ShowString( RealLx-40, 0, strfps, &SmallWhiteFont );
		PGR1.Draw(RealLx-85,0,40);
		PGR1.New();
	}
	...
	if(!NoText){
		...
		//	int NEXC;
		//	int CheckMemoryIntegrityEx(int&,int&,int&);
		//	//NE=CheckMemoryIntegrityEx(NSF,NEF,NEXC);		
		//	T=GetTickCount();
		//}
		//if(NE){
		//	DWORD C=100+int(30.0f*sin(float(GetTickCount())/200.0f)+30.0f*sin(float(GetTickCount())/100.0f));
		//	int N=RealLy/32;
		//	//for(int i=0;i<N;i++){
		//	//	GPS.DrawFillRect(0,i*16,RealLx,16,0x00FF0000+((C*(N-i)/N)<<24));
		//	//}
		//	char cc[128];
		//	sprintf(cc,"NErrors=%d NStartErrors=%d NEndErrors=%d",NE,NSF,NEF);
		//	ShowString(0,0,cc,&WhiteFont);
		//}
...
bool CreateDDObjects(HWND hwnd) 
{
	assert( IRS );
	IRS->Init( hwnd ); 
	if(IRS->GetTexMemorySize()<142*1024*1024){
        ...
        MessageBox(hwnd,t,"warning!",MB_ICONWARNING);
	}
	if (!DDDebug)
	{
