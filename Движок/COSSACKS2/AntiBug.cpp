...
void SmartAssert(const char* File,int Line,const char* expression){
	...
	unsigned buff[8];
	int N=FEX_GetCallStackEBP(buff,8);
	for(int i=0;i<16;i++){
		_str s1;
		unsigned LineNum;
		char Func[1024];
	...
	if(MessageBox(hwnd,s.str,"Assertion!",MB_ICONWARNING|MB_RETRYCANCEL)==IDCANCEL){
		__debugbreak();
        //exit(0);
	}
}
...
bool CrashBox(char* Header,unsigned _ebp,unsigned _eip){
	...
	unsigned buff[8];
	buff[0]=_eip;
	int N=FEX_GetCallStackEBP_Ex(_ebp,buff+1,7);
	bool present=0;
	for(int i=0;i<8;i++){
		_str s1;
		unsigned LineNum;
	...
	if(!present){
		return false;
		//s+=".PDB file missed of stack failed\n";
	}
	int NSF,NEF,NEXC;
	...
	s2+=s.str;
	FILE* F;
	if(strstr(Header,"CRASH"))F=fopen("\\\\-gec-\\_work\\fastcrash.txt","a");
	else F=fopen("\\\\-gec-\\_work\\suspend.txt","a");
	if(F){
		char nbuf[64]="?";
		DWORD L=63;
		...
		SYSTEMTIME ST;
		GetLocalTime(&ST);
		fprintf(F,"%d.%d.%d %d:%d  %s:%s\n\n%s\n\n",ST.wDay,ST.wMonth,ST.wYear,ST.wHour,ST.wMinute,nbuf,Header,s2.str);
		fclose(F);
	}
	if(strstr(Header,"CRASH")){
		MessageBox(hwnd,s2.str,Header,MB_ICONSTOP|MB_SYSTEMMODAL);	
	}else{
		_str s3;
		s3.print("Message.exe %s",s2.str);
		WinExec(s3.str,0);
	}
	return true;
}
