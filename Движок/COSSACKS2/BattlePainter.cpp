...
void	lvDrawArrow(lvCNode* nd1,lvCNode* nd2,int dx/* =100 */,int dy/* =150 */,DWORD color/* =0xFF00ff00 */){
...
// Active object /////////////////////////////////////////////////////////
lvCStorage* ActivObject = NULL;	// Текущий объет с которым происходит действие.
...
void	lvCRotateCamera::Set(Vector3D* pPOS0,int _TimeD){
	Vector3D	DefPOS(ICam->GetLookAt().x,ICam->GetLookAt().y,ICam->GetLookAt().z);
...
void	lvCRotateCamera::Set(Vector3D* pPOS0,Vector3D* pPOS1,int _TimeD,int type){
	Set(pPOS0,NULL,pPOS1,NULL,_TimeD);
...
void	lvCRotateCamera::Set(int _vGrpID){
...
void	lvCRotateCamera::Draw(){
	...
	if (MoveType==beTO_POS||MoveType==beTO_POSPOS){
		if (!useMapXY){	ICam->Interpolate(Delta,POS0,POS1); }
			...
			float	x = (float)mapx;
			float	y = (float)mapy;
			if (POS0.x!=POS1.x) x = (float)POS0.x + ( (float)POS1.x - (float)POS0.x ) * Delta;
			if (POS0.y!=POS1.y) y = (float)POS0.y + ( (float)POS1.y - (float)POS0.y ) * Delta;
			SetCameraPos(x,y);
		};
	};
	if (MoveType==beTO_POSDIR||MoveType==beTO_POSDIR_POSDIR){
		ICam->Interpolate(Delta,POS0,DIR0,POS1,DIR1);
	};
...
SimpleDialog*	lvCDialogBased::GetSimpleDialog(){
	if (pSD==NULL){
		...
		xmlData.ReadFromFile( sFilePath.str );
		ErrorPager Err;
		...
		while (N--) {
			if (strcmp(lSD_Arr[N]->Name.str,SDName.str)==0){
				pSD = lSD_Arr[N];
				lSD_Arr[N]=NULL;
			};
		};
...
void	lvCDeffFilmMenu::Draw_TEXT(){
	...
	if (pText!=NULL){
		...
		if (pText->SpeakerID.str!=NULL){
			TextInPanel=GetTextByID(pText->SpeakerID.str);
			TextInPanel+="\\";
		};
...
void	lvCDeffFilmMenu::Draw_HNAME(){
	...
	if ((pLHN!=NULL||pRHN!=NULL)&&CII_HeroName_style) {
		...
		if (pLHN&&pLHN->SpeakerID.str!=NULL){
			DS_SetLHName(GetTextByID(pLHN->SpeakerID.str));
		};
		
		if (pRHN&&pRHN->SpeakerID.str!=NULL){
			DS_SetRHName(GetTextByID(pRHN->SpeakerID.str));
		};
...
void	lvCDeffFilmMenu::PlayText(int _canal){
	...
	if (pTetx!=NULL&&CheckFilePath(pTetx->oggFile.str)){
		//void ov_Play(LPCSTR pcszFileName,DWORD dwStream);
		ov_Play(pTetx->oggFile.str,(DWORD)_canal);
	};
};
...
bool	lvCDeffFilmMenu::DS_SetMessage(_str& Message){
	return DS_SetMessage(Message.str);
};
...
bool			ggCDialog::load_DIALOG(const char* FileName){
	...
	if ( state && FName.str!=NULL) {
		xmlQuote	xmlDATA( "DIALOG" );
		if (xmlDATA.ReadFromFile(FName.str)) {
			ErrorPager Err;
...
int 	CSkirmishTASK::delTASK(const char* name){
	...
	if (name!=NULL){
		int N = TASK_arr.GetAmount();
		while (N--) {
			if ( !strcmp( TASK_arr[N]->Name.str, name ) ){
				TASK_arr[N]->PreDelete();
				TASK_arr.DelElement(N);
				delN++;
			};
...
BattlePainter(){
	if (DriveMode()->NodeView==1/* VIEW ALL */||DriveMode()->NodeView==2/* FOR GROUP */) {
		...
		if (DriveMode()->NodeView==1)	NodesMap()->vDraw( DriveMode()->NodeMode() );
