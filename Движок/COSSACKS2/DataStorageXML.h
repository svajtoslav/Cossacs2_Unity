...
//	int Type;
	...
	SAVE(MissZoneBuild);
		...
		SAVE_SECTION(0x00000002);
			REG_ENUM(_index,Node,ALL_vNODES_ON_MAP);
	ENDSAVE;
};
...
// lvCStorage ////////////////////////////////////////////////////////////
class lvCStorage : public BaseClass
{
public:
	lvCStorage();
...
// lvCNode ///////////////////////////////////////////////////////////////
class	lvCNode : public lvCStorage 
{
public:
	lvCNode() : selected(false), R(0), Dir(0), SegmFR(0) {};
	lvCNode(int _x,int _y) : selected(false), x(_x), y(_y), R(0), Dir(0), SegmFR(0) {};
	lvCNode(int _x,int _y,int _R) : selected(false), x(_x), y(_y), R(_R), Dir(0), SegmFR(0) {};
	lvCNode(int _x,int _y,int _R,int _Dir) : selected(false), x(_x), y(_y), R(_R), Dir(_Dir), SegmFR(0) {};;
	lvCNode(int _x,int _y,int _R,int _Dir,int _SegmFR) : selected(false), x(_x), y(_y), R(_R), Dir(_Dir), SegmFR(_SegmFR) {};;
	~lvCNode(){
		vDeleteDynamicData();
		vCLEAN();
	};
...
public:
	...
	virtual	bool	vSetNodeXY			(int _x, int _y);
	virtual	bool	vSetNodeID			(DWORD _id);
	virtual	void	vSetNodeName		(const char* _name);
	virtual	bool	vSetNode			(char* name,DWORD id,int x,int y);
	...
	virtual	void	vDrawDESCR(DWORD pos=2,int dy=50,int dx=10);
	virtual	void	vDraw(DWORD mode=1); // 1-окружность;10-x,y;100-NAME;1000-DESCR.
...
{
public:
	virtual	lvCNode*vAddNode(int x,int y,char* name=NULL,char* descr=NULL) = 0;

	virtual	bool	vDelNodeID(DWORD _id)		= 0;
	virtual	bool	vDelNodePT(lvCNode* pND)	= 0;
	virtual	bool	vDelNode(int _x,int _y,DWORD sqID=0xFFFFFFFF)		= 0;

	virtual	lvCNode*	vGetNode(DWORD _id)		= 0;
	virtual	lvCNode*	vGetNode(int _x,int _y,DWORD sqID=0xFFFFFFFF)	= 0;
};
// CNodesMAP_ST //////////////////////////////////////////////////////////
...
public:
	...
	// Personal
	virtual	lvCNode*vAddNode(int x,int y,char* name=NULL,char* descr=NULL);		// lvCNodeActionsVI
	virtual	bool	vDelNodeID(DWORD _id);										// lvCNodeActionsVI
	virtual	bool	vDelNodePT(lvCNode* pND);									// lvCNodeActionsVI
	...
	virtual	lvCNode*	vGetNode(DWORD _id);									// lvCNodeActionsVI
	virtual	lvCNode*	vGetNode(const char* name);								// lvCNodeActionsVI
	virtual	lvCNode*	vGetNode(int _x,int _y,DWORD sqID=0xFFFFFFFF);			// lvCNodeActionsVI
...
// lvCNodesOnMap ////////////////////////////////////////////////////////
class lvCNodesOnMap : public	ProcEnumerator
{
public:
...
enum	en_vGroup_Order		{ vgNO_ORDERS=0x00000000, vgMOVE=0x00000001, vgATTACK=0x00000002, vgSOME_ORDER=0x00000004, vgBRIG_ORDER=0x00000008 };
enum	en_vGroup_OrderST	{ vgOR_COMPLIT=0, vgOR_TRY=1, vgOR_PROCESS=2, vgOR_WAIT=3 };

// lvCGroup //////////////////////////////////////////////////////////////
class lvCGroup : public lvCStorage
{
public:
	lvCGroup();
	lvCGroup(const char* name);
	~lvCGroup();
	...
	virtual	en_vGroup_OrderST	GetORDER_ST()		{ return ORDER_STATE; };				// Состояние выполнения

	virtual	void	RemoveDeadUnits();
	virtual	void	ScreenOnGroup();
	virtual	void	DrawGroup();
	virtual	void	SetGroupName(char* name);
	virtual	char*	GetGroupName();
	virtual	void	SetGroupDescription(char* descr);
	virtual	char*	GetGroupDescription();
	...
	virtual	OneObject*	GetOneObj_lID(word lID);				// OneObject* по локальному  индексу
					// Adding operation
	virtual	void	AddUnitGID(word id);						// Добавить по глобальному ID
	virtual	void	AddUnitGPT(OneObject* pOB);					// Добавить по глобальному указателю
	virtual	void	AddUnitsFromAGroupID(word id);
	virtual	void	AddUnitsFromAGroupNM(char* name);
	virtual	void	AddSelectedUnits(byte NI=0xFF);				// Добавить выделенных юнитов.
	...
	virtual	void	RemAllUnits();
					// Remove operation
	virtual	int		RemoveNUnitsToCGroup(lvCGroup* pGR,int N=0xFFFF);	// Переместить N юнитов из группы в группу id
	virtual	int		RemoveNUnitsToAGroupID(int id, int N=0xFFFF);		// Переместить N юнитов из группы в статическую группу id
	virtual	int 	RemoveNUnitsToAGroupNM(char* name, int N=0xFFFF);	// Переместить N юнитов из группы в статическую группу name		
					// Operation on units in group
	virtual	int		KillUnits(int N=0xFFFF);					// Убить N юнитов (N==0xFFFF - всех)
	virtual	int		EraseUnits(int N=0xFFFF);					// Убрать (исчезнут) N юнитов (N==0xFFFF - всех)
	...
	virtual	int		GetTotalAmount();							// Кол-во юнитов в группе
	...
	DWORD					id;
	LinearArray<word,_WORD> IDS;
	LinearArray<word,_WORD> SNS;
	en_vGroup_Order			ORDER;
	en_vGroup_OrderST		ORDER_STATE;
	...
	int						NofNE;			// кол-во новых элементов в группе
			...
			class ADD_AGroup : public BaseFunction
			{
			public:
				int		AGroupsID;
				void EvaluateFunction(){
					...
					if (pGR!=NULL){
						if (0<=AGroupsID&&AGroupsID<AGroups.GetAmount()){
							pGR->AddUnitsFromAGroupID(AGroupsID);
						};
					};
				};
				SAVE(ADD_AGroup);
					REG_PARENT(BaseFunction);
					REG_ENUM(_index,AGroupsID,ALL_GROUPS_ON_MAP);
				ENDSAVE;
			} fADD_AGroup;
...
#ifdef __LUA__
	// for use lua script ////////////////////////////////////////////////////
	bool		call_lua_init;			// Call function from lua for init
	_str		lua_init_func_name;		// Name of lua init function for call
	int 		UseByLua;				// Group now control by lua
	_str		lua_main_func_name;		// Name of lua main function for call
	void		LUA_call(lua_State* L);	// call function by name "int main_lua()"
	bool		lua_error;

	class SET_LUA_SCRIPT : public BaseFunction
	{
	public:
		void	EvaluateFunction();
		_str	init_func_name;
		...
		SAVE(SET_LUA_SCRIPT);
			REG_PARENT(BaseFunction);
			REG_AUTO(init_func_name);
			REG_AUTO(main_func_name);
		ENDSAVE;
	}fSET_LUA_SCRIPT;
...
#ifdef __LUA__
		REG_AUTO(fSET_LUA_SCRIPT);
#endif

		REG_LOADSAVE("temp\\","lvCGroup.xml");
	ENDSAVE;
};

class lvCGroupSmart : public lvCGroup
{
public:
...
public:
	...
	lvCGroup*		GetGroupNM(const char* name);
	lvCGroup*		GetGroupID(DWORD id);
	...
	ClassArray<lvCGroup>	GRP_donotMove;				// List of donot move groups
	bool			__CheckMove(OneObject* pOB);
	bool			__AddDonMoveGrp(lvCGroup* pGrp);
	bool			__RemDonMoveGrp(lvCGroup* pGrp);
...
// lvCGroupsOnMap ////////////////////////////////////////////////////////
class lvCGroupsOnMap : public	ProcEnumerator
{
public:
	virtual DWORD GetValue(const char* ID);
	virtual char* GetValue(DWORD ID);
...
// Triger ////////////////////////////////////////////////////////////////
class	vvTRIGER : public vvBASE
{
public:
			 vvTRIGER()					{ InfID=_vvTRIGER_; Value=true; };
	virtual	~vvTRIGER()					{};
	...
	SAVE(vvTRIGER);
		REG_LOADSAVE("temp\\","vvTRIGER.xml");
		REG_PARENT(vvBASE);
		SAVE_SECTION(0x00000001);
...
// Word //////////////////////////////////////////////////////////////////
class	vvWORD : public vvBASE 
{
public:
			 vvWORD()					{ InfID=_vvWORD_; Value=0; };
	virtual	~vvWORD()					{};
...
// Integer ///////////////////////////////////////////////////////////////
class	vvINTEGER : public vvBASE
{
public:
			 vvINTEGER()		{ InfID=_vvINTEGER_; Value=0; };
	virtual	~vvINTEGER()		{};
	...
	SAVE(vvINTEGER);
		REG_LOADSAVE("temp\\","vvINTEGER.xml");
		REG_PARENT(vvBASE);
		SAVE_SECTION(0x00000001);
...
// lvCVValuesOnMap ///////////////////////////////////////////////////////
class lvCVValuesOnMap : public	ProcEnumerator
{
public:
	virtual DWORD GetValue(const char* ID);
	virtual char* GetValue(DWORD ID);
...
public:
	...
	SAVE(IIPara);
		REG_ENUM(_index,first_element,ALL_vGROUPS_ON_MAP);
...
public:
	...
	virtual	void	vSetBegEndNodePT(lvCNode* _beg,lvCNode* _end);
	...
	virtual	void	vSetEdgePT(lvCNode* _beg,lvCNode* _end,int _pT,int _sT,DWORD _mode,DWORD _id=0,char* name=NULL,char* descr=NULL);
...
public:
	...
	virtual	bool	vAddEdgePT(lvCNode* _beg,lvCNode* _end,int _pT,int _sT,DWORD _mode,char* name=NULL,char* descr=NULL);
...
public:
	...
	// FOR vGROUP
	virtual	void	AddPlan_vGRP(lvCGroup* pGRP);
	virtual	void	DelPlan_vGRP(lvCGroup* pGRP);
...
public:
	...
	ClassArray<vvBASE>	paramList;		// only ( _vvINTEGER_ || _vvTEXT_ || _vvPICTURE_ )
...
public:
	...
	_str	FileName;
	SAVE(CFileList);
		REG_FILEPATH(FileName,".lua,.luac,.script");
	ENDSAVE;
};
...
public:
	...
	virtual DWORD GetSaverID()						{return 'BE'+'EDIT';};
			...
			bool ResetAllClass();
			bool SaveToXML(xmlQuote& xml);
			bool LoadFromXML(xmlQuote& xml);
public:
